...
%DIF DEL jetlets_survey_article-oldtmp-16608.tex   Wed May  4 12:09:09 2016
%DIF ADD jetlets_survey_article.tex                Wed May  4 11:45:40 2016
...
%DIF -------
\usepackage{graphicx, overpic} %DIF > 
%DIF -------
...
%DIF -------
 \newcommand{\hoj}[1]{\todo[inline,color=blue!20]{HOJ: #1}} %DIF > 
%DIF -------
...
\graphicspath{{./images/}}
%DIF 48-49c48-49
%DIF < \title[Dual pairs and jetlet methods for ideal fluids]%
%DIF < {Dual pairs and jetlet methods for ideal incompressible fluid models in $n \geq 2$ dimensions}
%DIF -------
\title[Weak dual pairs and jetlet methods for ideal fluids]% %DIF > 
{Weak dual pairs and jetlet methods for ideal incompressible fluid models in $n \geq 2$ dimensions} %DIF > 
%DIF -------
...
%DIF 54c54
%DIF < \address{$^1$ Department of Mathematics, Imperial College London, London SW7 2AZ}
%DIF -------
\address{$^1$ Department of Mathematics, Imperial College London, London SW7 2AZ, UK} %DIF > 
%DIF -------
\author{D.~M. Meier$^2$}
...
\begin{abstract}
  ...
  fluid systems.
  \DIFdelbegin \DIFdel{These dual pairs result }\DIFdelend \DIFaddbegin \DIFadd{In our case we have a dual pair resulting }\DIFaddend from the action of
  diffeomorphisms on point particles (essentially by moving the points).
  We then augment our dual \DIFdelbegin \DIFdel{pairs }\DIFdelend \DIFaddbegin \DIFadd{pair }\DIFaddend by considering the action of diffeomorphisms
  on Taylor series, also known as \emph{jets}.
  The augmented \DIFaddbegin \emph{\DIFadd{weak}} \DIFaddend dual pairs induce a hierarchy of particle-like solutions
...
\DIFaddbegin \DIFadd{It was later realized that the use of dual pairs related to fluid applications was problematic and needed to be relaxed.
This lead to the notion of weak dual pairs \mbox{%DIFAUXCMD
\cite{GayBalmazVizman2012}
}%DIFAUXCMD
.
}\DIFaddend In this paper we will derive a hierarchy of different \DIFaddbegin \DIFadd{weak }\DIFaddend dual pairs, in order to obtain a class of finite-dimensionally parametrized solutions,
...
After the introduction, in Section~\ref{sec:approach}, we outline our strategy: The first goal is to show that jetlet models admit a \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair at each level in the hierarchy. Once this has been done, Theorem~\ref{thm:dual_pairs} (proven in the appendix) does the rest. Namely, the jetlets satisfy canonical Hamiltonian equations, and the right momentum map is conserved by the flow.
...
In Section~\ref{sec:first_order} we consider first order jetlets. Everything here goes in parallel, except that one has to be more careful when introducing the relevant right and left actions. Once one has the definitions in place, it is not difficult to recognize that the cotangent lifts for the right and left actions lead to\DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{, now, a }\emph{\DIFadd{weak}} \DIFaddend dual pair of momentum maps\DIFdelbegin \DIFdel{(this is mainly }\DIFdelend \DIFaddbegin \DIFadd{.
We have to add `weak' here since the transitivity of the left action on level sets of right momentum is lost, but a weak dual pair is retained, essentially }\DIFaddend because the group actions \DIFdelbegin \DIFdel{commute)}\DIFdelend \DIFaddbegin \DIFadd{still commute, see Definition~\ref{def:weak_dual_pair}}\DIFaddend .  Proposition~\ref{prop:1-solutions} collects the results that follow immediately from the \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair.

Throughout the paper we have taken care to explain the intuition behind the more abstract concepts. The discussion just after Proposition~\ref{prop:1-solutions} is an example of \DIFdelbegin \DIFdel{that}\DIFdelend \DIFaddbegin \DIFadd{this}\DIFaddend . There, we discuss the relationship between the jetlet solutions on the diffeomorphism group and the resulting trajectory on the space of Taylor jets.

In Section~\ref{sec:higher_order} we describe jetlets at general levels. Again, once the relevant spaces and actions have been introduced (which is now quite an intricate endeavor), one recognizes that the momentum maps are a \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair by the usual arguments. We give the form of the momentum maps explicitly just after Proposition~\ref{HL_action_prop}. Again, the \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair leads to analogous conclusions about dynamics. Namely, it is canonically Hamiltonian, with $J_R$  conserved; $J_L$ maps the dynamics to the Lie-Poisson dynamics on the one-form densities.

In Section~\ref{sec:Kelvin} we discuss in more detail the Kelvin circulation theorem. More precisely, we discuss the relationship between the `standard' circulation theorem for the fluid and the conserved momentum maps $J_R$  at the various levels of jetlets. The main result is represented schematically and proven in detail (all that is required, in essence, is the standard formula for cotangent lift momentum maps). Intuitively speaking, what we show is that the conserved jetlet momentum maps are `shadows' of the `full' \DIFdelbegin \DIFdel{right-momentum }\DIFdelend \DIFaddbegin \DIFadd{right momentum }\DIFaddend map of the fluid.\\
...
\DIFdelbegin \DIFdel{The Appendix is relatively extensive. A quick }\DIFdelend \DIFaddbegin \DIFadd{We provide a detailed appendix. In Appendix A, a brief }\DIFaddend discussion of symplectic and Poisson manifolds is followed by the definition of \DIFaddbegin \DIFadd{weak }\DIFaddend dual pairs. Then we prove Theorem~\ref{thm:dual_pairs}\DIFdelbegin \DIFdel{and Theorem~\ref{thm:commuting_actions}, which are the core results }\DIFdelend \DIFaddbegin \DIFadd{, which is the core result used in the paper. Appendix \ref{app:diagramatic} presents an overview over the spaces }\DIFaddend used in the paper \DIFaddbegin \DIFadd{and relates them to some general results of reduction theory, see in particular Figures \ref{fig_diagram_1} and \ref{fig_diagram_2}}\DIFaddend . 
...
\emph{vortex methods} \cite{Chorin1973}, do not exhibit structures which express the nested character of the diffeomorphism group.
 ...
 It is notable that the zeroth level of the hierarchy has been studied \DIFdelbegin \DIFdel{for a long time }\DIFdelend in the context of partial differential equations with hydrodynamic background.
...
\label{sec:approach}
\DIFdelbegin \DIFdel{In this section we describe }\DIFdelend \DIFaddbegin \DIFadd{Let us begin by describing }\DIFaddend our strategy for obtaining
...
here for convenience.
  \begin{quote}
    ...
    a symplectic manifold.  Let $J_1,J_2:S \to P_1,P_2$ form a \DIFdelbegin \DIFdel{dual pair 
    (see definition on page \pageref{thm:dual_pairs})}\DIFdelend \DIFaddbegin \DIFadd{weak dual pair,
    see Definition~\ref{def:weak_dual_pair}}\DIFaddend .
    Let $h \in C^1(P_1)$.
...
We will find a \DIFaddbegin \DIFadd{(weak) }\DIFaddend dual pair, $J_1,J_2: S \to \mathfrak{g}^*$, where
\begin{enumerate}
...
\end{enumerate}
Theorem~\ref{thm:dual_pairs} applied to the \DIFaddbegin \DIFadd{(weak) }\DIFaddend dual pair $J_1,J_2$
...
\begin{align*}
  h(\Pi) = \frac{1}{2}\Pi \cdot \mathbb{I}^{-1} \cdot \Pi\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
...
The unreduced Hamiltonian is $H(R,P) = h(J_R(R,P))$.
  ...
  Recall that $T^*\SO(3)$ is a symplectic manifold. \DIFdelbegin \DIFdel{It is now easy to
  check that the conditions of
  Lemma~\ref{lem:transitive-action} and Theorem~\ref{thm:commuting_actions} hold, from which
  we can observe }\DIFdelend \DIFaddbegin \DIFadd{The momentum maps
  $J_L,J_R$ arise canonically from the left and right action of
  $\SO(3)$ on $T^* \SO(3)$. Since the actions commute and it can also
  be checked }\DIFaddend that $J_L$ and $J_R$ have symplectically orthogonal
  kernels\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, it follows from~\mbox{%DIFAUXCMD
\cite[Corollary~2.6]{GayBalmazVizman2012}
}%DIFAUXCMD
that }\DIFaddend the diagram
  \begin{alignat*}{2}
...
% In the following proposition we will show this for a general Hamiltonian on $\mathfrak{X}_{\rm div}(\R^n)^*$.
  ...
  \begin{align*}
    \dot{m} + \lie_u [m] = 0 \quad , \quad u = dh(m)%DIF < \label{eq:LP}.
  \DIFaddbegin \DIFadd{. %DIF > \label{eq:LP}
  }\DIFaddend \end{align*}
\end{prop}
\begin{proof}
  ...
  \begin{align*}
    ...
    &= \langle m , \lie_{dh(m)}[ df_v(m) ] \rangle\DIFaddbegin \DIFadd{.
  }\DIFaddend \end{align*}
...
\begin{align*}
  h_{p,\sigma}(m) = \frac{1}{2} \langle m , K_{p,\sigma} * m \rangle_{L^2}\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
...
momentum maps \DIFdelbegin \DIFdel{$J_L,J_R : T^*\SDiff(\R^n) \to \mathfrak{X}_{\rm div}(\R^n)^*$}\DIFdelend \DIFaddbegin \DIFadd{$J_L,J_R\colon T^*\SDiff(\R^n) \to \mathfrak{X}_{\rm div}(\R^n)^*$}\DIFaddend . In particular these maps form the dual pair
\begin{align*}
  ...
  \mathfrak{X}_{\rm div}(\R^n)^*.
\end{align*}
For a mathematically rigorous treatment of this dual pair we refer to\DIFaddbegin \DIFadd{~}\DIFaddend \cite{GayBalmazVizman2012}.
...
Unfortunately, this dual pair does not help us in solving\DIFaddbegin \DIFadd{~}\DIFaddend \eqref{eq:MMDiff}
...
This will yield the particle-like solutions described in \cite{MumfordMichor2013}. In the later parts of the paper we will generalize the treatment to obtain a \DIFaddbegin \emph{\DIFadd{weak}} \DIFaddend dual pair for each level in the hierarchy of particle-like solutions.
...
\begin{align*}
  ...
  \stackrel{ \varphi \in \SDiff(\R^n) }{\longmapsto}
  \big(\varphi^{-1}(q) , (\partial_i\varphi^j(q) p_j) e^i \big)\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
...
\begin{align*}
  J_L^{(0)}( q , p ) = p \otimes \delta_q\DIFaddbegin \DIFadd{,
}\DIFaddend \end{align*}
...
is the space of $C^0$-exact one-forms and $d{\bf x}$ is the canonical volume \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{form }\DIFaddend on $\R^n$
(see Appendix~\ref{sec:measure_valued_momap}).
Since $J_L^{(0)}$ is a \DIFdelbegin \DIFdel{cotangent-lift }\DIFdelend \DIFaddbegin \DIFadd{cotangent lift }\DIFaddend momentum map, it is equivariant and therefore Poisson, again by \cite[Thm~12.4.9]{MandS}.
...
Once one has found a solution $(q(t), p(t))$ to Hamilton's equations on $T^*\mathbb{R}^n$ and thus also a solution to Hamilton's equations on $\mathfrak{X}_{\rm div}(\mathbb{R}^n)^*$, one can proceed to integrate the corresponding time-dependent vector field to obtain the fluid motion $\varphi_t$ in $\SDiff(\mathbb{R}^n)$. It is natural (but not mandatory) to choose the initial map $\varphi_0$ to be the identity or at least to be a diffeomorphism that satisfies $\varphi_0(q_a(0))= q_a(0)$ for all $a$.
 ...
 \begin{alignat*}{2}
  ...
  \end{alignat*}
  is a \DIFaddbegin \DIFadd{(proper) }\DIFaddend dual pair.
...
constructing the right leg of \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{an --- in this case weak --- }\DIFaddend dual pair for the higher levels in the hierarchy of particle-like solutions.
...
\end{prop}
\begin{proof}
	Note that $H = h_{p,\sigma} \circ J_L^{(0)}$ and apply \DIFdelbegin \DIFdel{Theorems~\ref{thm:dual_pairs}and~\ref{thm:commuting_actions}}\DIFdelend \DIFaddbegin \DIFadd{Theorem~\ref{thm:dual_pairs}}\DIFaddend .
  See the remark after \DIFdelbegin \DIFdel{Proposition~\ref{prop:k-solutions} }\DIFdelend \DIFaddbegin \DIFadd{Theorem~\ref{thm:k-solutions} }\DIFaddend for more details on the kernel smoothness condition on $p$.
\end{proof}

\subsection{First order particle-like solutions}
\label{sec:first_order}
In this section we revisit the first order particle-like solutions of \cite{CotterHolmJacobsMeier2014} and discuss their \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair, before extending the treatment to the higher levels of the hierarchy in the subsequent section. 
  ...
  \begin{align*}
    ...
    = p\indices{_a_i} \dx^i \otimes \delta_{q_a^{(0)}}
     -p\indices{_a_i^j} q\indices{_a^k_j} \dx^i \otimes \partial_k \delta_{q_a^{(0)}}\DIFaddbegin \DIFadd{.
  }\DIFaddend \end{align*}
...
\DIFaddend In Proposition~\ref{HL_action_prop} we will generalize this construction to define a right action for the higher levels in the hierarchy of particle-like solutions.
  The action \eqref{1-rightaction} yields the \DIFdelbegin \DIFdel{cotangent-lift }\DIFdelend \DIFaddbegin \DIFadd{cotangent lift }\DIFaddend momentum map defined by the condition
  \begin{align*}
    \langle J_R^{(1)}(q,p) , u \rangle = p\indices{_a_i^j} q\indices{_a^i_k} \partial_j u^k(z_a)\DIFaddbegin \DIFadd{.
  }\DIFaddend \end{align*}
  ...
  \DIFaddend \begin{prop}\label{prop:J1-dual-pair}
    The momentum maps $J_L^{(1)}$ and $J_R^{(1)}$ form a \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair.
  \end{prop}
   \DIFdelbegin %DIFDELCMD < \begin{proof}
%DIFDELCMD <   %%%
\DIFdel{We first check the conditions of Lemma~\ref{lem:transitive-action}.
  Note that $\SDiff(\R^n)$ acts transitively on $Q^{(1)}_N$ since it
  acts transitively on representatives $\psi \in \SDiff(\R^n)$ of
  elements $q$ (i.e.}%DIFDELCMD < \ %%%
\DIFdel{$q^{(0)}_a = \psi(z_a)$ and
  $q^{(1)}_a = D\psi|_{z_a}$, or, $q = \Jet^1_z(\psi)$). Secondly, the
  cotangent-lifted action preserves level sets of $J_R^{(k)}$.
  From~}%DIFDELCMD < \eqref{eq:SDiff_cotangent_action_Q1} %%%
\DIFdel{we see that $\varphi \in
  \SDiff(\R^n)$ acts on the first level of $(q,p) \in T^* Q^{(1)}_N$ as }\begin{align*}\DIFdel{
    (\varphi\cdot q)\indices{_a^i_j} }&\DIFdel{= (D\varphi|_{q^{(0)}_a})\indices{^i_k}\,q\indices{_a^k_j}, }\\
    \DIFdel{(\varphi\cdot p)\indices{_a_i^j} }&\DIFdel{= p\indices{_a_k^j}(D\varphi^{-1}|_{\varphi\cdot q^{(0)}_a})\indices{^k_i}.
  }\end{align*}
  %DIFAUXCMD
\DIFdel{The explicit calculation
  }\begin{align*}\DIFdel{
    J_R^{(1)}(\varphi\cdot(q,p))
    }&\DIFdel{= - p\indices{_a_l^j} (D\varphi^{-1}|_{\varphi\cdot q^{(0)}_a})\indices{^l_i} \,
         (D\varphi|_{q^{(0)}_a})\indices{^i_m}\, q\indices{_a^m_k}
         \dz^k \otimes \partial_j \delta_{z_a} }\\
    &\DIFdel{= - p\indices{_a_l^j} \, \delta^l_m \, q\indices{_a^m_k}
         \dz^k \otimes \partial_j \delta_{z_a} = J_R^{(1)}(q,p)
  }\end{align*}
  %DIFAUXCMD
\DIFdel{shows that~}%DIFDELCMD < \eqref{eq:JR1} %%%
\DIFdel{is preserved under the cotangent-lifted
  action of $\SDiff(\R^n)$. Finally, we must show that
  $(J_R^{(1)})^{-1}(m)$ is a section of $T^* Q^{(1)}_N$ for any $m$ in the range of $J_{R}^{(1)}$. Let
...
%DIFDELCMD <   %%%
\DIFdel{Thus Lemma~\ref{lem:transitive-action} implies that $\SDiff(\R^n)$
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{We will postpone the proof, as this is a special case of a proposition which comes later in the paper (Proposition \ref{prop:k dual pairs}).
}\DIFaddend 
  ...
  \DIFdelbegin \DIFdel{Proposition~\ref{prop:k-solutions} }\DIFdelend \DIFaddbegin \DIFadd{Theorem~\ref{thm:k-solutions} }\DIFaddend for more details on the kernel
  ...
  \begin{align*}
    Q_1^{(k)} \subset \R^n \times \SL(n) \times S^1_2 \times S^1_3 \times \cdots\times S^1_k\DIFaddbegin \DIFadd{,
  }\DIFaddend \end{align*}
...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  \DIFdelbegin \DIFdel{$\langle J_L^{(k)}(q,p) , u \rangle = \langle (q,p) , \Jet_z^{(k)}( u \circ \varphi) \rangle$ }\DIFdelend \DIFaddbegin \begin{equation}\DIFadd{\label{eq:moma_left}
    \langle J_L^{(k)}(q,p) , u \rangle = \langle (q,p) , \Jet_z^{(k)}( u \circ \varphi) \rangle
  }\end{equation}
  \DIFaddend for any $\varphi \in \SDiff(\R^n)$ whose $k$-jet is given by $q$ for any $(q,p) \in T^*Q^{(k)}_N$.
 ...
 \DIFdelbegin \DIFdel{$\langle J_R^{(k)}(q,p) , w \rangle = \langle (q,p) , \Jet_z^{(k)}( T\varphi \circ w) \rangle$ }\DIFdelend \DIFaddbegin \begin{align}\DIFadd{
  \langle J_R^{(k)}(q,p) , w \rangle = \langle (q,p) , \Jet_z^{(k)}( T\varphi \cdot w) \rangle, \label{Moma_right}
   }\end{align}
  \DIFaddend for any $\varphi \in \SDiff(\R^n)$ whose $k$-jet is given by $q$ for any $(q,p) \in T^*Q^{(k)}_N$. \DIFaddbegin \DIFadd{Here, we wrote $T\varphi \cdot w$ for the function obtained by applying the differential of $\varphi$ to $w$.
  }\DIFaddend 
  ...
  \begin{equation}\label{eq:JRk}
    ...
    \sum_{\substack{ |\alpha| \leq k \\ 1 \leq \ell \leq |\alpha| }} \;
      ...
      \sum_{m=1}^\ell (-1)^{|\gamma_m|}\left( \prod_{n \neq m}  \delta_{\gamma_n}\DIFdelbegin \DIFdel{^{[b_n]} }\DIFdelend \DIFaddbegin \DIFadd{^{[j_n]} }\DIFaddend \right) \dz^{j_m} \otimes \partial_{\gamma_m}\delta_{z_a}.
  \end{equation}

  \DIFdelbegin \DIFdel{To see that these are dual pairs, we simply generalize the proof of Proposition~\ref{prop:J1-dual-pair}. We first check the conditions
  of Lemma~\ref{lem:transitive-action}. Note that $\SDiff(\R^n)$ acts
  transitively on $Q^{(k)}_N$ because groups always act transitively on equivalence classes, and $Q^{(k)}_{N} := \SDiff(\R^{n}) / \iso^{(k)}(z)$.
  Secondly, the cotangent-lifted
  action preserves level sets of $J_R^{(k)}$.
	To see this, let
  $\varphi \in \SDiff(\R^n)$ }\DIFdelend \DIFaddbegin \begin{prop} \label{prop:k dual pairs}
  	\DIFadd{The maps $J_L^{(k)}$ }\DIFaddend and \DIFdelbegin \DIFdel{$(q,p) \in T^* Q^{(k)}_N$, then we
  have
  }\begin{align*}\DIFdel{
    \langle J_R^{(k)}\big(\varphi\cdot(q,p)\big), w \rangle
    }&\DIFdel{= \langle T^{*} \varphi \cdot (q,p) , ( \varphi^{-1} \cdot q ) \cdot w \rangle }\\
    &\DIFdel{= \langle T^{*} \varphi \cdot (q,p) ,  T\varphi^{-1} \cdot (q \cdot w) \rangle }\\
    &\DIFdel{= \langle (q,p) , T \varphi \cdot T\varphi^{-1} (q \cdot w) \rangle }\\
    &\DIFdel{= \langle (q,p) , q \cdot w \rangle = \langle J_{R}^{(k)}(q,p) , w \rangle,
  }\end{align*}
  %DIFAUXCMD
\DIFdel{where we used on the second line that the actions commute.
	Finally, we must show that $(J_R^{(k)})^{-1}(m)$ is a section of $T^* Q^{(k)}_N$
  for any $m$ in the range of $J_{R}^{(k)}$.
  Let $(q,p) \in \big(J_R^{(k)}\big)^{-1}(m)$ and let
  $\psi \in \SDiff(\R^n)$ such that $q = \Jet^{k}_z(\psi)$. 
	Then
  $T^{*}\psi \cdot (q,p) = (\Jet^k_z(\rm{id}),\pi)$ for some $\pi$.
	Since the identity jet $\bar{q} := \Jet^k_z(\rm{id})$ satisfies
  $\bar{q}\indices{_a^i_j} = \delta^i_j$ and $\bar{q}\indices{_a^i_{[j_1 \cdots j_\ell]}} = 0$ for $l > 1$, we
  find that~}%DIFDELCMD < \eqref{eq:JRk} %%%
\DIFdel{reduces to }\begin{displaymath}\DIFdel{
    J_R^{(k)}(\bar{q},\pi) = \sum_{a=1}^N
    \sum_{|\alpha| \leq k}
			\pi\indices{_a_i^\alpha} (-1)^{|\alpha|} \dz^i \otimes \partial_\alpha \delta_{z_a}.
  }\end{displaymath}
  %DIFAUXCMD
\DIFdel{Since the points $z_a$ are distinct this mapping
  $J_R^{(k)}(\bar{q},\,\cdot\,)$ is clearly injective into
  $\mathfrak{X}_{\rm div}(\R^n)^*$.
	By invariance of }\DIFdelend $J_R^{(k)}$ \DIFdelbegin \DIFdel{under
  the action of $\SDiff(\R^n)$ it follows that
  $\pi \in T^*_{\bar{q}} Q^{(k)}_N$ is the unique element such that $J_R^{(k)}(\bar{q},\pi) = m$}\DIFdelend \DIFaddbegin \DIFadd{form a weak dual pair.
  }\end{prop}
  \begin{proof}
  	 \DIFadd{The result is a direct application of \mbox{%DIFAUXCMD
\cite[Corollary 2.8]{GayBalmazVizman2012}
}%DIFAUXCMD
. To make the exposition more self-contained, we provide some details. By \mbox{%DIFAUXCMD
\cite[Corollary 2.6]{GayBalmazVizman2012}
}%DIFAUXCMD
we need only show that $J_L$ and $J_R$ are equivariant, and that $J_L$ is invariant under the right action of $\iso(z)$.
	Equivariance follows from the fact that $J_R$ and $J_L$ are derived from cotangent lifted group actions \mbox{%DIFAUXCMD
\cite[Corollary 4.2.11]{FOM}
}%DIFAUXCMD
.
	...
	Explicitly, any element $q \in Q^{(k)}$ is expressible as the $k$-jet of some diffeomorphism $\varphi$}\DIFaddend ,
	and \DIFdelbegin \DIFdel{since $\SDiff(\R^n)$ acts by
  fiber-preserving diffeomorphisms on $T^* Q^{(k)}_N$, it follows that $(J_R^{(k)})^{-1}(m)$ is a section of $T^* Q^{(k)}_N$.
  }\DIFdelend \DIFaddbegin \DIFadd{$\rho( q , \psi) = \Jet_z^{(k)}( \varphi \circ \psi)$.
	}\DIFaddend 

	\DIFdelbegin \DIFdel{Thus Lemma~\ref{lem:transitive-action} implies that $\SDiff(\R^n)$
  acts transitively on the level sets of the momentum map $J_R^{(k)}$
  and by Theorem~\ref{thm:commuting_actions}, $J_L^{(k)}$ }\DIFdelend \DIFaddbegin \DIFadd{Similarly, any element of the tangent fiber $T_\varphi \SDiff(\R^n)$ may be written as a composition $u \circ \varphi$ for some $u \in \mathfrak{X}_{\rm div}(\R^n)$.
	...
	\DIFadd{for $(q,v) \in TQ^{(k)}$ and where $u \in \mathfrak{X}_{\rm div}(\R^n)$ }\DIFaddend and \DIFdelbegin \DIFdel{$J_R^{(k)}$ form a dual pair . This }\DIFdelend \DIFaddbegin \DIFadd{$\varphi \in \SDiff( \R^n)$ are arbitrary up to the constraint $(q,v) = \Jet_{z}^{(k)}( u \circ \varphi)$.
	...
	\DIFadd{for all $(q,p) \in T^*Q^{(k)}$ and $(q,v) \in TQ^{(k)}$.
	This action is equivalent to the one defined in the discussion preceding \mbox{%DIFAUXCMD
\cite[Corollary 4.2.11]{FOM}
}%DIFAUXCMD
.
	...
	\DIFadd{where $\bar{\varphi} \in \SDiff(\R^n)$ is any diffeomorphism such that $\Jet_z^{(k)}( \bar{\varphi} ) = \rho( q , \psi^{-1} )$.
	If we let $\varphi$ be such that $\Jet^{(k)}_z( \varphi ) = q$ then we can simply choose $\bar{\varphi} = \varphi \circ \psi^{-1}$.
	Thus we get
	}\begin{align*}\DIFadd{
			\langle J_L( \rho^*( \psi , (q,p) ) ) , u \rangle }&\DIFadd{= \langle  \rho^*( \psi ,  (q,p) ) , \Jet^{(k)}_z ( u \circ \varphi \circ \psi^{-1} ) \rangle }\\
			&\DIFadd{= \langle  \rho^*( \psi ,  (q,p) )  ,  \rho( \Jet_z^{(k)}(u \circ \varphi) , \psi^{-1} ) \rangle }\\
			&\DIFadd{= \langle (q,p) , \Jet_z^{(k)}( u \circ \varphi) \rangle }\\
			&\DIFadd{= \langle J_L(q,p) , u \rangle, 
	}\end{align*}
	\DIFadd{using~}\eqref{eq:tangent action} \DIFadd{and~}\eqref{eq:dual action} \DIFadd{in the second and third equality.
	Thus, we see that $J_L$ is invariant under the right action of $\iso(z)$ on $Q^{(k)}$ and the result follows.
  }\end{proof}

  \DIFadd{In the case of $k=1$ we obtain the weak dual pair of the previous section and Proposition \ref{prop:J1-dual-pair} is a corollary of Proposition \ref{prop:k dual pairs}.
  Proposition \ref{prop:k dual pairs} }\DIFaddend gives us the final result on
  jetlet parametrized solutions.
  \DIFdelbegin %DIFDELCMD < \begin{prop}\label{prop:k-solutions}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \begin{thm}\label{thm:k-solutions}
    \DIFaddend Let $p \ge \frac{n}{2} + k + 1$ and $\sigma > 0$.
  ...
  \DIFdelbegin %DIFDELCMD < \end{prop}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \end{thm}
  \DIFaddend Remark that $K_{p,\sigma}$ has smoothness $C^{2p-n-1}$ by considering
...
\DIFaddbegin \DIFadd{weak}\footnote{%DIF > 
  \DIFadd{With $\SDiff(\R^n)$ as right action this would be a proper dual
...
By applying Theorem\DIFdelbegin \DIFdel{\ref{thm:dual_pairs} with $J_1 = J_\text{conv}$ and $J_2 = J_\text{spatial}$ }\DIFdelend \DIFaddbegin \DIFadd{~\ref{thm:dual_pairs} with $J_1 = J_\text{spatial}$
and $J_2 = J_\text{conv}$ }\DIFaddend we know that Hamiltonian dynamics on
...
\begin{equation}\label{eq:abstract-circulation}
  ...
  \DIFaddbegin \qquad \DIFadd{\text{for all } w \in \mathfrak{iso}(z)}\DIFaddend
\end{equation}
\DIFdelbegin\DIFdel{for all $w \in \mathfrak{X}_{\rm div}(\R^n)$.}\DIFdelend
This instance of Noether's theorem applies to regularized models as well as to the non-regularized case of Euler's \DIFaddbegin \DIFadd{fluid equations}\DIFaddend .
...
\begin{equation*}
  w(x) =  \gamma'_0(s) \otimes \delta(x-\gamma_0(s))\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation*}
Assuming \DIFdelbegin \DIFdel{$\epsilon$ }\DIFdelend \DIFaddbegin \DIFadd{$\varepsilon$ }\DIFaddend is small and writing
...
\begin{align*}
  ...
  &= \int_{\mathbb{R}^n} \langle \dot{\varphi}(x) , \textrm{D}\varphi(x)\cdot w\DIFdelbegin \DIFdel{_\epsilon}\DIFdelend \DIFaddbegin \DIFadd{_\varepsilon}\DIFaddend (x) \rangle_{\mathbb{R}^n} \,\textrm{d} x \\
...
\begin{equation*}
	\langle \mathscr{K}(\gamma) , m \rangle = \int_{\gamma} \tilde{m}\DIFaddbegin \DIFadd{.
}\DIFaddend \end{equation*}
...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and the corresponding quotient map is given by $\Jet_z^k$.
 \DIFaddbegin \DIFadd{Let us also introduce the right action $R\colon \SDiff(\R^n) \times \iso(z) \to \SDiff(\R^n)$ given by right composition of functions and write $R_\psi = R(\cdot , \psi)$, so that $R_\psi \varphi = \varphi \circ \psi$. We also introduce the cotangent lifted right action $TR^*$ by means of the defining relation
 }\begin{equation}\DIFadd{\label{eq:cotangent-action}
 	\langle TR^*_{\psi^{-1}}p_\varphi, v_\varphi \circ \psi \rangle = \langle p_\varphi, v_\varphi\rangle
 }\end{equation}
 \DIFadd{for arbitrary $p_\varphi \in T_\varphi^*\SDiff(\R^n)$ and $v_\varphi \in T_\varphi\SDiff(\R^n)$.
}

 \DIFaddend The space $T^*Q^{(k)}_N$ naturally embeds into $T^*\SDiff(\R^n) / \iso^{(k)}(z)$\DIFdelbegin \DIFdel{in the sense that
 }\DIFdelend \DIFaddbegin \DIFadd{, where the quotient is by the cotangent lifted action. More precisely, }\DIFaddend for any $(q,p) \in T^*Q^{(k)}_N$ \DIFdelbegin \DIFdel{and any }\DIFdelend \DIFaddbegin \DIFadd{we can construct the corresponding element $i(q,p) \in T^*\SDiff(\R^n) / \iso^{(k)}(z)$ in the following way: take any $\varphi$ such that $\Jet_z^k(\varphi) = q$ and find $\iota_\varphi(q,p) \in T_\varphi^*\SDiff(\R^n)$ that satisfies, for all }\DIFaddend $v_\varphi \in T_\varphi \SDiff(\mathbb{R}^n)$\DIFdelbegin \DIFdel{with $\Jet_z^k(\varphi) = q$ we can consider the element $i(q,p) \in T^*\SDiff(\R^n)$ given by $\langle i(q,p) , v_\varphi \rangle = \langle p , v_\varphi( \varphi^{-1} \cdot q) \rangle$
 \mbox{%DIFAUXCMD
\cite[(2.2.4)]{HRS}
}%DIFAUXCMD
.
 The commutativity of the following diagram uniquely defines the jetlet solutions: %DIF <  \begin{equation}
%DIF <   \begin{tikzcd}
%DIF <     \mathfrak{X}_{\rm div}(\R^n)^* & \ar[l,"J_\text{spatial}"] T^*\SDiff(\R^n) \ar[r,"J_\text{conv}"] \ar[d,"\pi"] & \mathfrak{iso}(z)^* \\
%DIF <     & T^*\SDiff(\R^n) / \iso^{(k)}(z) & \\
%DIF <     & T^*Q^{(k)}_N \uar{i} \ar[uur,"J_R^{(k)}"',bend right = 30] \ar[uul,"J_L^{(k)}",bend left = 30] &
%DIF <   \end{tikzcd}
%DIF <  \end{equation}
...
\DIFdelend \DIFaddbegin \DIFadd{,
 }\begin{equation}\DIFadd{\label{eq:iota-map}
   \langle \iota_\varphi(q,p) , v_\varphi \rangle = \langle (q, p) , (v_\varphi \circ \varphi^{-1}) \cdot q \rangle,
 }\end{equation}
 \DIFadd{cf.~\mbox{%DIFAUXCMD
\cite[(2.2.4)]{HRS}
}%DIFAUXCMD
, where we denote by $(v_\varphi \circ \varphi^{-1}) \cdot q$ the infinitesimal action from the left of $v_\varphi \circ \varphi^{-1} \in \mathfrak{X}_{\rm div}(\R^n)$ on $q$.
 Then set $i(q,p) = [\iota_\varphi(q,p)]$.
}\DIFaddend 

 \DIFdelbegin \DIFdel{For the left side of the diagram}\DIFdelend \DIFaddbegin \DIFadd{To see that $i(q,p)$ is well defined}\DIFaddend , note that \DIFdelbegin \DIFdel{the statement $\pi(p_\varphi) = i(p_q)$ for some $p_\varphi \in T^* \SDiff(\mathbb{R}^n)$ and$p_q \in T^*Q_N^{(k)}$ is equivalent to the statement that $\langle p_\varphi , w \circ \varphi \rangle = \langle p_q , \Jet^{(k)}_z ( w \circ \varphi) \rangle$.
From the definitions of $J_\text{spatial}$ and $J_L^{(k)}$ we find 
  }\begin{displaymath}\DIFdel{
  \left< J_\text{spatial}(p_\varphi), w\right> = \left< p_\varphi, w \circ \varphi \right> = \langle p_q , \Jet^{(k)}_z ( w \circ \varphi) \rangle = \left< J_L^{(k)}(p_q), w \right>
  }\end{displaymath}
  %DIFAUXCMD
\DIFdel{when $\pi(p_\varphi) = i(p_q)$.
This verifies the left half }\DIFdelend \DIFaddbegin \DIFadd{if $\varphi' = \varphi \circ \psi$ for $\psi \in \iso^{(k)}(z)$, then for any $v_{\varphi'} \in T_{\varphi'} \SDiff(\R^n)$ we have}
\begin{multline*}\DIFadd{
...
\end{multline*}
 \DIFadd{using~}\eqref{eq:cotangent-action} \DIFadd{and~}\eqref{eq:iota-map} \DIFadd{in the first and second equalities.
 Since $[TR^*_{\psi^{-1}} \iota_\varphi(q,p)] = [\iota_{\varphi'}(q,p)]$, we conclude that $i(q,p)$ is well defined.
}

 \DIFadd{We claim that
 }\begin{equation}\DIFadd{
 	i(q, p) = }[\DIFadd{TR^*_{\varphi^{-1}} J_L^{(k)}(q, p)}]\DIFadd{, \label{i_explicit}
 }\end{equation}
 \DIFadd{where $\varphi$ is such that $\Jet_z^k(\varphi) = q$. This follows since for any $v_{\varphi} \in T_{\varphi}\SDiff(\R^n)$
 }\begin{equation*}\DIFadd{
...
\DIFadd{With these preliminary remarks in mind, we can now show the commutativity of the following diagram:}
...
\end{displaymath}
\DIFadd{Here, $S = (\pi^{-1} \circ i)\bigl(T^*Q^{(k)}_N\bigr)$ and $J_{\rm spatial}$ and $J_{\rm conv}$ are defined in the natural manner through the left and right actions of $\SDiff(\R^n)$ and $\iso(z)$.
To verify the left side }\DIFaddend of the diagram\DIFaddbegin \DIFadd{, we let $(q,p) \in T^*Q^{(k)}_N$ and $\varphi$ such that $\Jet_z^k(\varphi) = q$. Then we obtain from }\eqref{i_explicit} \DIFadd{that for any $w \in \mathfrak{X}_{\rm div}(\R^n)$}
\begin{align*}
	&\DIFadd{\langle [J_\text{spatial}](i(q,p)), w\rangle = \langle J_\text{spatial}( TR^*_{\varphi^{-1}} J_L^{(k)}(q, p)), w\rangle} \\
	\quad&\DIFadd{= \langle TR^*_{\varphi^{-1}} J_L^{(k)}(q, p), w \circ \varphi \rangle
	 = \langle J_L^{(k)}(q, p), w \rangle,}
\end{align*}
\DIFadd{as required. This also shows that $[J_{\rm spatial}]$ is well defined
on $T^*\SDiff(\R^n) / \iso^{(k)}(z)$ since the final expression does
not depend on the choice of representative $\varphi$ for $q$}\DIFaddend .
\DIFdelbegin \DIFdel{The right hand side can be verified in the same manner. }\DIFdelend 

%DIF <  As for the right-hand part, we again use the special form of $p_\varphi$ to conclude that, for all $ w \in \mathfrak{iso}(z) \subset \mathfrak{X}_{\rm div}(\R^n)$,
%DIF <  \begin{equation}
%DIF <  	\left<J_\text{conv}(p_\varphi), w\right> = \left<p_\varphi, T\varphi  (w) \right> = \left<J_L^{(k)}(p_q),  T\varphi (w \circ \varphi^{-1}) \right>. \label{compare_1}
%DIF <  \end{equation}
%DIF <  On the other hand, if we write $\phi_\varphi: Q^{(k)}_N \to Q^{(k)}_N$ for the left action $\phi_\varphi (q) = \varphi \cdot q$, we have 
%DIF <  \begin{equation*}
%DIF <    \left<J_R^{(k)}(p_q), w\right> = \left< p_q, T_{\phi_\varphi} (w_{Q^{(k)}_N} (q_0))   \right> =  \left<p_q, (T\varphi (w \circ \varphi^{-1}) )_{Q^{(k)}_N} (q) \right>,
%DIF <  \end{equation*}
%DIF <  where we denoted the infinitesimal left action of $w$ on $Q^{(k)}_N$ by $w_{Q^{(k)}_N}$. The velocity corresponding (via the inverse of the Legendre transform) to $p_q$ can be written as $ T\Jet_z^k( (K * J_L^{(k)}(p_q)) \circ \varphi)$,
%DIF <  and therefore one concludes that 
%DIF <  \begin{equation}
%DIF <  	\left<J_R^{(k)}(p_q), w\right> = \left< J_L^{(k)}(p_q), T \varphi (w \circ \varphi^{-1}) \right>. \label{compare_2}
%DIF <  \end{equation}
%DIF <  This shows the commutativity of the right-hand part of the  diagram above (compare \eqref{compare_1} and \eqref{compare_2}).
  \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{Again , the right }\DIFdelend \DIFaddbegin \DIFadd{For the right leg, note that 
}\begin{align*}
	&\DIFadd{\langle [J_\text{conv}](i(q,p)), w\rangle = \langle J_\text{conv}( TR^*_{\varphi^{-1}} J_L^{(k)}(q, p)), w\rangle} \\
	&\DIFadd{= \langle  TR^*_{\varphi^{-1}} J_L^{(k)}(q, p), T\varphi \cdot w \rangle = \langle  J_L^{(k)}(q, p), (T\varphi \cdot w) \circ \varphi^{-1} \rangle} \\
	&\DIFadd{= \langle (q,p),  \Jet_z^k(T\varphi \cdot w)\rangle = \langle  J^{(k)}_R(q,p), w\rangle.}
\end{align*}
\DIFadd{Here we used~}\eqref{i_explicit}\DIFadd{, the definition of $J_\text{conv}$,
and~}\eqref{eq:cotangent-action}\DIFadd{, respectively, in the first three equalities
and~}\eqref{Moma_right} \DIFadd{in the final step. Again we see that $[J_\text{conv}]$ is well-defined since it does not
...
\DIFadd{The right }\DIFaddend legs of both \DIFaddbegin \DIFadd{weak }\DIFaddend dual pairs yield the conserved quantities.
...
%DIF < As a side remark we mention that \eqref{compare_2} offers an alternative route to derive one momentum map once one has found the other. In contrast,  in the previous section we computed both $J_L^{(k)}$ and $J_R^{(k)}$ using the coordinate expressions, presented in the proof of Proposition~\ref{HL_action_prop}, for the respective actions on $Q^{(k)}_N$.
\DIFaddbegin \DIFadd{This diagram also provides some insight into the relationship between the developments in this paper and classical Marsden-Weinstein reduction theory \mbox{%DIFAUXCMD
\cite{MarsdenWeinstein1974}
}%DIFAUXCMD
.
For instance, letting $J_{\rm conv}^{(k)}: T^*\SDiff(\R^n) \to \mathfrak{iso}^{(k)}(z)^*$ be the momentum map associated to the cotangent lift of the right action of $\iso^{(k)}(z)$ on $\SDiff(\R^n)$,
 one can show that $i\bigl(T^*Q^{(k)}_N\bigr) = (J_{\rm conv}^{(k)})^{-1}(0) / \iso^{(k)}(z)$.  For more details on the connections between general reduction theory and the results of the present paper, see Appendix \ref{app:diagramatic}.
}\DIFaddend 
...
\begin{equation*}
  ...
  =-\frac{1}{2} \omega_i^k \omega_j^l
   -\frac{1}{4} \big( \sigma_i^k \sigma_j^l + \tau_i^k \tau_j^l \big)\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{,
}\DIFaddend \end{equation*}
...
\begin{equation*}
  ...
  = -\frac{3}{2} j [\omega,\tilde{\mu}]\DIFaddbegin \DIFadd{, 
}\DIFaddend \end{equation*}
...
The initial state is given in \DIFdelbegin \DIFdel{center-of-mass polar-coordinates}\DIFdelend \DIFaddbegin \DIFadd{center of mass polar coordinates}\DIFaddend ,
...
\section{Conclusion}
  In this paper we derived a hierarchy of \DIFaddbegin \DIFadd{weak }\DIFaddend dual pairs
...
\DIFaddbegin \DIFadd{We are indebted to the anonymous referees for very carefully refereeing our article, including catching a problem with our initial use of dual pairs.
}\DIFaddend JE, DDH, HOJ and DMM are grateful for partial support by the European Research Council Advanced Grant 267382 FCCA.

\appendix

\section{Hamiltonian mechanics}
\label{sec:Hamiltonian}
The goal of this section is to prove \DIFdelbegin \DIFdel{Theorems~\ref{thm:dual_pairs} and~\ref{thm:commuting_actions} }\DIFdelend \DIFaddbegin \DIFadd{Theorem~\ref{thm:dual_pairs} }\DIFaddend (see page~\pageref{thm:dual_pairs}).
...
\begin{equation}
  ...
  \left \langle x , \left[ \pder{f}{x} , \pder{g}{x} \right] \right \rangle\DIFaddbegin \DIFadd{, 
  }\DIFaddend \label{eq:Lie-Poisson}
\end{equation}
...
\end{prop}
  ...
  by solving \DIFdelbegin \DIFdel{lower dimensional }\DIFdelend \DIFaddbegin \DIFadd{lower-dimensional }\DIFaddend Hamiltonian equations
  on $P_1$.
  \subsection{\DIFdelbegin \DIFdel{Dual }\DIFdelend \DIFaddbegin \DIFadd{Weak dual }\DIFaddend pairs}
  In this section we review the notion of \DIFdelbegin \DIFdel{dual pairs
  \mbox{%DIFAUXCMD
\cite{MarsdenWeinstein1983,Weinstein1983,GayBalmazVizman2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{weak dual pairs
  \mbox{%DIFAUXCMD
\cite{GayBalmazVizman2012}
}%DIFAUXCMD
.
  This is a relaxation of the more frequently invoked notion of a dual pair \mbox{%DIFAUXCMD
\cite{MarsdenWeinstein1983,Weinstein1983}
}%DIFAUXCMD
}\DIFaddend .
  Let $(S,\omega)$ be a symplectic manifold.
  ...
  \end{equation*}
  \DIFdelbegin \DIFdel{Let $\psi:S \to P_1$ }\DIFdelend \DIFaddbegin \begin{defn}[Weak dual pair \mbox{%DIFAUXCMD
\cite{GayBalmazVizman2012}
}%DIFAUXCMD
]\label{def:weak_dual_pair}
  \DIFadd{Let $J_1\colon S \to P_1$ }\DIFaddend be a Poisson map.
  The kernel of \DIFdelbegin \DIFdel{$\psi$ }\DIFdelend \DIFaddbegin \DIFadd{$J_1$ }\DIFaddend is the distribution
  \begin{equation*}
    \kernel(\DIFdelbegin \DIFdel{\psi}\DIFdelend \DIFaddbegin \DIFadd{J_1}\DIFaddend ) = \{ v \in TS \mid \DIFdelbegin \DIFdel{T\psi }\DIFdelend \DIFaddbegin \DIFadd{TJ_1 }\DIFaddend \cdot v  = 0 \}.
  \end{equation*}
  If \DIFdelbegin \DIFdel{$J:S \to P_2$ }\DIFdelend \DIFaddbegin \DIFadd{$J_2\colon S \to P_2$ }\DIFaddend is a Poisson map as well, and
  \begin{equation*}
    \kernel(J\DIFaddbegin \DIFadd{_2}\DIFaddend )\DIFdelbegin \DIFdel{= }\DIFdelend \DIFaddbegin \DIFadd{^\omega \subset }\DIFaddend \kernel(\DIFdelbegin \DIFdel{\psi}\DIFdelend \DIFaddbegin \DIFadd{J_1) }\quad \DIFadd{, }\quad
    \DIFadd{\kernel(J_1}\DIFaddend )^\omega \DIFaddbegin \DIFadd{\subset \kernel(J_2)
  }\DIFaddend \end{equation*}
  we call the diagram
  \begin{equation*}
    P_1 \DIFdelbegin %DIFDELCMD < \stackrel{\psi}{\longleftarrow} %%%
\DIFdelend \DIFaddbegin \stackrel{J_1}{\longleftarrow} \DIFaddend S \DIFdelbegin %DIFDELCMD < \stackrel{J}{\longrightarrow} %%%
\DIFdelend \DIFaddbegin \stackrel{J_2}{\longrightarrow} \DIFaddend P_2
  \end{equation*}
  a \emph{\DIFaddbegin \DIFadd{weak }\DIFaddend dual pair\DIFdelbegin %DIFDELCMD < }%%%
\DIFdel{.
%DIF <   In the case that $S$ is a cotangent bundle we call $\psi$ or $J$
%DIF <   \emph{Clebsch variables} \cite{MarsdenWeinstein1983}.
}\DIFdelend \DIFaddbegin }\DIFadd{.
  }\end{defn}
  \DIFadd{We would have a proper dual pair if the kernel inclusions were replaced by equalities.
}\DIFaddend 

  \begin{thm} \label{thm:dual_pairs}
    Let $J_1,J_2:S \to P_1,P_2$ form a \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair.
    Let $h \in C^1(P_1)$.
  ...
  \begin{proof}
    ...
    we see that \DIFdelbegin \DIFdel{$\dot{x} \in TS$ is contained
    in the symplectic orthogonal to $\kernel(J_1)$.
    Since }\DIFdelend \DIFaddbegin \DIFadd{$\dot{x} \in \kernel(J_1)^\omega$.
    As }\DIFaddend $J_1$ and $J_2$ form a \DIFaddbegin \DIFadd{weak }\DIFaddend dual pair, this \DIFdelbegin \DIFdel{observation }\DIFdelend implies $\dot{x}  \in \kernel(J_2)$.
    \DIFdelbegin \DIFdel{We may apply this argument at each time $t$ and observe
    }\DIFdelend \DIFaddbegin \DIFadd{Thus we have found
    }\DIFaddend \begin{equation*}
      \frac{d}{dt} J_2( x(t)) = TJ_2 \cdot \dot{x}(t)  
      = \DIFdelbegin \DIFdel{0 }%DIFDELCMD < \quad %%%
\DIFdel{\forall t.
    }\DIFdelend \DIFaddbegin \DIFadd{0.
    }\DIFaddend \end{equation*}
  \end{proof}

\DIFdelbegin \DIFdel{The next theorem and lemma will be used later for the purpose of
  identifying dual pairs. }\DIFdelend \DIFaddbegin \section{\DIFadd{Diagrammatic overview}} \label{app:diagramatic}
\DIFadd{We present here a diagrammatic representation of some of the spaces used in the present paper. We begin by recalling a number of general results that hold for finite-dimensional Lie groups, before we indicate their relevance to the developments in the main text.
}\DIFaddend 

\DIFdelbegin %DIFDELCMD < \begin{thm}[Example 2.2 of \cite{GayBalmazVizman2012}] \label{thm:commuting_actions}
%DIFDELCMD <     %%%
\DIFdel{Denote by $\omega$ the canonical symplectic form on the cotangent bundle $T^*Q$ of some manifold $Q$.
Let $G_1,G_2$ act on }\DIFdelend \DIFaddbegin \begin{figure}[h]
\begin{center}
...
\put(67, 5){$\psi$}
\put(61, 42){$G$}
\put(42, 33) {\DIFaddFL{$g$}}
\put(18, 27.9) {\DIFaddFL{$e$}}
\put(17, 40) {\DIFaddFL{${\tiny \updownarrow} G_{q_0}$ }}
\put(17, 4) {\DIFaddFL{$q_0$ }}
\put(35, 4) {\DIFaddFL{$q = \Pi(q) = \phi_g(q_0)$ }}
\put(25, 8) {\DIFaddFL{$\Pi$ }}
\put(-3, 1) {\DIFaddFL{$Q$ }}
\put(96.5, 22) {\DIFaddFL{$K$ }}
\end{overpic}
\end{center}
%DIF > \includegraphics[scale=0.175,bb=0 0 100 100]{flows2.pdf}
%DIF > \footnotesize
\caption{\DIFaddFL{
\label{fig_diagram_1}
...
\begin{itemize}
\item
\DIFadd{Let a Lie group $G$ act on a manifold }\DIFaddend $Q$ by \DIFdelbegin \DIFdel{commuting group actions. }\footnote{\DIFdel{That is, acting first by $G_1$ and second by $G_2$ is identical to acting first by $G_2$ then second by $G_1$.}}
    %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{Let $\rho_i: G_i \times T^*Q \to T^*Q$ for $i=1,2$, denote the cotangent lifted actions
    and let $J_1: T^*Q \to \mathfrak{g}_1^*$ and $J_2: T^*Q \to \mathfrak{g}_2^*$ denote the momentum maps }\DIFdelend \DIFaddbegin \DIFadd{the action $\phi: G \times Q \to Q$, which we also write as $\phi_g(\cdot) = \phi( g, \cdot )$. If we fix a particular value $q_0 \in Q$, we can construct a mapping $\Pi: G \to Q$ }\DIFaddend given by \DIFdelbegin %DIFDELCMD < \eqref{eq:cotangent_momap}%%%
\DIFdel{. Moreover, assume $G_1$ is infinitesimally }\DIFdelend \DIFaddbegin \DIFadd{$\Pi(g) = \phi_g(q_0)$. Let us assume that the action is transitive, so that $\Pi$ is surjective. We denote by $G_{q_0}$ the isotropy subgroup leaving $q_0$ invariant, that is,
}\begin{equation*}\DIFadd{
  G_{q_0} := \{ g \in G | \phi_g(q_0) = q_0\}.
}\end{equation*}
\DIFadd{Note that $\Pi^{-1}(q) =g  G_{q_0}$ 
for any $g \in G$ such that $\Pi(g) = q$, and hence we can identify $Q$  with $G/G_{q_0}$. Suppose a further Lie group, $K$, also acts on $Q$ with group action $\psi: Q \times K \to Q$, which commutes with $\phi$. This situation arises naturally, for instance, when $K$ is a subgroup of $G$ and, in turn,  $G_{q_0}$ is a normal subgroup of $K$. In that case, one can define the action $\psi$ as
}\begin{equation}\DIFadd{
  \psi_k(q) := \Pi( g k) = \phi_{gk}(q_0), \label{right_ac_general}
}\end{equation}
\DIFadd{and check that $\phi$ and $\psi$ indeed commute:
}\begin{equation*}\DIFadd{
  \phi_s(\psi_k (q)) = \phi_s(\phi_{gk}(q_0)) = \phi_{sgk}(q_0) = \psi_k(\phi_s (q)).
}\end{equation*}
\DIFadd{We refer to Figure \ref{fig_diagram_1} for a representation of the relevant spaces and maps.
}\item \DIFadd{The actions $\phi$ and $\psi$ on $Q$ can be lifted to actions $\Phi$ and $\Psi$ on the cotangent bundle $T^*Q$ in the usual manner (see Figure \ref{fig_diagram_2}). These cotangent lifted actions induce equivariant momentum maps $J_1: T^*Q \to \mathfrak{g}^*$ and $J_2:T^*Q \to \mathfrak{k}^*$, where $\mathfrak{g}^*$ and $\mathfrak{k}^*$ are the duals of the Lie algebras of $G$ and $K$. Due to their equivariance, $J_1$ and $J_2$ are Poisson maps (where the duals of the Lie algebras are equipped with appropriate Lie--Poisson brackets). Since the actions $\phi$ and $\psi$ commute, the action $\Psi$ leaves level sets of $J_1$ invariant, and vice versa. This implies that $J_1$ and $J_2$ are a weak dual pair, and if moreover $\Psi$ is }\DIFaddend transitive on the level sets of \DIFaddbegin \DIFadd{$J_1$ and vice versa, then $J_1$ and }\DIFaddend $J_2$ \DIFdelbegin \DIFdel{in the sense that $\kernel(J_2) \subset \mathfrak{g}_1 \cdot T^*Q$.
...
%DIFDELCMD <     %%%
\DIFdel{Let us first show that $\kernel(J_1) = [\rho_1(\mathfrak{g}_1 , T^*Q )]^\omega$. Recall that on account of the  definition of momentum maps we have 
    $d \langle J_1 , \xi_1 \rangle = i_{\rho_1(\xi_1)} \omega$, 
    where $\rho_1(\xi_1)$ is the infinitesimal generator of $\xi_1 \in \mathfrak{g}_1$.
    ...
    Thus $v \in \kernel(J_1)$. The previous steps may be reversed}\DIFdelend \DIFaddbegin \DIFadd{are a proper dual pair, see~\mbox{%DIFAUXCMD
\cite[Corollary~2.6]{GayBalmazVizman2012}
}%DIFAUXCMD
.
%DIF > 
}\item \DIFadd{Let $\mathcal{H}:T^*G \to \mathbb{R}$ be a right-invariant Hamiltonian. This means that $\mathcal{H}$ is invariant with respect to the  cotangent lift $TR^*$ of the multiplication from the right of $G$ by itself. In particular, the reduced Hamiltonian $h:\mathfrak{g}^* \to \mathbb{R}$ satisfies $\mathcal{H}(\alpha_g) = h \circ TR_g^*(\alpha_g)$ for any $\alpha_g \in T^*G$}\DIFaddend , and the \DIFdelbegin \DIFdel{converse follows immediately.
    We find
    $\kernel(J_1) = [\rho_1(\mathfrak{g}_1,T^*Q)]^\omega$. Of course the same argument applies to $J_2$. }%DIFDELCMD < 

%DIFDELCMD <     %%%
\DIFdel{We now show that $\kernel(J_2) = \rho_1(\mathfrak{g}_1,T^*Q)$.
    Again, let $\xi_1 \in \mathfrak{g}_1, \xi_2 \in \mathfrak{g}_2$.
    Recall that $\omega = - d \theta$ where $\theta$ is the canonical one-form on }\DIFdelend \DIFaddbegin \DIFadd{reduced dynamics in $\mathfrak{g}^*$ are of Lie--Poisson type. The momentum map $J_1$ can be used to induce the so-called }\emph{\DIFadd{collective Hamiltonian}} \DIFadd{$H = h\circ J_1$ on $T^*Q$. Note that }\DIFaddend $T^*Q$ \DIFaddbegin \DIFadd{is a symplectic manifold, and that the symplectic (canonical) dynamics with respect to the collective Hamiltonian are mapped by (the Poisson map) $J_1$ to the reduced dynamics on $\mathfrak{g}^*$}\DIFaddend . Moreover, \DIFdelbegin \DIFdel{recall that $\theta$ is invariant under cotangent lifted group actions \mbox{%DIFAUXCMD
\cite[Theorem 3.2.12]{FOM}
}%DIFAUXCMD
. Thus by Cartan's formula \mbox{%DIFAUXCMD
\cite[Theorem 2.4.13(iv)]{FOM}
}%DIFAUXCMD
we can calculate
    }\begin{displaymath}\DIFdel{
      0 = \lie_{\rho_1(\xi_1)} }[ \DIFdel{\theta}] \DIFdel{= d \langle \theta_1 , \rho_1(\xi_1) \rangle + i_{\rho_1(\xi_1)}(d\theta) = d \langle \theta_1 , \rho_1(\xi_1) \rangle - i_{\rho_1(\xi_1)} \omega, 
    }\end{displaymath}
    %DIFAUXCMD
\DIFdel{where $i_{\rho_1(\xi_1)} \omega$ denotes contraction of a two-form with a vector field.
Finally we calculate
    }\begin{align*}\DIFdel{
    	\omega( \rho_1(\xi_1) , \rho_2(\xi_2) ) }&\DIFdel{=  -\langle d \langle \theta, \rho_1(\xi_1) \rangle ,  \rho_2(\xi_2) \rangle }\\
      &\DIFdel{= -\lie_{\rho_2(\xi_2) }[ \langle \theta,\rho_1(\xi_1) \rangle] = - \langle \lie_{\rho_2(\xi_2)}[\theta] , \rho_1(\xi_1) \rangle - \langle \theta , }[\DIFdel{\rho_2(\xi_2), \rho_1(\xi_1) }] \DIFdel{\rangle.
    }\end{align*}
    %DIFAUXCMD
\DIFdel{The first term in the last line vanishes because $\theta$ is invariant under the flow of $\rho_2(\xi_2)$.
The second term in the last line vanishes because the actions of $G_1$ and $G_2$ commute. Thus $\omega(\rho_1(\xi_1) , \rho_2(\xi_2) ) = 0$, and we have proven symplectic orthogonality.
%DIF <     First note that $\rho_1(\xi_1) = X_{ \langle J_1 , \xi_1 \rangle}$,
%DIF <     and the same calculation applies to $\xi_2$.
%DIF <     Thus we observe by using Corollary~\ref{cor:Lie_hom} that
%DIF <     \begin{align*}
%DIF <       X_{ \omega( \rho_1(\xi_1) , \rho_2(\xi_2) ) }
%DIF <       &= - [X_{ \langle J_1,\xi_1\rangle} , X_{\langle J_2,\xi_2\rangle} ] \\
%DIF <       &= - [\rho_1(\xi_1) , \rho_2(\xi_2)] = 0.
%DIF <     \end{align*}
%DIF <     The final equality
%DIF <     derives from the fact that the actions of $G_1$ and $G_2$ commute.
%DIF <     We have proven that $\rho_1(\mathfrak{g}_1 , S)$ and $\rho_2(S,\mathfrak{g}_2)$ are
%DIF <     symplectically orthogonal.
    Now note that
    $\langle TJ_2 ( \rho_1(\xi_1, x)), \xi_2 \rangle = - \omega( \rho_1(\xi_1, x) ,\rho_2( \xi_2,x )) = 0$. Thus $\rho_1(\xi_1 , x) \in \kernel( J_2)$, hence
...
%DIFDELCMD <   %%%
%DIF <  With these foundations established, we proceed
  %DIF <  to discuss a simple example of Poisson mechanics
  %DIF <  before moving to the more sophisticated case
  %DIF <  of fluids.
%DIFDELCMD < 

%DIFDELCMD < \begin{lem}\label{lem:transitive-action}
%DIFDELCMD <   %%%
\DIFdel{Let $G_1, G_2$ act on the manifold $Q$. Let $\rho_1^*$ denote the cotangent-lifted action associated to $G_1$ and
...
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \DIFadd{$J_2$ is conserved under the dynamics on $T^*Q$. The conservation law follows from Noether's theorem because $J_1$, and hence $H$, are left invariant by $\Psi$ (see Theorem \ref{thm:dual_pairs}).  Note that the elements of $T^*Q$ play the role of symplectic variables (or Clebsch variables in the sense of \mbox{%DIFAUXCMD
\cite{MarsdenWeinstein1983}
}%DIFAUXCMD
).
%DIF > 
}\item \DIFadd{The appeal of Clebsch variables is their symplectic nature. The symmetry of $H$ with respect to $\Psi$ implies that reduced dynamics on $T^*G/H$ can be constructed by symplectic reduction. Note however that the resulting quotient manifold is not symplectic in general.
%DIF > 
}\DIFaddend \item
\DIFdelbegin \DIFdel{$G_1$ acts transitively on $Q$;
  }%DIFDELCMD < \item %%%
\DIFdel{the cotangent-lifted action $\rho_1^*$ preserves level sets of $J_2$;
  }%DIFDELCMD < \item %%%
\DIFdel{for any $\mu$ in the range of $J_2$, the level set $J_2^{-1}(\mu)$ is a section of $T^* Q$. }%DIFDELCMD < \end{enumerate}
%DIFDELCMD <   %%%
\DIFdel{Then the action $\rho_1^*$ is transitive on level sets of }\DIFdelend %DIF > for the following see Lee Thm 23 showing that the isotropy group is closed, the proof of Thm 9.22 showing that the action of a closed Lie subgroup is proper and free; thus Thm 2.2.2 of Hamiltonian Reduction by Stages book applies.
\DIFaddbegin \DIFadd{Note also that there is a symplectic diffeomorphism between $T^*Q$ and $J^{-1}(0)/G_{q_0}$, where $J$ here is the momentum map associated with the cotangent lift of the action (from the right) of $G_{q_0}$ on $G$, see \mbox{%DIFAUXCMD
\cite[Theorem~2.2.2]{HRS}
}%DIFAUXCMD
.
}\end{itemize}
\begin{figure}[h]
...
\put(86.8, 30){$\Psi$}
\put(57, 49){$G$}
\put(44, 41.2) {\DIFaddFL{$e$}}
\put(43.3, 47) {\DIFaddFL{${\tiny \updownarrow} G_{q_0}$ }}
\put(97, 23.5) {\DIFaddFL{$T^*Q$ }}
\put(96.5, 49) {\DIFaddFL{$K$ }}
\put(27, 40){\includegraphics[scale = 0.13]{arrow_left.eps}}
\put(17, 49) {\DIFaddFL{$T^*G$ }}
\put(3, 25) {\DIFaddFL{$\mathcal{H}$ }}
\put(16, 24.5) {\DIFaddFL{$H = h \circ J_1$ }}
\put(22, 12) {\DIFaddFL{$h $ }}
\put(7, 7) {\DIFaddFL{$\mathbb{R} $ }}
\put(52.5, 7) {\DIFaddFL{$\mathfrak{g}^*$ }}
\put(96.5, 7) {\DIFaddFL{$\mathfrak{k}^*$ }}
\put(49.5, 17) {\DIFaddFL{$J_1$ }}
\put(85.5, 17) {\DIFaddendFL $J_2$ \DIFdelbeginFL \DIFdelFL{.
}%DIFDELCMD < \end{lem}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL }
\put(22, 2.5) {\DIFaddFL{$TR^* $ }}
\end{overpic}
\end{center}
%DIF > \includegraphics[scale=0.175,bb=0 0 100 100]{flows2.pdf}
%DIF > \footnotesize
\caption{\DIFaddFL{The cotangent lifted actions induce momentum maps $J_1$ and $J_2$. The Hamiltonian $\mathcal{H}$, assumed to be right-invariant, is also shown, along with the reduced Hamiltonian $H$ and the collective Hamiltonian $H = h \circ J_1$.
}\label{fig_diagram_2}
 }
\end{figure}
\DIFaddend 
...
%DIFDELCMD <   %%%
\DIFdelend %DIF > 
\DIFaddbegin \DIFadd{In translating the above facts to the case of interest in the present paper, one encounters technical subtleties to do with the infinite-dimensionality of $\SDiff(\R^n)$. Nevertheless, bullet-by-bullet parallels can be recognized between the developments in the main text of the paper and the general results above, as we will discuss now. For simplicity, we restrict ourselves in what follows to the case of a single particle, the extension to $N$ particles being straightforward.
}\begin{itemize}
\item \DIFadd{Let $G = \SDiff(\R^n)$, and let $Q= Q_1^{(k)}$ be the space of single-particle $k$-jetlets. We fix the point $q_0 = (z, \mathbf{1}, 0, \ldots, 0) \in Q_1^{(k)}$ corresponding to the Taylor expansion of the identity in $\SDiff(\R^n)$ evaluated at $z \in \R^n$ (cf. Section \ref{sec:higher_order}). We let the projection $\Pi$ be given by ${\rm Jet}_z^{(k)}$, and hence the left action of an element $\varphi \in \SDiff(\R^n)$ on $q \in Q$ is $\phi_\varphi(q) = {\rm Jet}_z^{(k)}(\varphi \circ \rho)$, for any $\rho$ such that ${\rm Jet}_z^{(k)}(\rho) = q$. The role of the isotropy subgroup $G_{q_0}$ is played by ${\rm iso}^{(k)}_z$. }\DIFaddend Let \DIFdelbegin \DIFdel{$(q,p), (q',p') \in J_2^{-1}(\mu)$ where $(q,p) \neq (q',p')$. Since $J_2^{-1}(\mu)$ is a section of $T^* Q$, this implies that $q \neq q'$.
Now $G_1$ acts transitively }\DIFdelend \DIFaddbegin \DIFadd{$K = {\rm iso}(z)$, and note that ${\rm iso}^{(k)}(z)$ is a normal subgroup of ${\rm iso}(z)$ (\mbox{%DIFAUXCMD
\cite[Proposition 4.1]{JacobsRatiuDesbrun2013}
}%DIFAUXCMD
). Hence, we can define a right action of ${\rm iso}(z)$ }\DIFaddend on \DIFdelbegin \DIFdel{$Q$, so there exists a $g \in G_1$ }\DIFdelend \DIFaddbegin \DIFadd{$Q_1^{(k)}$ given by }\eqref{right_ac_general}\DIFadd{, namely
  }\begin{equation*}\DIFadd{
    \psi_\rho(q) = }{\DIFadd{\rm Jet}}\DIFadd{_z^{(k)}(\varphi \circ \rho),
  }\end{equation*}
\DIFadd{for any $\varphi$ }\DIFaddend such that \DIFdelbegin \DIFdel{$q' = g \cdot q$. Since $\rho_1^*$ preserves
  $J_2^{-1}(\mu)$, we find that $g \cdot (q,p) \in J_2^{-1}(\mu) \cap T_{q'}^* Q$. Since
  $J_2^{-1}(\mu)$ is a section, this implies that $g \cdot (q,p) = (q',p')$ must hold.
}%DIFDELCMD < \end{proof}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{${\rm Jet}_z^{(k)}(\varphi) = q$.
}\item \DIFadd{The cotangent lift of the left and right actions on $Q$ lead to the momentum maps $J_L^{(k)}$ and $J_R^{(k)}$, explicitly computed in Section  \ref{sec:higher_order} and shown to be a weak dual pair in Proposition \ref{prop:k dual pairs}.
}\item  \DIFadd{In Theorem \ref{thm:k-solutions} we showed that the canonical Hamiltonian equations on $T^*{Q_1}^{(k)}$ associated with the collective Hamiltonian $H^{(k)} = h_{p, \sigma} \circ J_L^{(k)}$ lead to trajectories that are mapped, by $J_L^{(k)}$, to solutions of Hamilton's equations on $\mathfrak{X}_{\rm div}(\R^n)^*$. Moreover, we showed that $J_R^{(k)}$ is a constant of motion.
}\item \DIFadd{We briefly visit further symplectic reduction in Appendix \ref{sec:eom}. For more on this topic we refer to \mbox{%DIFAUXCMD
\cite{JacobsRatiuDesbrun2013, CotterHolmJacobsMeier2014}
}%DIFAUXCMD
.
}\item \DIFadd{In Section \ref{sec:Kelvin} we constructed the mapping $i:T^*Q_1^{(k)} \to T^*\SDiff(\R^n)/{\rm iso}^{(k)}_z$ given by }\eqref{i_explicit}\DIFadd{, namely,
}\begin{equation*}\DIFadd{
 	i(q, p) = }[\DIFadd{TR^*_{\varphi^{-1}} J_L^{(k)}(q, p)}]\DIFadd{.
 }\end{equation*}
\DIFadd{As we briefly mentioned towards the end of that section, if we denote by $J_{\rm conv}^{(k)}: T^*\SDiff(\R^n) \to \mathfrak{iso}^{(k)}(z)^*$ the momentum map associated to the cotangent lift of the right action of $\iso^{(k)}(z)$ on $\SDiff(\R^n)$,
then one can show that $i\bigl(T^*Q^{(k)}_1\bigr) = (J_{\rm conv}^{(k)})^{-1}(0) / \iso^{(k)}(z)$, as suggested by \mbox{%DIFAUXCMD
\cite[Theorem~2.2.2]{HRS}
}%DIFAUXCMD
.
}\end{itemize}
...
\label{sec:multi}
A multiset is \DIFdelbegin \DIFdel{nothing but }\DIFdelend a set with some notion of multiplicity \cite{Blizard1989}.
