...
%DIF DEL jetlets_survey_article-oldtmp-24701.tex   Sun Feb  7 00:53:53 2016
%DIF ADD jetlets_survey_article.tex                Sat Feb  6 22:34:08 2016
...
%DIF 21a23
\newcommand{\dz}{\ensuremath{\textrm{d}z}} %DIF > 
%DIF -------
...
%DIF 48d51
%DIF < % \author{C.~J. Cotter, J. Eldering, D.~D. Holm, H.~O. Jacobs, D.~M. Meier}
%DIF -------
%\author{C.~J. Cotter, J. Eldering, D.~D. Holm, H.~O. Jacobs, D.~M. Meier}
\usepackage[foot]{amsaddr}
\author{C.~J. Cotter$^1$, J. Eldering$^1$, D.~D. Holm$^1$, H.~O. Jacobs$^{1,*}$,}
\address{$^1$ Department of Mathematics, Imperial College London, London SW7 2AZ}
\author{D.~M. Meier$^2$}
...
One of these Poisson maps was the embedding map from a finite-dimensional manifold into the infinite-dimensional space dual to the divergence free \DIFdelbegin \DIFdel{vector-fields}\DIFdelend \DIFaddbegin \DIFadd{vector fields}\DIFaddend .  In other words, the space of point vortices is merely an invariant manifold of ideal fluid motion.\footnote{This is not quite correct, as one must ignore the infinite self-energy terms of point vortices and then extend the space of admissible solutions to non-smooth velocity fields.  However, modulo this physically motivated caveat the statement holds.}
...
boundaries of other levels (a bit like in a \DIFdelbegin \DIFdel{CW complex}\DIFdelend \DIFaddbegin \DIFadd{CW-complex}\DIFaddend ).
...
The Appendix is relatively extensive. A quick discussion of symplectic and Poisson manifolds is followed by the definition of dual pairs. Then we prove Theorem~\ref{thm:dual_pairs} and Theorem~\ref{thm:commuting_actions}, which are the core results used in the paper. In Appendix~\ref{sec:multi} we describe the index conventions used in the main texts (e.g., when calculating momentum maps for the general jetlet solutions)\DIFdelbegin \DIFdel{. Appendix~}\DIFdelend \DIFaddbegin \DIFadd{, while Appendix~\ref{sec:measure_valued_momap} briefly discusses the more abstract point of how the dual space of vector fields can be viewed as tensor product of 1-forms and distributions.
Finally, appendix~}\DIFaddend \ref{sec:eom} provides information describing the equations of motion for 1-jets, but in reduced coordinates, which arise after the reduction eliminating the conserved quantities $J_R$.
...
\emph{vortex methods} \cite{Chorin1973}, do not exhibit structures which express the nested character of the diffeomorphism group.
 ...
 In the context of \DIFdelbegin \DIFdel{image-registration }\DIFdelend \DIFaddbegin \DIFadd{image registration }\DIFaddend algorithms, the need to obtain
...
denote the space of divergence free \DIFdelbegin \DIFdel{vector-fields }\DIFdelend \DIFaddbegin \DIFadd{vector fields }\DIFaddend resulting from the Hodge decomposition.
...
here for convenience.
  \begin{quote}
    ...
    Let \DIFdelbegin \DIFdel{$h \in C^2(P_1)$}\DIFdelend \DIFaddbegin \DIFadd{$h \in C^1(P_1)$}\DIFaddend .
...
\item $\mathfrak{g}$ is the space of \DIFdelbegin \DIFdel{vector-fields}\DIFdelend \DIFaddbegin \DIFadd{vector fields}\DIFaddend ,
...
matrix\footnote{%
  ...
  $P \in T^*_R SO(3)$ and $v \in T_R SO(3)$. More generally, the pairing $\langle \cdot , \cdot \rangle$ will always refer to the natural pairing between a \DIFdelbegin \DIFdel{vector-space }\DIFdelend \DIFaddbegin \DIFadd{vector space }\DIFaddend and its dual in this paper.}.
...
The unreduced Hamiltonian is $H(R,P) = h(J_R(R,P))$.
  ...
  Recall that $T^*\SO(3)$ is a symplectic manifold. \DIFdelbegin \DIFdel{By }\DIFdelend \DIFaddbegin \DIFadd{It is now easy to
  check that the conditions of Lemma~\ref{lem:transitive-action} and
  }\DIFaddend Theorem~\ref{thm:commuting_actions} \DIFaddbegin \DIFadd{hold, from which
  }\DIFaddend we can observe that $J_L$ and $J_R$ have symplectically
  ...
  with respect to the \DIFdelbegin \DIFdel{Nambu-bracket}\DIFdelend \DIFaddbegin \DIFadd{Nambu bracket}\DIFaddend .
...
\label{sec:reg_fluids}
  ...
  Consider the \DIFdelbegin \DIFdel{Lie-algebra }\DIFdelend \DIFaddbegin \DIFadd{Lie algebra }\DIFaddend of vector fields on $\mathbb{R}^n$,
...
%DIF <   a Frechet vector-space, and
%DIF >   a Frechet vector space, and
...
%DIF < introduction of higher-order particles in Section~\ref{sec:higher_order}.
%DIF > introduction of higher order particles in Section~\ref{sec:higher_order}.
...
and the \DIFdelbegin \DIFdel{$i^{\rm th}$ }\DIFdelend \DIFaddbegin \DIFadd{$i$-th }\DIFaddend coordinate of the \DIFdelbegin \DIFdel{$a^{\rm th}$ }\DIFdelend \DIFaddbegin \DIFadd{$a$-th }\DIFaddend particle
...
Once one has found a solution $(q(t), p(t))$ to Hamilton's equations on $T^*\mathbb{R}^n$ and thus also a solution to Hamilton's equations on $\mathfrak{X}_{\rm div}(\mathbb{R}^n)^*$, one can proceed to integrate the corresponding time-dependent vector field to obtain the fluid motion $\varphi_t$ in $\SDiff(\mathbb{R}^n)$. It is natural (but not mandatory) to choose the initial map $\varphi_0$ to be the identity or at least to be a diffeomorphism that satisfies $\varphi_0(q_a(0))= q_a(0)$ for all $a$.
 ...
 \begin{alignat*}{2}
  ...
  This means in particular that the specification of $(q_1, \ldots, q_N)$ fixes the \DIFdelbegin \DIFdel{$0^{\rm th}$ }\DIFdelend \DIFaddbegin \DIFadd{zeroth }\DIFaddend order Taylor expansion (at the locations $z_a$) of the corresponding set of diffeomorphisms. With this in mind, let us define the isotropy group
\begin{align}
  \iso(z)  = \{ \psi \in \SDiff(\mathbb{R}^n) | \psi(z_a) = z_a \mbox{ for all } a\}
  \label{eq:isotropy_group}
\end{align}
...
\subsection{\DIFdelbegin \DIFdel{$1^{\rm st}$-order }\DIFdelend \DIFaddbegin \DIFadd{First order }\DIFaddend particle-like solutions}
\label{sec:first_order}
In this section we revisit the \DIFdelbegin \DIFdel{$1^{\rm st}$-order }\DIFdelend \DIFaddbegin \DIFadd{first order }\DIFaddend particle-like solutions of \cite{CotterHolmJacobsMeier2014} and discuss their dual pair, before extending the treatment to the higher levels of the hierarchy in the subsequent section. 
  ...
  If we let $q_a$ denote the position of the \DIFdelbegin \DIFdel{$a^{\rm th}$ }\DIFdelend \DIFaddbegin \DIFadd{$a$-th }\DIFaddend particle,
  $q\indices{_a^i}$ denote the \DIFdelbegin \DIFdel{$i^{\rm th}$ }\DIFdelend \DIFaddbegin \DIFadd{$i$-th }\DIFaddend component of this position,
  and let $q\indices{_a^i_j}$ denote the $(i,j)$ entry of the \DIFdelbegin \DIFdel{$a^{\rm th}$
  }\DIFdelend \DIFaddbegin \DIFadd{$a$-th
  }\DIFaddend matrix then the resulting momentum map is defined by the condition
  \begin{align*}
  ...
  \begin{align*}
    J_L^{(1)}(q, p)
    = p\indices{_a_i} \dx^i \otimes \delta\DIFdelbegin \DIFdel{_{q_a}
     }\DIFdelend \DIFaddbegin \DIFadd{_{q_a^{(0)}}
     }\DIFaddend -p\indices{_a_i^j} q\indices{_a^k_j} \dx^i \otimes \partial_k \delta\DIFdelbegin \DIFdel{_{q_a}
  }\DIFdelend \DIFaddbegin \DIFadd{_{q_a^{(0)}}
  }\DIFaddend \end{align*}

  Next, we construct a dual momentum map associated with a right action on $Q_N^{(1)}$. Analogous to the previous section  it is useful at this stage to write elements of $Q_N^{(1)}$ in the form $ \varphi \cdot z = (\varphi (z_1, {\mathbf{1}}), \ldots, \varphi \cdot (z_N , \mathbf{1}))$, $\varphi \in \SDiff(\mathbb{R}^n)$, for some designated element $ z=( (z_1, \mathbf{1}), \ldots, (z_N, \mathbf{1}))$.
  Clearly, every element of $Q_N^{(1)}$ can be written in this form for some $\varphi \in \SDiff(\mathbb{R}^n)$. Indeed, with this convention the specification of an element in $Q_N^{(1)}$ fixes the \DIFdelbegin \DIFdel{$1^{\rm st}$ }\DIFdelend \DIFaddbegin \DIFadd{first }\DIFaddend order Taylor expansion of $\varphi$ at the locations $z_a$, $a = 1, \ldots, N$. With this in mind, let us recall the isotropy group $\iso(z)$ defined earlier in \eqref{eq:isotropy_group}, which leaves these locations invariant, and define a right action on  $Q_N^{(1)}$ given by 
\begin{align}
...
In Proposition~\ref{HL_action_prop} we will generalize this construction to define a right action for the higher levels in the hierarchy of particle-like solutions.
  ...
  \end{align*}
  In terms of the \DIFdelbegin \DIFdel{Dirac delta }\DIFdelend \DIFaddbegin \DIFadd{Dirac-delta }\DIFaddend functional we may write this as the measure-valued momentum map
  \begin{equation}\label{eq:JR1}
    J_R^{(1)}(q,p) = - p\indices{_a_i^j} q\indices{_a^i_k} \DIFdelbegin \DIFdel{\dx}\DIFdelend \DIFaddbegin \DIFadd{\dz}\DIFaddend ^k \otimes \partial_j \delta_{z_a}
  \end{equation}
  \DIFaddend \begin{prop}\DIFaddbegin \label{prop:J1-dual-pair}
    \DIFaddend The momentum maps $J_L^{(1)}$ and $J_R^{(1)}$ form a dual pair.
  \end{prop}
  \begin{proof}
  \DIFdelbegin \DIFdel{The actions of $\iso(z)$ and }\DIFdelend \DIFaddbegin \DIFadd{We first check the conditions of Lemma~\ref{lem:transitive-action}.
  Note that }\DIFaddend $\SDiff(\R^n)$ \DIFdelbegin \DIFdel{on $Q_N^{(1)}$
    clearly commute
    and lift to symplectic actions on $T^*Q_N^{(1)}$.Moreover, the
  action }\DIFdelend \DIFaddbegin \DIFadd{acts transitively on $Q^{(1)}_N$ since it
  acts transitively on representatives $\psi \in \SDiff(\R^n)$ of
  elements $q$ (i.e.}\ \DIFadd{$q^{(0)}_a = \psi(z_a)$ and
  $q^{(1)}_a = D\psi|_{z_a}$, or, $q = \Jet^1_z(\psi)$). Secondly, the
  cotangent-lifted action preserves level sets of $J_R^{(k)}$.
  From~}\eqref{eq:SDiff_cotangent_action_Q1} \DIFadd{we see that $\varphi \in
  \SDiff(\R^n)$ acts on the first level }\DIFaddend of \DIFaddbegin \DIFadd{$(q,p) \in T^* Q^{(1)}_N$ as
  }\begin{align*}\DIFadd{
    (\varphi\cdot q)\indices{_a^i_j} }&\DIFadd{= (D\varphi|_{q^{(0)}_a})\indices{^i_k}\,q\indices{_a^k_j}, }\\
    \DIFadd{(\varphi\cdot p)\indices{_a_i^j} }&\DIFadd{= p\indices{_a_k^j}(D\varphi^{-1}|_{\varphi\cdot q^{(0)}_a})\indices{^k_i}.
  }\end{align*}
  \DIFadd{The explicit calculation
  }\begin{align*}\DIFadd{
    J_R^{(1)}(\varphi\cdot(q,p))
    }&\DIFadd{= - p\indices{_a_l^j} (D\varphi^{-1}|_{\varphi\cdot q^{(0)}_a})\indices{^l_i} \,
         (D\varphi|_{q^{(0)}_a})\indices{^i_m}\, q\indices{_a^m_k}
         \dz^k \otimes \partial_j \delta_{z_a} }\\
    &\DIFadd{= - p\indices{_a_l^j} \, \delta^l_m \, q\indices{_a^m_k}
         \dz^k \otimes \partial_j \delta_{z_a} = J_R^{(1)}(q,p)
  }\end{align*}
  \DIFadd{shows that~}\eqref{eq:JR1} \DIFadd{is preserved under the cotangent-lifted
  action of }\DIFaddend $\SDiff(\R^n)$\DIFdelbegin \DIFdel{is transitive on the level sets of $J_R^{(1)}$,
  since it is transitive overall.
By }\DIFdelend \DIFaddbegin \DIFadd{. Finally, we must show that
  $(J_R^{(1)})^{-1}(m)$ is a
  section of $T^* Q^{(1)}_N$ for any $m$ in the range of $J_{R}^{(1)}$. Let
  ...
  \DIFadd{Thus Lemma~\ref{lem:transitive-action} implies that $\SDiff(\R^n)$
  acts transitively on the level sets of the momentum map $J_R^{(1)}$.
  The actions of $\iso(z)$ and $\SDiff(\R^n)$ clearly commute so
  }\DIFaddend Theorem~\ref{thm:commuting_actions} \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{implies that }\DIFaddend $J_L^{(1)}$ and
  ...
  \begin{figure}
  	\centering
        ...
        \caption{(\emph{top}) Quiver plots of 2D vector fields induced by \DIFdelbeginFL \DIFdelFL{$1^{\rm st}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{first }\DIFaddendFL order jetlets. 
        (\emph{bottom})   \DIFdelbeginFL \DIFdelFL{Stream-line }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Streamline }\DIFaddendFL plots of 3D vector fields induced by \DIFdelbeginFL \DIFdelFL{$1^{\rm st}$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{first }\DIFaddendFL order jetlets.}
        \label{fig:zoo}
  \end{figure}

  \subsection{\DIFdelbegin \DIFdel{Higher-order }\DIFdelend \DIFaddbegin \DIFadd{Higher order }\DIFaddend particles}
  \label{sec:higher_order}
  In this section we will introduce a hierarchy of particle solutions whose $k$-th level includes the solutions at the \DIFdelbegin \DIFdel{$(k-1)^{\rm th}$ }\DIFdelend \DIFaddbegin \DIFadd{$(k-1)$-th }\DIFaddend level.
  The \DIFdelbegin \DIFdel{$0^{\rm th}$-level }\DIFdelend \DIFaddbegin \DIFadd{zeroth level }\DIFaddend in the hierarchy are the standard particle-like solutions,
  while the \DIFdelbegin \DIFdel{$1^{\rm st}$ }\DIFdelend \DIFaddbegin \DIFadd{first }\DIFaddend level describes the particles with internal $\SL(n)$ variables
  ...
  The \DIFdelbegin \DIFdel{zeroth-order }\DIFdelend \DIFaddbegin \DIFadd{zeroth order }\DIFaddend Taylor expansion of $\varphi$ about $0$
  is $\varphi(0)$, and the collection of such Taylor
  expansions is all of $\R^n$.
  The \DIFdelbegin \DIFdel{$1^{\rm st}$-order }\DIFdelend \DIFaddbegin \DIFadd{first order }\DIFaddend Taylor expansion of $\varphi$ about $0$ is
  \begin{align*}
    \varphi^i( x) = \varphi^i(0) + \partial_j \varphi^i(0) x^j + o( \|x\|).
  \end{align*}
  The tuple of coefficients $(\varphi(0) , D\varphi(0) ) \in \R^n \times \SL(n)$ is
  called the \DIFdelbegin \DIFdel{$1^{\rm st}$-order }\DIFdelend \DIFaddbegin \DIFadd{first order }\DIFaddend jet of $\varphi$ evaluated at $0$.
  Going further, the \DIFdelbegin \DIFdel{second-order }\DIFdelend \DIFaddbegin \DIFadd{second order }\DIFaddend Taylor expansion of $\varphi$ about $0$
  is
  \begin{align*}
  ...
  and we see that the space of \DIFdelbegin \DIFdel{second-order }\DIFdelend \DIFaddbegin \DIFadd{second order }\DIFaddend jets is a \DIFdelbegin \DIFdel{sub-manifold }\DIFdelend \DIFaddbegin \DIFadd{submanifold }\DIFaddend $Q^{(2)}_1 \subset \R^n \times \SL(n) \times S^1_2$.
  Finally, for $k \geq 2$ the space of $k$-th order jets is a \DIFdelbegin \DIFdel{sub-manifold
  }\DIFdelend \DIFaddbegin \DIFadd{submanifold
  ...
  This coordinate is used to model the partial derivative of the \DIFdelbegin \DIFdel{$i^{\rm th}$ }\DIFdelend \DIFaddbegin \DIFadd{$i$-th }\DIFaddend coordinate
...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  \DIFaddbegin \DIFadd{Equivalently, we may define $J_L^{(k)}$
  as the unique map such that $\langle J_L^{(k)}(q,p) , u \rangle = \langle (q,p) , \Jet_z^{(k)}( u \circ \varphi) \rangle$ for any $\varphi \in \SDiff(\R^n)$ whose $k$-jet is given by $q$ for any $(q,p) \in T^*Q^{(k)}_N$.
  }

  \DIFaddend The right action of $\iso(z)$ yields the momentum map $J_R^{(k)}$ defined implicitly by the relation
  \begin{equation*}
  ...
  \DIFaddbegin \DIFadd{Equivalently, we may define $J_R^{(k)}$
  as the unique map such that $\langle J_R^{(k)}(q,p) , w \rangle = \langle (q,p) , \Jet_z^{(k)}( T\varphi \circ w) \rangle$ for any $\varphi \in \SDiff(\R^n)$ whose $k$-jet is given by $q$ for any $(q,p) \in T^*Q^{(k)}_N$.
  }

  
  \DIFaddend We can write \DIFdelbegin \DIFdel{these momentum maps }\DIFdelend \DIFaddbegin \DIFadd{$J_L^{(k)}$ and $J_R^{(k)}$ }\DIFaddend explicitly, using the \DIFdelbegin \DIFdel{Dirac delta }\DIFdelend \DIFaddbegin \DIFadd{Dirac-delta }\DIFaddend distribution, as
  \begin{multline*}\DIFaddbegin \label{eq:JLk}
    \DIFaddend J_L^{(k)}(q,p) =
    \sum_{a=1}^N \Bigg(
  ...
  \end{multline*}
  and
  \DIFaddbegin \begin{equation}\DIFadd{\label{eq:JRk}
  ...
  \DIFadd{To see that these are dual pairs, we simply generalize the proof of
  ...
  action preserves level sets of $J_R^{(k)}$. To see this, let
  $\varphi \in \SDiff(\R^n)$ and $(q,p) \in T^* Q^{(k)}_N$, then we
  have
  }\begin{align*}\DIFadd{
    \langle J_R^{(k)}\big(\varphi\cdot(q,p)\big), w \rangle
    }&\DIFadd{= \langle T^{*} \varphi \cdot (q,p) , ( \varphi^{-1} \cdot q ) \cdot w \rangle }\\
    &\DIFadd{= \langle T^{*} \varphi \cdot (q,p) ,  T\varphi^{-1} \cdot (q \cdot w) \rangle }\\
    &\DIFadd{= \langle (q,p) , T \varphi \cdot T\varphi^{-1} (q \cdot w) \rangle }\\
    &\DIFadd{= \langle (q,p) , q \cdot w \rangle = \langle J_{R}^{(k)}(q,p) , w \rangle,
  }\end{align*}
  \DIFadd{where we used on the second line that the actions commute.
  ...
  find that~}\eqref{eq:JRk} \DIFadd{reduces to
  }\DIFaddend \begin{equation*}
    J_R^{(k)}(\DIFdelbegin \DIFdel{q}\DIFdelend \DIFaddbegin \bar{q}\DIFaddend ,\DIFdelbegin \DIFdel{p}\DIFdelend \DIFaddbegin \DIFadd{\pi}\DIFaddend ) = \sum_{a=1}\DIFdelbegin \DIFdel{^{N}
    \sum_{\substack{ |\alpha| \leq k \\ 1 \leq \ell \leq |\alpha| }} }%DIFDELCMD < \;
%DIFDELCMD < 		%%%
\DIFdelend \DIFaddbegin \DIFadd{^N
    }\DIFaddend \sum\DIFdelbegin \DIFdel{_{
			\substack{
				j_1,\dots,j_\ell \in \{1,\dots,n\} \\
				[\gamma_1,\dots,\gamma_\ell] \in \Pi(\alpha,\ell)
				}
			}
			p}\DIFdelend \DIFaddbegin \DIFadd{_{|\alpha| \leq k}
			\pi}\DIFaddend \indices{_a_i^\alpha} \DIFdelbegin \DIFdel{q}%DIFDELCMD < \indices{_a^i_{[j_1 \cdots j_\ell]}}
%DIFDELCMD <       %%%
\DIFdel{\sum_{m=1}^\ell }\DIFdelend (-1)\DIFdelbegin \DIFdel{^{|\gamma_m|}}%DIFDELCMD < \left( %%%
\DIFdel{\prod_{n \neq m}  \delta_{\gamma_n}^{[b_n]} }%DIFDELCMD < \right) %%%
\DIFdelend \DIFaddbegin \DIFadd{^{|\alpha|} }\dz\DIFadd{^i }\DIFaddend \otimes \partial\DIFdelbegin \DIFdel{_{\gamma_m}}\DIFdelend \DIFaddbegin \DIFadd{_\alpha }\DIFaddend \delta_{z_a}\DIFaddbegin \DIFadd{.
  }\DIFaddend \end{equation*}
  \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{Since the points $z_a$ are distinct this mapping
  $J_R^{(k)}(\bar{q},\,\cdot\,)$ is clearly injective into
  $\mathfrak{X}_{\rm div}(\R^n)^*$. By invariance of $J_R^{(k)}$ under
  the }\DIFaddend action of $\SDiff(\R^n)$ \DIFdelbegin \DIFdel{is transitive on $Q_N^{(k)}$, and therefore transitive }\DIFdelend \DIFaddbegin \DIFadd{it follows that
  ...
  \DIFadd{Thus Lemma~\ref{lem:transitive-action} implies that $\SDiff(\R^n)$
  acts transitively }\DIFaddend on the level sets of the momentum map \DIFdelbegin \DIFdel{$J_R^{(1)}$.
  By }\DIFdelend \DIFaddbegin \DIFadd{$J_R^{(k)}$
  and by }\DIFaddend Theorem~\ref{thm:commuting_actions}, $J_L^{(k)}$ and
  $J_R^{(k)}$ form a dual pair. This gives us the final result on
  jetlet parametrized solutions.
  \DIFdelbegin \DIFdel{The proof is identical to that given in Proposition~\ref{prop:0-solutions}.
  }\DIFdelend \begin{prop}\label{prop:k-solutions}
...
By \DIFdelbegin \DIFdel{the general principles discussed earlier, the }\DIFdelend \DIFaddbegin \DIFadd{applying Theorem \ref{thm:dual_pairs} with $J_1 = J_\text{conv}$ and $J_2 = J_\text{spatial}$ we know that Hamiltonian }\DIFaddend dynamics on $T^*\SDiff(\R^n)$ 
\DIFdelbegin \DIFdel{exhibits }\DIFdelend \DIFaddbegin \DIFadd{with respect to a Hamiltonian of the form $h \circ J_1$ will
exhibit }\DIFaddend the constant of motion
...
of vector fields $w_\varepsilon$ such that \DIFdelbegin \DIFdel{$w_\varepsilon(\gamma(s)) = \gamma'(s)$
}\DIFdelend \DIFaddbegin \DIFadd{$w_\varepsilon(\gamma_0(s)) = \gamma'_0(s)$
}\DIFaddend and such that the (weak in $L^2$) limit $w = \lim_{\varepsilon \to 0} w_\varepsilon$ is the generalized function
\begin{equation*}
...
representation of the fluid flow \DIFaddbegin \DIFadd{and $\gamma(s,t) := \varphi_t(\gamma_0(s))$}\DIFaddend . 
...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and the corresponding quotient map is given by $\Jet_z^k$.  
 ...
 for any \DIFdelbegin \DIFdel{$(p,q) \in T^*Q^{(k)}_N$ }\DIFdelend \DIFaddbegin \DIFadd{$(q,p) \in T^*Q^{(k)}_N$ }\DIFaddend and any $v_\varphi \in T_\varphi \SDiff(\mathbb{R}^n)$ with $\Jet_z^k(\varphi) = q$ we can
 consider the element \DIFdelbegin \DIFdel{$i(p,q) \in T^*\SDiff(\R^n)$ }\DIFdelend \DIFaddbegin \DIFadd{$i(q,p) \in T^*\SDiff(\R^n)$ }\DIFaddend given by $\langle i(q,p) , v_\varphi \rangle = \langle p , v_\varphi( \varphi^{-1} \cdot q) \rangle$
 \cite[(2.2.4)]{HRS}.
...
\DIFdelbegin \DIFdel{Namely, for the left-hand part }\DIFdelend \DIFaddbegin \DIFadd{For the left side }\DIFaddend of the diagram\DIFdelbegin \DIFdel{recall that we have shown in the previous section that a jetlet solution has a velocity vector in $T \SDiff(\R^n)$ of the form
  }\begin{displaymath}\DIFdel{
    v_{\varphi} = (K * J_L^{(k)}(p_q)) \circ \varphi
  }\end{displaymath}
  %DIFAUXCMD
\DIFdel{for some $p_q \in T^*_q Q^{(k)}_N$, where $q = \Jet^k_z(\varphi)$. From this it follows that the corresponding Legendre momentum $p_\varphi$ satisfies, for all $ w \in \mathfrak{X}_{\rm div}(\R^n)$}\DIFdelend , \DIFdelbegin \begin{displaymath}\DIFdel{
  \left< J_\text{spatial}(p_\varphi), w\right> = \left< p_\varphi, w \circ \varphi \right> = \left< J_L^{(k)}(p_q), w \right>,
  }\end{displaymath}
  %DIFAUXCMD
\DIFdel{which shows commutativity of the left-hand part of the diagram.
As for the right-hand part, we again use the special form of $p_\varphi$ to conclude that, for all $ w \in \mathfrak{iso}(z) \subset \mathfrak{X}_{\rm div}(\R^n)$,
 }\begin{displaymath}\DIFdel{
 ...
 %DIFAUXCMD
\DIFdel{On the other hand, if we write $\phi_\varphi: Q^{(k)}_N \to Q^{(k)}_N$ for the left action $\phi_\varphi (q) = \varphi \cdot q$, we have 
 }\begin{displaymath}\DIFdel{
   \left<J_R^{(k)}(p_q), w\right> = \left< p_q, T_{\phi_\varphi} (w_{Q^{(k)}_N} (q_0))   \right> =  \left<p_q, (T\varphi (w \circ \varphi^{-1}) )_{Q^{(k)}_N} (q) \right>,
 }\end{displaymath}
 %DIFAUXCMD
\DIFdel{where we denoted the infinitesimal left action of $w$ on $Q^{(k)}_N$ by $w_{Q^{(k)}_N}$. The velocity corresponding (via the inverse of the Legendre transform) to $p_q$ can be written as $ T\Jet_z^k( (K * J_L^{(k)}(p_q)) \circ \varphi)$,
 and therefore one concludes that 
 }\begin{displaymath}\DIFdel{
 	\left<J_R^{(k)}(p_q), w\right> = \left< J_L^{(k)}(p_q), T \varphi (w \circ \varphi^{-1}) \right>. \label{compare_2}
 }\end{displaymath}
 %DIFAUXCMD
\DIFdel{This shows the commutativity of the right-hand part of the diagram above (compare }%DIFDELCMD < \eqref{compare_1} %%%
\DIFdelend \DIFaddbegin \DIFadd{note that the statement $\pi(p_\varphi) = i(p_q)$ for some $p_\varphi \in T^* \SDiff(\mathbb{R}^n)$ }\DIFaddend and \DIFdelbegin %DIFDELCMD < \eqref{compare_2}%%%
\DIFdel{)}\DIFdelend \DIFaddbegin \DIFadd{$p_q \in T^*Q_N^{(k)}$ is equivalent to the statement that $\langle p_\varphi , w \circ \varphi \rangle = \langle p_q , \Jet^{(k)}_z ( w \circ \varphi) \rangle$.
From the definitions of $J_\text{spatial}$ and $J_L^{(k)}$ we find 
  ...
  \DIFadd{when $\pi(p_\varphi) = i(p_q)$.
  This verifies the left half of the diagram. The right hand side can be verified in the same manner}\DIFaddend .
...
%DIF >  This shows the commutativity of the right-hand part of the  diagram above (compare \eqref{compare_1} and \eqref{compare_2}).
  ...
  \DIFaddend Again, the right legs of both dual pairs yield the conserved quantities.
...
\DIFdelbegin \DIFdel{As a side remark we mention that }%DIFDELCMD < \eqref{compare_2} %%%
\DIFdel{offers an alternative route to derive one momentum map once one has found the other. In contrast,  in the previous section we computed both $J_L^{(k)}$ and $J_R^{(k)}$ using the coordinate expressions, presented in the proof of Proposition~\ref{HL_action_prop}, for the respective actions on $Q^{(k)}_N$.
}\DIFdelend %DIF > As a side remark we mention that \eqref{compare_2} offers an alternative route to derive one momentum map once one has found the other. In contrast,  in the previous section we computed both $J_L^{(k)}$ and $J_R^{(k)}$ using the coordinate expressions, presented in the proof of Proposition~\ref{HL_action_prop}, for the respective actions on $Q^{(k)}_N$.
...
\DIFaddbegin \begin{equation*}
  \dot{r} = \frac{\partial H}{\partial p_r} = \frac{1}{4}p_r\,\DIFadd{r}^2 + \mathcal{O}(r^4), \qquad
...
and momentum $p^{(0)} = 0$. \DIFaddbegin \DIFadd{From here on, we use the Frobenius inner
product to identify $\mathfrak{sl}(2)^* \cong \mathfrak{sl}(2)$, which
for explicit matrices corresponds to taking the transpose.
}\DIFaddend For such a setup we have equations of motion
\begin{equation*}
  \dot{\mu}\indices{_i^j}
...
\end{equation*}
or in short, \DIFdelbegin \DIFdel{$\dot{\mu} = [\mu,\xi]$ with
$\xi\indices{^i_j} = \partial_j u^i(0) = \partial_{jk} K^{i l}(0) \mu\indices{_l^k}$}\DIFdelend \DIFaddbegin \DIFadd{$\dot{\mu} = -\ad^*_\xi(\mu) = [\mu,\xi^T]$ with
$\xi\indices{^i_j} = \partial_j u^i(0) = -\partial_{jk} K^{i l}(0) \mu\indices{_l^k}$}\DIFaddend .
To ease calculations, let us choose the basis
\DIFdel{for $\mathfrak{sl}(2)^* \cong \mathfrak{sl}(2)$, where the
identification is made through the Frobenius inner product, }\DIFdelend
...
the kernel as a tensor product\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{. A calculation verified by symbolic
computer algebra software shows that
}\DIFaddend \begin{equation*}
  \partial_{ij} K^{kl}(0)
  = \DIFaddbegin \DIFadd{\frac{1}{4} \left(\begin{array}{rr}
     -\begin{pmatrix} 1 & 0 \\ 0 & 3 \end{pmatrix} &
      \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix} \\[2.5ex]
      \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix} &
     -\begin{pmatrix} 3 & 0 \\ 0 & 1 \end{pmatrix}
    \end{array}\right)}
  \DIFadd{=-}\DIFaddend \frac{1}{2} \omega_i^k \omega_j^l
   -\frac{1}{4} \big( \sigma_i^k \sigma_j^l + \tau_i^k \tau_j^l \big).
\end{equation*}
\DIFaddbegin \DIFadd{where the indices $i,j$ and $k,l$ label the outer and inner matrix
elements respectively. }\DIFaddend With these decompositions we find that $\xi$ as
a function of $\mu$ can be written as
\begin{equation*}
  \xi = \DIFdelbegin \DIFdel{-j}\DIFdelend \DIFaddbegin \DIFadd{j}\DIFaddend \,\omega + \frac{1}{2}(s\,\sigma + t\,\tau).
\end{equation*}
...
\begin{equation}\label{eq:mu-dynamics}
  \dot{\mu}
  = \DIFaddbegin [\DIFadd{\mu , \xi^T}]
  \DIFadd{= }\DIFaddend \big[j\,\omega+s\,\sigma+t\,\tau \,,\, -j\,\omega + \frac{1}{2}(s\,\sigma + t\,\tau)\big]
...
$y_1(t)$ of a $1$-jetlet, basically\footnote{%
  ...
  properties of the dynamics~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Ribeiro2014}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Palmer2012,Ribeiro2014}
}%DIFAUXCMD
}\DIFaddend .%
...
The initial state is given in center-of-mass polar-coordinates\DIFdelbegin \DIFdel{by
}\DIFdelend \DIFaddbegin \DIFadd{,
see~}\eqref{eq:CoM-coords} \DIFadd{and~}\eqref{eq:polar-coords}\DIFadd{, by
}\DIFaddend \begin{align*}
    r    &= 6, \qquad p_r    = -1.5, \\
    \phi &= 0, \qquad p_\phi = 6d\DIFdelbegin \DIFdel{,
}\DIFdelend \DIFaddbegin \DIFadd{.
}\DIFaddend \end{align*}
Furthermore, we use
...
\begin{enumerate}
	...
	\item Finding a way to implement boundary conditions.
        ...
        \item An analysis of convergence to Euler equations when $\sigma \to 0$ for the case where the power of the Helmholtz operator, $p$, goes to infinity.  The advantage of the $p=\infty$ case \DIFdelbegin \DIFdel{, }\DIFdelend is that the limiting kernel can be written in terms of elementary functions \cite{MicheliGlaunes2014}.
\end{enumerate}
...
given by
\begin{equation*}
  \{ F , G \}_{\rm can} = \pder{F}{q}\pder{G}{p} - \pder{G}{q} \DIFdelbegin \DIFdel{\pder{G}{p}}
\DIFdelend \DIFaddbegin \DIFadd{\pder{F}{p}}\DIFaddend .
\end{equation*}
...
\begin{defn}
  ...
  The group $G$ is said to \emph{act symplectically} \DIFaddbegin \DIFadd{if
  }\DIFaddend \begin{equation*}
...
and let $\mathfrak{g}^*$ denote \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend dual.
...
\begin{defn}
  ...
  \emph{Poisson map} if \DIFdelbegin \DIFdel{$\{ f \circ \psi , g \circ \psi \}_1 = \{ f , g \}_2$  }\DIFdelend \DIFaddbegin \DIFadd{$\{ f \circ \psi , g \circ \psi \}_1 = \{ f , g \}_2 \circ \psi$  }\DIFaddend for any $f,g \in C^{\infty}(P_2)$.
\end{defn}
...
\begin{prop}[Lemma 1.2 of \cite{Weinstein1983} or Proposition 10.3.2 of \cite{MandS}] \label{prop:Poisson_dynamics}
  ...
  Let \DIFdelbegin \DIFdel{$h_2 \in C^2(P_2)$}\DIFdelend \DIFaddbegin \DIFadd{$h_2 \in C^1(P_2)$}\DIFaddend .
  If $x(t) \in P_1$ is a solution to Hamilton's equations with respect
  to \DIFdelbegin \DIFdel{$h_1 = h_2 \circ \psi \in C^2(P_1)$}\DIFdelend \DIFaddbegin \DIFadd{$h_1 = h_2 \circ \psi \in C^1(P_1)$}\DIFaddend , then $y(t) = \psi(x(t)) \in P_2$ is a solution
  to Hamilton's equations with respect to $h_2$.
\end{prop}

  \DIFaddbegin \DIFadd{Remark that unlike in~\mbox{%DIFAUXCMD
\cite{MandS}
}%DIFAUXCMD
, we only require $C^1$~smoothness
  since we do not use existence and uniqueness of solutions.
}

  \DIFaddend When the dimension of $P_2$ is larger than that of $P_1$,
...
%  \emph{Clebsch variables} \cite{MarsdenWeinstein1983}.
  ...
  The next theorem \DIFaddbegin \DIFadd{and lemma }\DIFaddend will be used later for the purpose of
  ...
  \begin{thm}[Example 2.2 of \cite{GayBalmazVizman2012}] \label{thm:commuting_actions}
    ...
    Moreover, assume $G_1$ \DIFdelbegin \DIFdel{acts transitively }\DIFdelend \DIFaddbegin \DIFadd{is infinitesimally transitive }\DIFaddend on the level sets of $J_2$
    \DIFaddbegin \DIFadd{in the sense that $\kernel(J_2) \subset \mathfrak{g}_1 \cdot T^*Q$}\DIFaddend .
  ...
  \begin{proof}
    ...
    \end{equation*}
    where $i_{\rho_1(\xi_1)} \omega$ denotes contraction of a two-form with a \DIFdelbegin \DIFdel{vector-field}\DIFdelend \DIFaddbegin \DIFadd{vector field}\DIFaddend .
    Finally we calculate
    \begin{align*}
    	\omega( \rho_1(\xi_1) , \rho_2(\xi_2) ) &=  -\langle d \langle \theta, \rho_1(\xi_1) \rangle ,  \rho_2(\xi_2) \rangle \\
      &= -\lie_{\rho_2(\xi_2) }[ \langle \theta,\rho_1(\xi_1) \rangle] = - \langle \lie\DIFdelbegin \DIFdel{_{\rho_1(\xi_2)}[\theta] }\DIFdelend \DIFaddbegin \DIFadd{_{\rho_2(\xi_2)}[\theta] }\DIFaddend , \rho_1(\xi_1) \rangle - \langle \theta , [\rho_2(\xi_2), \rho_1(\xi_1) ] \rangle.
    \end{align*}
...
%    symplectically orthogonal.
    Now note that
    \DIFdelbegin \DIFdel{$\langle TJ_2 ( \rho_1(\xi_1, x)), \xi_2 \rangle = \omega( \rho_1(\xi_1, x) ,\rho_2(x, \xi_2 )) = 0$}\DIFdelend \DIFaddbegin \DIFadd{$\langle TJ_2 ( \rho_1(\xi_1, x)), \xi_2 \rangle = - \omega( \rho_1(\xi_1, x) ,\rho_2( \xi_2,x )) = 0$}\DIFaddend .
...
\DIFaddbegin \begin{lem}\label{lem:transitive-action}
  \DIFadd{Let $G_1, G_2$ act on the manifold $Q$. Let $\rho_1^*$ denote the
  ...
  \item \DIFadd{$G_1$ acts transitively on $Q$;
  }\item \DIFadd{the cotangent-lifted action $\rho_1^*$ preserves level sets of $J_2$;
  }\item \DIFadd{for any $\mu$ in the range of $J_2$, the level set $J_2^{-1}(\mu)$ is a section of $T^* Q$.
  }\end{enumerate}
  \DIFadd{Then the action $\rho_1^*$ is transitive on level sets of $J_2$.
}\end{lem}

\begin{proof}
  \DIFadd{Let $(q,p), (q',p') \in J_2^{-1}(\mu)$ where $(q,p) \neq (q',p')$.
...
\DIFaddend \section{Multi-indices}
\label{sec:multi}
...
\section{The dual space \DIFdelbegin \DIFdel{of $\mathfrak{X}_{\rm div}(\R^n)$}\DIFdelend \DIFaddbegin \DIFadd{to divergence free vector fields}\DIFaddend }
\label{sec:measure_valued_momap}
In this section we will provide a terse and incomplete
\DIFdelbegin \DIFdel{characterizations }\DIFdelend \DIFaddbegin \DIFadd{characterization }\DIFaddend of the dual space of divergence free
...
we can view \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend dual as a space of distributions.
...
If we restrict ourselves to the case of divergence free \DIFdelbegin \DIFdel{vector-fields}\DIFdelend \DIFaddbegin \DIFadd{vector fields}\DIFaddend , we need to quotient
the dual space appropriately.  In particular, we see that the annihilator of $\mathfrak{X}_{\rm div}(\R^n)$
as a subspace of \DIFdelbegin \DIFdel{$\mathfrak{X}(\R^n)$ is
}\DIFdelend \DIFaddbegin \DIFadd{$\mathfrak{X}(\R^n)^*$ is
}\DIFaddend \begin{align*}
	(\mathfrak{X}_{\rm div}(\R^n) )^\circ := \{ m \in \mathfrak{X}(\R^n)^* \mid \langle m , u \rangle = 0 , \forall u \in \mathfrak{X}_{\rm div}(\R^n) \}\\
		:= {\rm closure}\{ p \otimes \dx \in \mathfrak{X}(\R^n)\DIFaddbegin \DIFadd{^* }\DIFaddend \mid p \text{ is a closed one-form} \}.
\end{align*}
...
The dual space $\mathfrak{X}_{\rm div}(\R^n)^*$ is identical to the quotient space \DIFdelbegin \DIFdel{$\mathfrak{X}(\R^n) / (\mathfrak{X}_{\rm div}(\R^n))^\circ$}\DIFdelend \DIFaddbegin \DIFadd{$\mathfrak{X}(\R^n)^* / (\mathfrak{X}_{\rm div}(\R^n))^\circ$}\DIFaddend .
...
This is a harmless identification as long as we do not pair it with a non-divergence free \DIFdelbegin \DIFdel{vector-field}\DIFdelend \DIFaddbegin \DIFadd{vector field}\DIFaddend .
