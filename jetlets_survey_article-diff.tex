...
%DIF DEL jetlets_survey_article-oldtmp-6322.tex   Sat May  7 11:17:12 2016
%DIF ADD jetlets_survey_article.tex               Sat May  7 11:03:02 2016
...
Finally, \DIFdelbegin \DIFdel{appendix}\DIFdelend \DIFaddbegin \DIFadd{Appendix}\DIFaddend ~\ref{sec:eom} provides information describing the equations of motion for 1-jets, but in reduced coordinates, which arise after the reduction \DIFdelbegin \DIFdel{eliminating }\DIFdelend \DIFaddbegin \DIFadd{that eliminates }\DIFaddend the conserved quantities\DIFaddbegin \DIFadd{, }\DIFaddend $J_R$.
...
This verifies the left half }\DIFdelend \DIFaddbegin \DIFadd{if $\varphi' = \varphi \circ \psi$ for $\psi \in \iso^{(k)}(z)$, then for any $v_{\varphi'} \in T_{\varphi'} \SDiff(\R^n)$ we have
 ...
 \DIFadd{= \langle (q, p), (v_{\varphi'} \circ \psi^{-1} \circ \varphi^{-1}) \cdot q \rangle
 = \langle (q, p), (v_{\varphi'} \circ }{\DIFadd{\varphi'}}\DIFadd{^{-1}) \cdot q \rangle
...
%DIFDELCMD <   %%%
\DIFdel{Again , the }\DIFdelend \DIFaddbegin \DIFadd{For the right leg, note that 
}\begin{align*}
...
\DIFadd{The }\DIFaddend right legs of both \DIFaddbegin \DIFadd{weak }\DIFaddend dual pairs yield the conserved quantities.
...
\DIFdelbegin \DIFdel{The next theorem and lemma will be used later for the purpose of
  ...
  \begin{thm}[\DIFdel{Example 2.2 of \cite{GayBalmazVizman2012}}] \label{thm:commuting_actions}\DIFdel{
    ...
    Moreover, assume $G_1$ is infinitesimally transitive on the level sets of $J_2$
    in the sense that $\kernel(J_2) \subset \mathfrak{g}_1 \cdot T^*Q$.
  ...
  \begin{proof}\DIFdel{
    ...
    \DIFdel{Thus by Cartan's formula} \cite[Theorem 2.4.13(iv)]{FOM} \DIFdel{we can calculate}
    \begin{equation*}
      \DIFdel{0 = \lie_{\rho_1(\xi_1)} [ \theta] = d \langle \theta_1 , \rho_1(\xi_1) \rangle + i_{\rho_1(\xi_1)}(d\theta) = d \langle \theta_1 , \rho_1(\xi_1) \rangle - i_{\rho_1(\xi_1)} \omega, }
    \end{equation*}
    \DIFdel{where $i_{\rho_1(\xi_1)} \omega$ denotes contraction of a two-form with a vector field.
    Finally we calculate}
    \begin{align*}
    	\DIFdel{\omega( \rho_1(\xi_1) , \rho_2(\xi_2) )} &\DIFdel{=  -\langle d \langle \theta, \rho_1(\xi_1) \rangle ,  \rho_2(\xi_2) \rangle }\\
      &\DIFdel{= -\lie_{\rho_2(\xi_2) }[ \langle \theta,\rho_1(\xi_1) \rangle] = - \langle \lie_{\rho_2(\xi_2)}[\theta] , \rho_1(\xi_1) \rangle - \langle \theta , [\rho_2(\xi_2), \rho_1(\xi_1) ] \rangle.}
    \end{align*}
...
%    symplectically orthogonal.
    Now note that
    $\langle TJ_2 ( \rho_1(\xi_1, x)), \xi_2 \rangle = - \omega( \rho_1(\xi_1, x) ,\rho_2( \xi_2,x )) = 0$.
...
\begin{lem}\label{lem:transitive-action}
  \DIFdel{Let $G_1, G_2$ act on the manifold $Q$. Let $\rho_1^*$ denote the
  ...
  hold:}
  \begin{enumerate}
  \item \DIFdel{$G_1$ acts transitively on $Q$;}
  \item \DIFdel{the cotangent-lifted action $\rho_1^*$ preserves level sets of $J_2$;}
  \item \DIFdel{for any $\mu$ in the range of $J_2$, the level set $J_2^{-1}(\mu)$ is a section of $T^* Q$.}
  \end{enumerate}
  \DIFdel{Then the action $\rho_1^*$ is transitive on level sets of $J_2$.}
\end{lem}

\begin{proof}
  \DIFdel{Let $(q,p), (q',p') \in J_2^{-1}(\mu)$ where $(q,p) \neq (q',p')$.
...
\DIFaddbegin
\section{\DIFadd{Diagrammatic overview}} \label{app:diagramatic}
...
\begin{equation*}
  \DIFadd{G_{q_0} := \{ g \in G | \phi_g(q_0) = q_0\}.}
\end{equation*}
...
\begin{equation}
  \DIFadd{\psi_k(q) := \Pi( g k) = \phi_{gk}(q_0),} \label{right_ac_general}
\end{equation}
\DIFadd{and check that $\phi$ and $\psi$ indeed commute:}
\begin{equation*}
  \DIFadd{\phi_s(\psi_k (q)) = \phi_s(\phi_{gk}(q_0)) = \phi_{sgk}(q_0) = \psi_k(\phi_s (q)).}
\end{equation*}
\DIFadd{We refer to Figure \ref{fig_diagram_1} for a representation of the relevant spaces and maps.}
\item \DIFadd{The actions $\phi$ and $\psi$ on $Q$ can be lifted to actions $\Phi$ and $\Psi$ on the cotangent bundle $T^*Q$ in the usual manner (see Figure \ref{fig_diagram_2}). These cotangent lifted actions induce equivariant momentum maps $J_1: T^*Q \to \mathfrak{g}^*$ and $J_2:T^*Q \to \mathfrak{k}^*$, where $\mathfrak{g}^*$ and $\mathfrak{k}^*$ are the duals of the Lie algebras of $G$ and $K$. Due to their equivariance, $J_1$ and $J_2$ are Poisson maps (where the duals of the Lie algebras are equipped with appropriate Lie--Poisson brackets). Since the actions $\phi$ and $\psi$ commute, the action $\Psi$ leaves level sets of $J_1$ invariant, and vice versa. This implies that $J_1$ and $J_2$ are a weak dual pair, and if moreover $\Psi$ is transitive on the level sets of $J_1$ and vice versa, then $J_1$ and $J_2$ are a proper dual pair, see~\cite[Corollary~2.6]{GayBalmazVizman2012}.}
%
\item \DIFadd{Let $\mathcal{H}:T^*G \to \mathbb{R}$ be a right-invariant Hamiltonian. This means that $\mathcal{H}$ is invariant with respect to the  cotangent lift $TR^*$ of the multiplication from the right of $G$ by itself. In particular, the reduced Hamiltonian $h:\mathfrak{g}^* \to \mathbb{R}$ satisfies $\mathcal{H}(\alpha_g) = h \circ TR_g^*(\alpha_g)$ for any $\alpha_g \in T^*G$, and the reduced dynamics in $\mathfrak{g}^*$ are of Lie--Poisson type. The momentum map $J_1$ can be used to induce the so-called \emph{collective Hamiltonian} $H = h\circ J_1$ on $T^*Q$. Note that $T^*Q$ is a symplectic manifold, and that the symplectic (canonical) dynamics with respect to the collective Hamiltonian are mapped by (the Poisson map) $J_1$ to the reduced dynamics on $\mathfrak{g}^*$. Moreover, $J_2$ is conserved under the dynamics on $T^*Q$. The conservation law follows from Noether's theorem because $J_1$, and hence $H$, are left invariant by $\Psi$ (see Theorem \ref{thm:dual_pairs}).  Note that the elements of $T^*Q$ play the role of symplectic variables (or Clebsch variables in the sense of \cite{MarsdenWeinstein1983}).}
%
...
\item \DIFadd{We briefly visit further symplectic reduction in Appendix \ref{sec:eom}. For more on this topic we refer to} \cite{JacobsRatiuDesbrun2013, CotterHolmJacobsMeier2014}.
\item \DIFadd{In Section \ref{sec:Kelvin} we constructed the mapping $i:T^*Q_1^{(k)} \to T^*\SDiff(\R^n)/{\rm iso}^{(k)}_z$ given by \eqref{i_explicit}, namely,}
\begin{equation*}
 	\DIFadd{i(q, p) = [TR^*_{\varphi^{-1}} J_L^{(k)}(q, p)].}
 \end{equation*}
