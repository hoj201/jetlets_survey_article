...
\label{sec:first_order}
  ...
  matrix then the resulting momentum map is
  \begin{align*}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      \left(
        \sum_{
        \substack{
          \beta_1,\dots,\beta_k \in \{1,\dots,n\} \\
          [\gamma_1,\dots,\gamma_k] = \alpha
          }
          }
          \partial_{\beta_1 \cdots \beta_k} \varphi_1(\varphi_2(x)) 
          \partial_{\gamma_1} \varphi_2^{\beta_1} \cdots
          \partial_{\gamma_n} \varphi_2^{\beta_k}
        \right).
    \end{align*}
  ...
  \begin{align*}
    &J_L^{(k)}(q,p) = p \indices{_{ia}^{\alpha}} dx^a \otimes
    \sum_{k=1}^{|\alpha|} (-1)^{k}
    \left(
      \sum_{\substack{
          \beta_1,\dots,\beta_n \in \{1,\dots,n\} \\
          [\gamma_1,\dots,\gamma_n] = \alpha
          }
        }
      q\indices{^{i\beta_1}_{\gamma_1}} \cdots q\indices{^{i\beta_k}_{\gamma_k}}
      \partial_{\beta_1 \cdots \beta_k} \delta(\cdot - q^{(0)i}) \right)\\
    &J_R^{(k)}(q,p) = p\indices{_{ia}^\alpha} \sum_{k=1}^{|\alpha|}
    \Bigg( \sum_{\substack{
          \beta_1,\dots,\beta_k \in \{1,\dots,n\} \\
          [\gamma_1,\dots,\gamma_k] = \alpha
          }}
        \\
     &\quad \left[
          \sum_{j=1}^{k} (-1)^{|\gamma_j|} q \indices{^{ia}_{\beta_1 \cdots \beta_k}}
          \delta_{\gamma_1}^{\beta_1} \cdots \delta_{\gamma_{k-1}}^{\beta_{j-1}}
          \delta_{\gamma_{j+1}}^{\beta_{j+1}} \cdots \delta_{\gamma_n}^{\beta_k}
          dx^{\beta_k} \otimes \partial_{\gamma_k} \delta( \cdot - q^{(0)i})  \right]
     \Bigg)
  \end{align*}
