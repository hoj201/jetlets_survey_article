...
The Appendix is relatively extensive. A quick discussion of symplectic and Poisson manifolds is followed by the definition of dual pairs. Then we prove Theorem~\ref{thm:dual_pairs} and Theorem~\ref{thm:commuting_actions}, which are the core results used in the main paper. In Appendix~\ref{sec:multi} we describe the index conventions used in the main texts (e.g., when calculating momentum maps for the general jetlet solutions). Appendix~\ref{sec:eom} provides information describing the equations of motion for 2-jets, but in reduced coordinates, which arise after the reduction eliminating the conserved quantities $J_R$.
