...
curve:
\begin{equation}\label{eq:asymptotic-dynamics}
  \begin{split}
    J_L(q_1,p_1,q_2,p_2)
    ...
    &= \Big[R_\varphi \cdot \begin{pmatrix} \xi \\ p_\varphi \end{pmatrix}\Big]_i
       \Big[R_\varphi \cdot \begin{pmatrix} 1   \\ 0         \end{pmatrix}\Big]^j e^i\,\partial_j
       \delta_{\bar{q}} +\mathcal{O}(r)
  \end{split}
\end{equation}
with $\varphi(t) = \frac{3}{4} p_\varphi\,t$. Our aim is to show that
the factor in front of $e^i\,\partial_j \delta$, which we shall denote
...
\begin{equation*}
  \tilde{\mu}(t)
  = \Big[R_\varphi \cdot \begin{pmatrix} \xi \\ p_\varphi \end{pmatrix}\Big] \otimes
...
since it is actually a dual element, we can simply project out the
trace part to obtain $\tilde{\mu} \in \mathfrak{sl}(2)^*$. Also note
that the $\omega$ component of $\tilde{\mu}(0)$ is
$j = \frac{1}{2}p_\varphi$. Differentiating to time yields
\begin{equation*}
  \dot{\tilde{\mu}}
...
\je{fix sign error, add more general picture using $J_L$.}
