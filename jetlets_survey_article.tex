...
\hoj{I'm doing my best to make sure the audience understands what a ``toy'' model is.
This is because we have chosen not to pursue a convergence proof.  Do you think I succeeded in emphasizing this point?}
...
\begin{proof}
  ...
  \hoj{Well, $T^*\SDiff$ does not make any appearance here from what I can see.
  	As a side note, given any representation, $\rho$, of Lie-algebra on a vector-space, $V$, the action on $V^*$
	 is \emph{defined} by the equation $\langle m , \rho(\xi) \cdot v \rangle + \langle \rho(\xi) \cdot m , v \rangle = 0$
	 for all $v \in V$ and $m \in V^*$.  This is the special case where
   we consider the adjoint action of a Lie algebra on itself.}
  \je{Ok, I think I can make sense of this now when I think of the Lie
    derivative simply as the (co)adjoint action of the inf.dim.\ algebra
    on itself. I've replaced $df_v$ by $v$ (to be interpreted
...
In this section we revisit the $1^{\rm st}$-order particle-like solutions of \cite{CotterHolmJacobsMeier2014} and discuss its dual pair, before extending the treatment to the higher levels of the hierarchy in the subsequent section. 
  ...
  smoothness $C^{2p-n}$ by considering that the Fourier
...
The Hamiltonian $H^{(0)}$ for two $0$-jetlet particles can be reduced
to a one degree of freedom system when the total linear momentum
is zero. In this case we have
\begin{equation}\label{eq:H-two-0jetlets}
  H = \frac{1}{2} \sum_{a,b=1}^2 p_{a\,i} K^{ij}(q_a - q_b) p_{b\,j}
\end{equation}
on $T^* \R^4$ with the canonical Poisson brackets.
...
\begin{equation}\label{eq:EoM-lin-reduced}
  ...
  \end{aligned}
\end{equation}
When the total momentum is zero, i.e.~$\bar{p} = 0$, we can perform
...
\begin{equation*}
  ...
  H &= \frac{1}{4} \tilde{p}_i \big[ K^{ij}(0) - K^{ij}(2\tilde{q}) \big] \tilde{p}_j
     = \frac{1}{4} \begin{pmatrix} p_r \\ p_\phi \end{pmatrix}^T R_\phi^T
       \big[ K(0) - K(R_\phi\cdot(2r,0)) \big] R_\phi
       \begin{pmatrix} p_r \\ p_\phi \end{pmatrix}\\
    &= \frac{1}{4} \begin{pmatrix} p_r \\ p_\phi \end{pmatrix}^T
       \big[ K(0) - K(2r,0) \big] \begin{pmatrix} p_r \\ p_\phi \end{pmatrix},
  \end{aligned}
\end{equation*}
