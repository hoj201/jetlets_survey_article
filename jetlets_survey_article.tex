...
In the next section we will derive a weak dual pair wherein the symplectic
...
Once one has found a solution $(q(t), p(t))$ to Hamilton's equations on $T^*\mathbb{R}^n$ and thus also a solution to Hamilton's equations on $\mathfrak{X}_{\rm div}(\mathbb{R}^n)^*$, one can proceed to integrate the corresponding time-dependent vector field to obtain the fluid motion $\varphi_t$ in $\SDiff(\mathbb{R}^n)$. It is natural (but not mandatory) to choose the initial map $\varphi_0$ to be the identity or at least to be a diffeomorphism that satisfies $\varphi_0(q_a(0))= q_a(0)$ for all $a$.
 ...
 \begin{alignat*}{2}
  ...
  \end{alignat*}
  is a weak dual pair. 
  This weak dual pair allows us to express conservation
...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  \end{prop}
  \begin{proof}
  	Let us check that $J_L^{(k)}$ is invariant under the right action of $\iso(z)$.
	For notational clarity, let us denote this right action by $\rho : Q^{(k)} \times \iso(z) \to Q^{(k)}$.
	Then the tangent lift of this action can be denoted by $D\rho: TQ^{(k)} \times \iso(z) \to TQ^{(k)}$.
	The cotangent lifted action is a left action, $D\rho^* : \iso(z) \times T^*Q^{(k)} \to T^*Q^{(k)}$, defined implicitly by the constraint
	\begin{align}
		\langle D\rho^*( \psi , (q,p) ) , D\rho( (q,v) , \psi) \rangle = \langle (q,p) , (q,v) \rangle \label{eq:dual action}
	\end{align}
	for all $(q,p) \in T^*Q^{(k)}$ and $(q,v) \in TQ^{(k)}$.
	In particular, one can verify that $D\rho^*( \psi , (q,p) )$ is a co-vector over the point $\tilde{q} = \rho( q , \psi^{-1}) \in T^*Q^{(k)}$.
	By the definition of $J_L$ we observe
	\begin{align*}
		\langle J_L( D\rho^*( \psi , (q,p) ) ) , u \rangle = \langle  D\rho^*( \psi ,  (q,p) ) , \Jet^{(k)}_z ( u \circ \bar{\varphi} ) \rangle 
	\end{align*}
	where $\bar{\varphi} \in \SDiff(\R^n)$ is any diffeomorphism such that $\Jet_z^{(k)}( \bar{\varphi} ) = \rho( q , \psi^{-1} )$.
	If we let $\varphi$ be such that $\Jet^{(k)}_z( \varphi ) = q$ then we may choose $\bar{\varphi} = \varphi \circ \psi^{-1}$.
	Thus we get
	\begin{align*}
			\langle J_L( (q,p) \cdot \psi ) , u \rangle = \langle  D\rho^*( \psi ,  (q,p) ) , \Jet^{(k)}_z ( u \circ \varphi \circ \psi^{-1} ) \rangle 
	\end{align*}
	By \eqref{eq:dual action} we see
	\begin{align*}
		\langle J_L( (q,p) \cdot \psi ) , u \rangle &=  \langle  D\rho^*( \psi^{-1} , D\rho^*( \psi ,  (q,p) ) ) ,  D\rho( \Jet^{(k)}_z ( u \circ \varphi \circ \psi^{-1} ) , \psi^{-1} ) \rangle \\
			&= \langle (q,p) , \Jet^{(k)}( u \circ \varphi) \rangle \\
			&= \langle J_L(q,p) , u \rangle
	\end{align*}
	Thus, we see that $J_L$ is invariant under the right action of $\iso(z)$ on $Q^{(k)}$.
	By Corollary 2.6 of \cite{GayBalmazVizman2012} the proposition follows.
  \end{proof}
...
\end{prop}
  ...
  \begin{equation*}
    \kernel(J)^\omega \subset \kernel(\psi)^\omega \quad , \quad \kernel(\psi)^\omega \subset \kernel(J)^\omega
  \end{equation*}
  we call the diagram
  \begin{equation*}
    P_1 \stackrel{\psi}{\longleftarrow} S \stackrel{J}{\longrightarrow} P_2
  \end{equation*}
  a \emph{weak dual pair} (see \cite{GayBalmazVizman2012}).
...
%  \emph{Clebsch variables} \cite{MarsdenWeinstein1983}.
  ...
  identifying dual pairs.
	\todo[inline]{hoj:  I think we can remove this theorem, and the remainder of this subsection.}
