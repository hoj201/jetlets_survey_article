...
%  \je{Shouldn't $\delta_{q_a}$ be $\delta_{z_a}$ here? Or change the formula above? -- DM: I agree. I have changed it and removed the box.}
 ...
 For notational ease  we will in the following let $q\indices{_a_\emptyset}$ denote the position of the $a^{\rm th}$ particle. In the following proposition, recall the definition  of ${\rm iso}(z)$ given earlier in \eqref{isotropy_group}.
...
where we wrote $\Pi(\alpha, k)$ for the set of $k^{\rm th}$-order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{app:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
    ...
    We can choose distinct points $z^1,\dots,z^N \in \R^n$
    and easily apply the same process to $Q_N^{(k)}$.  In this case we observe the action to be
    \begin{align*}
      (q \cdot \psi)\indices{^{ai}_\alpha} = \sum_{k=1}^{|\alpha|}
      \left(
        \sum_{
        \substack{
          j_1,\dots,j_k \in \{1,\dots,n\} \\
          ...
          \partial_{\gamma_1} \psi^{j_1}(z^a) \cdots
          \partial_{\gamma_k} \psi^{j_k}(z^a)
        \right).
    \end{align*}
  \end{proof}
  ...
  \begin{equation*}
	...
	\sum_{a=1}^N \left( p_a \otimes \delta_{q^a_\emptyset} +
	 \sum_{|\alpha| < k } \sum_{\ell = 1}^{|\alpha|}
		\sum_{
			\substack{
				j_1,\dots,j_\ell \in \{1,\dots,n\} \\
				[\gamma_1,\dots,\gamma_l] \in \Pi(\alpha,\ell)
				}
			}
			p\indices{_a_m^\alpha} \partial_{j_1 \cdots j_\ell}u^m (q\indices{^a_\emptyset})
      q\indices{^{a j_1}_{\gamma_1}} \cdots
      q\indices{^{a j_\ell}_{\gamma_\ell}}
			\right)
  \end{equation*}
  ...
  \begin{equation*}
    ...
    \sum_{ | \alpha | \leq k } \sum_{\ell=1}^{|\alpha|}
		\sum_{
			\substack{
				j_1,\dots,j_\ell \in \{1,\dots,n\} \\
			...
			p\indices{_a_i^\alpha} q\indices{^{ai}_{[j_1 \cdots j_\ell]}}
...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and we denote the corresponding quotient map by $\Pi: \SDiff(\R^n) \to Q^{(k)}_N$. Note that in order to specify $\Pi$ one has to fix some point $q_0 \in Q^{(k)}_N$, with respect to which $\Pi$ can then be defined as $\Pi (\varphi) = \varphi \cdot q_0$. 
\hoj{If $\Pi$ is the quotient map for $\SDiff(\R^n)$ mod $\iso^{(k)}(z)$, then $\Pi = \Jet^{(k)}_z$, the map which sends $\SDiff(\R^n)$ to it's $k$-jet about $z$.  This is another way of stating the definition of a jet about $z$.  I see no flexibility on this matter since it is a definition.  If you don't believe me, test it on the identity diffeo for $k=0$.  You will find that $\Pi(id) = z$.  Under these conventions, with the introduction of the variable $q_0$, it must be the case that $q_0 = z$.  Since $q_0$ is redundant perhaps we should remove the use of $q_0$ in this section, and replace $\Pi$ with $\Jet_z^{(k)}$.}
\dm{I actually disagree with this, Henry. The point is that one still has to define a diffeomorphism from the quotient space to $Q^{(k)}_N$. That is, strictly speaking the equality $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$ is really meant in the sense that there exists a diffeomorphism between the spaces on the left hand side and the right hand side. Choosing a different anchor point $q_0$ simply changes that diffeomorphism a little, but it doesn't change anything beyond that. Aside of this detail, of course it is particularly natural to choose the anchor point given by $z$ and I'm happy to default to that. I'll make the changes soon.}
 ...
 \begin{tikzcd}
   ...
   & T^*\SDiff(\R^n) / \iso^{(k)}(z) & \\
   & T^*Q^{(k)}_N \uar{i} \ar[uur,"J_R"',bend right = 30] \ar[uul,"J_L",bend left = 30] &
 \end{tikzcd}
\end{equation}
  Namely, for the left-hand part of the diagram recall that we have shown in the previous section that a jetlet solution has a velocity vector in $T \SDiff(\R^n)$ of the form \begin{align} v_{\varphi} = (K * J_L(p_q)) \circ \varphi \notag \end{align} for some $p_q \in T^*_q Q^{(k)}_N$, where $q = \Pi(\varphi)$. From this it follows immediately that the corresponding Legendre momentum $p_\varphi$ satisfies, for all $ w \in \mathfrak{X}_{\rm div}(\R^n)$,
  \begin{align}
  \left< \lambda(p_\varphi), w\right> = \left< p_\varphi, w \circ \varphi \right> = \left< J_L(p_q), w \right>, \notag
  \end{align}
  which shows commutativity of the left-hand part of the diagram. As for the right-hand part, we again use the special form of $p_\varphi$ to conclude that, for all $ w \in \mathfrak{X}_{\rm div}(\R^n)$,
 \begin{align}
 	\left<\rho(p_\varphi), w\right> = \left<p_\varphi, T\varphi  (w) \right> = \left<J_L(p_q),  T\varphi (w \circ \varphi^{-1}) \right>. \label{compare_1}
 \end{align}
 On the other hand, if we write $\phi_\varphi: Q^{(k)}_N \to Q^{(k)}_N$ for the left action $\phi_\varphi (q) = \varphi \cdot q$, we have 
 \begin{align}
 	\left<J_R(p_q), w\right> = \left< p_q, T_{\phi_\varphi} (w_{Q^{(k)}_N} (q_0))   \right> =  \left<p_q, (T\varphi (w \circ \varphi^{-1}) )_{Q^{(k)}_N} (q) \right>, \notag \end{align}
 where we denoted the infinitesimal left action of $w$ on $Q^{(k)}_N$ by $w_{Q^{(k)}_N}$. The velocity corresponding (via the inverse of the Legendre transform) to $p_q$ can be written as $ T\Pi ( (K * J_L(p_q)) \circ \varphi)$,
 and therefore one concludes that 
 \begin{align}
 	\left<J_R(p_q), w\right> = \left< J_L(p_q), T \varphi (w \circ \varphi^{-1}) \right>. \label{compare_2}
 \end{align}
...
From the diagram it is immediately clear that the conservation law exhibited in
our particle models is a shadow of the conservation of \eqref{eq:circulation}.
The conservation of circulation of the full regularized model is given by $\rho$,
while that of the jetlet solutions is given by $J_R$.
In other words, conservation of $J_R$ in the jetlet solutions is a shadow
...
we see that our particle models contain a model of Kelvin's circulation theorem within them (cf. also \cite[Theorem 5.5]{JacobsRatiuDesbrun2013}).

 As a side remark we mention that \eqref{compare_2} offers an alternative route to derive one momentum map once one has found the other. In contrast,  in the previous section we computed both $J_L$ and $J_R$ using the coordinate expressions, presented in the proof of proposition \ref{HL_action_prop},  for the respective actions on $Q^{(k)}_N$. 
%
