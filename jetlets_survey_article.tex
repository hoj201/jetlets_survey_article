...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  which does act freely on $Q^{(k)}_N$.\todo{We might be able to drop this sentence.  See the todonote under~\ref{lem:transitive-action}} Since also the cotangent-lifted
...
\todo[inline]{I think we can drop the requirement that $G_{1}$ act freely.
It doesn't appear necessary that the element $g$ in the proof be unique.
This would help in the main text where $G_{1} = \SDiff$, which does not act freely on $T^* Q^{(k)}$}
