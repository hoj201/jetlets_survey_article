...
\begin{abstract}
  ...
  We prove that two merging particles at the $0$th level in the hierachy
...
It is notable that the zeroth level of the heirarchy is a classical particle-like solution which appears in many geodesic
...
The jet groups lie at the foundations of certain representation theories of diffeomorphism groups (see \cite[Appendix 2]{VershikGelfandGraev1975} or \cite{Kirillov1981})\todo{I can't seem to find \cite{Kirillov1981} anywhere.  I think it is the proper citation for this sentence, but it seems weird to cite a paper I've only seen described in other papers.}.
...
After the introduction, in Section 2, we outline our strategy: The goal is to show that jetlet models admit a dual pair at each level in the hierarchy. Once this has been done, Theorem \ref{thm:dual_pairs} (proven in the appendix) does the rest. Namely, the jetlets satisfy canonical Hamiltonian equations, and the right momentum map is conserved by the flow.
...
can be replaced by a merged state whose momenta don't blow up.
...
In Proposition \ref{HL_action_prop} we will generalize this construction to define a right action for the higher levels in the hierarchy of particle-like solutions.
  ...
  In fact, $Q_1^{(k)}$ will turn out to be a trivial principle bundle.
  The fibers, $\SL(n) \times S^1_2 \times \cdots \times S^1_k$, have the group
  structure of a jet group (see \cite[Chapter 4]{KMS99} for details on group multiplications in the context of $\Diff(\R^n)$ and
  \cite{JacobsRatiuDesbrun2013} for the $\SDiff(M)$ case in the context of fluids).
