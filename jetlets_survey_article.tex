...
\todo[inline]{Also refer to Appendix~\ref{sec:measure_valued_momap}.}
...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  To see that these are dual pairs, we first check the conditions of
  Lemma~\ref{lem:transitive-action}. Note that $\SDiff(\R^n)$ acts
  transitively on $Q^{(k)}$. Although $\SDiff(\R^n)$ does not act
  freely, its action on $Q^{(k)}$ factors through the jet groupoid,
  which does act freely on $Q^{(k)}$. Since also the cotangent-lifted
  action of $\SDiff(\R^n)$ is uniquely determined by its jet at the
  base point $q \in Q^{(k)}$, the proof of
  lemma~\ref{lem:transitive-action} goes through. Secondly, the
  cotangent-lifted action preserves level sets of $J_R^{(k)}$. Let
  $\varphi \in \SDiff(\R^n)$ and $(q,p) \in T^* Q^{(k)}$, then we have
  \begin{align*}
    \langle J_R^{(k)}\big(\varphi\cdot(q,p)\big), w \rangle
    &= \langle p \cdot T_{\varphi \cdot q} \Jet^k(\varphi^{-1}) ,
               T_z(\Jet^k(\varphi) \circ q) \cdot w \rangle \\
    &= \langle p , T_{\varphi \cdot q} \Jet^k(\varphi^{-1}) \cdot
               T_q \Jet^k(\varphi) \cdot T_z q \cdot w \rangle \\
    &= \langle p , T_q \big(\Jet^k(\varphi^{-1}) \circ \Jet^k(\varphi)\big) \cdot
                   T_z q \cdot w \rangle \\
    &= \langle p , T_q \big(\Jet^k(\textrm{id}) \circ \Jet^k(\varphi)\big) \cdot
                   T_z q \cdot w \rangle \\
    &= \langle p , T_z q \cdot w \rangle
     = \langle J_R^{(k)}(q,p), w \rangle.
  \end{align*}
  Finally, $(J_R^{(k)})^{-1}(m)$ is a section of $T^* Q^{(k)}$ when
  non-empty. Let $(q,p) \in \big(J_R^{(k)}\big)^{-1}(m)$ and let
  $\varphi \in \SDiff(\R^n)$ such that $q = \Jet^{k}_z(\varphi)$. Then
  $\varphi^{-1} \cdot (q,p) = (\Jet^k_z(\rm{id}),\pi)$ for some $\pi$.
  ...
  \end{equation*}
  Since the points $z_a$ are distinct and
  $\pi\indices{_a_i^j} \in \mathfrak{sl}(n)^*$, this mapping
  ...
  $\pi \in T^*_{\bar{q}} Q^{(k)}$ is the unique element such that
  $J_R^{(k)}(\bar{q},\pi) = m$, and since $\SDiff(\R^n)$ acts by
  fiber-preserving diffeomorphisms on $T^* Q^{(k)}$, it follows that
  $(J_R^{(k)})^{-1}(m)$ is a section of $T^* Q^{(k)}$.
...
\begin{lem}\label{lem:transitive-action}
  ...
  hold:
  \begin{enumerate}
  \item $G_1$ acts transitively and freely on $Q$;
...
\begin{proof}
  ...
  $q \neq q'$. Now $G_1$ acts transitively and freely on $Q$, so there
  exists a unique $g \in G_1$ such that $q' = g \cdot q$. Since
  $\rho_1^*$ preserves $J_2^{-1}(\mu)$, we find that
  $g \cdot (q,p) \in J_2^{-1}(\mu) \cap T_{q'}^* Q$, which implies
  that $g \cdot (q,p) = (q',p')$ must hold.
\end{proof}
