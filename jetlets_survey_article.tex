...
In this instance we have a dual pair $J_L , J_R : T^*\SDiff(\R^n) \to \mathfrak{X}_{\rm div}(\R^n)^*$ induced by
the left and right action of $\SDiff(\R^n)$ on itself.
The dynamics on $T^*\SDiff(\R^n)$ exhibits the constant of motion
\begin{align}
  \langle J_R(\varphi,p_\varphi) , w \rangle = \langle p_\varphi , T\varphi \cdot w \rangle \label{eq:circulation}
\end{align}
for all $w \in \mathfrak{X}_{\rm div}(\R^n)$.\footnote{The pairing $\langle \cdot , \cdot \rangle$ will always refer to the natural pairing between a vector-space and its dual in this paper.}
This instance of Noether's theorem applies to regularized models as well as to the non-regularized case of Euler's.
In the case of Euler's fluid equations, the above conservation law implies Kelvin's circulation theorem \cite[Ch 1, \S 9]{ArnoldKhesin1992}.
...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and the corresponding quotient map is given by $\Jet_z^k$.  
 The space $T^*Q^{(k)}$ naturally embeds into $T^*\SDiff(\R^n) / \iso^{(k)}(z)$.  The commutativity of the following diagram uniquely defines the jetlet solutions:
  \begin{equation}
 \begin{tikzcd}
   \mathfrak{X}_{\rm div}(\R^n)^* & \ar[l,"J_L"'] T^*\SDiff(\R^n) \ar[r,"J_R"] \ar[d,"\pi"] & \mathfrak{iso}(z)^* \\
   & T^*\SDiff(\R^n) / \iso^{(k)}(z) & \\
...
\end{equation}
  ...
  \begin{align}
  \left< J_L(p_\varphi), w\right> = \left< p_\varphi, w \circ \varphi \right> = \left< J_L^{(k)}(p_q), w \right>, \notag
  \end{align}
 ...
 \begin{align}
 	\left<J_R(p_\varphi), w\right> = \left<p_\varphi, T\varphi  (w) \right> = \left<J_L^{(k)}(p_q),  T\varphi (w \circ \varphi^{-1}) \right>. \label{compare_1}
 \end{align}
...
The conservation of circulation of the full regularized model is given by $J_R$,
while that of the jetlet solutions is given by $J_R^{(k)}$.
In other words, conservation of $J_R^{(k)}$ in the jetlet solutions is a shadow
of the conservation of $J_R$, which is equivalent to conservation of \eqref{eq:circulation}.
