...
\end{figure}
  ...
  Let $z \in Q_N^{(0)}$. \dm{ It isn't clear what the point $z$ is here. Maybe we should say that $z$ is the particle location in the reference configuration, or the particle location at time $t=0$?}
...
\dm{Here, instead, we could just say: Introduce the (trivial) momentum map $J_R^{(0)} := 0$ to formally construct a dual pair. Then we would not have to introduce the somewhat pointless group action above.}
  ...
  associated to the trivial symmetry.
\dm{If we change the above and just introduce the momentum map directly, we should probably remove the following sentence.}
  Specifically, $J_R^{(0)}$ expresses the trivial  
...
In this section we revisit the $1^{\rm st}$-order particle-like solutions of \cite{CotterHolmJacobsMeier2014} and discuss its dual pair, before extending the treatment to the higher levels of the hierarchy in the subsequent section. 
  ...
  \end{align*}
  Moreover, there is a (non-trivial) right group action by $\iso(z)$ \dm{Reminder, $\iso(z)$ needs to be clarified; something like $\iso(z)$ is the subgroup of $\SDiff$ that leaves invariant all particle locations in the reference configuration.} on $Q^{(1)}_N$ given by
  \begin{align*}
  ...
  \begin{align*}
    J_R^{(1)}(q,p) = - p\indices{_a_i^j} q\indices{_a^i_k} dx^k \otimes \partial_j \delta_{q_a}
  \end{align*}
  \je{Shouldn't $\delta_{q_a}$ be $\delta_{z_a}$ here? Or change the formula above?}
  \begin{prop}
  ...
  Proposition~\ref{prop:k-solutions}.
  ...
  In this section we will introduce a hierarchy of particle solutions.
\dm{Have commented out:...,  where the $k^{\rm th}$ level of the hierarchy includes the $(k-1)^{\rm th}$ level. -- I didn't understand that.}
  ...
  of a diffeomorphism at some point $z_a \in \R^n$, \dm{Reminder: $z_a$ has to be clarified} i.e.\ $\partial_\beta \varphi^i(z_a)$.
...
where we wrote $\Pi(\alpha, k)$ for the set of $k^{\rm th}$-order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{app:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  and $\delta_\alpha^\beta$ is a generalization of the Kronecker-delta symbol to multi-indices.
...
In this instance we have a dual pair $\lambda, \rho : T^*\SDiff(\R^n) \to \mathfrak{X}_{\rm div}(\R^n)$ induces by
...
Lastly, these conserved quantities are inherited by the finite dimensional jet-particle solutions.
To see this, it is useful to assemble the $k$-th order isotropy group,
...
\begin{equation}
 ...
 \end{tikzcd}
\end{equation}
Again, the right-let of both dual pairs yields the conserved quantities.
...
\begin{figure}[ht]
  ...
  \caption{A contour plot of the reduced
    Hamiltonian~\eqref{eq:H-rot-reduced} for $p_\phi = 0$.}
...
\begin{figure}[ht]
  ...
  \caption{A contour plot of the reduced
    Hamiltonian~\eqref{eq:H-rot-reduced} for $p_\phi = 0.2$. The red
