...
\end{align*}
\dm{Henry, can you check the indices above? I think it probably should be $i$ on the left hand side?}
...
In this section we revisit the $1^{\rm st}$-order particle-like solutions of \cite{CotterHolmJacobsMeier2014} and discuss its dual pair, before extending the treatment to the higher levels of the hierarchy in the subsequent section. 
  ...
  for different initial values of the traceless matrix $\mu\indices{_i^j} = p\indices{_i^\ell} q\indices{^j_\ell}$.
...
\dm{Have commented out:...,  where the $k^{\rm th}$ level of the hierarchy includes the $(k-1)^{\rm th}$ level. -- I didn't understand that.}
  ...
  like $q \indices{^{ia}_{\beta}}$ where $a \in \{1,\dots,N\}$, $i \in \{ 1 , \dots, n \}$
  and $\beta$ is a multi-index on $\R^n$.
  This coordinate is used to model the partial derivative of the $i^{\rm th}$ coordinate
  of a diffeomorphism at some point $z^a \in \R^n$, \dm{Reminder: $z_a$ has to be clarified} i.e.\ $\partial_\beta \varphi^a(z_a)$.
  % This is the primary motivation for this notational convention.
  We will let $q^{a}_\emptyset$ denote the position of the $a^{\rm th}$ particle.
  ...
  evaluated at the points $z^1,\dots,z^N$.  We can think of $z$'s as points in the reference configuration.  If you deform the reference configuration
  the Taylor series transforms accordingly.}

  \begin{prop}
    The group $\SDiff(\R^n)$ acts on $Q_N^{(k)}$ by a left Lie group
    action.  If $z^1 ,\dots,z^N \in \R^n$ are distinct points,
    then the isotropy group $\iso(z^1,\dots,z^N) \subset \SDiff(\R^n)$
    ...
    \begin{align*}
      ...
      \left(
        ...
        \substack{
          ...
          \partial_{i_1 \cdots i_k} \varphi_1(\varphi_2(x)) 
          \partial_{\gamma_1} \varphi_2^{b_1} \cdots
          \partial_{\gamma_k} \varphi_2^{b_k}
        \right),
    \end{align*}
where we wrote $\Pi(\alpha, k)$ for the set of $k^{\rm th}$-order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{app:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
    ...
    \begin{align*}
    (\varphi \cdot q)\indices{^{ai}_\alpha} = 
     \sum_{k=1}^{|\alpha|}
      \left(
        \sum_{
        \substack{
          ...
          \partial_{i_1 \cdots i_k} \varphi^i(q \indices{^{a}_\emptyset} ) 
          q \indices{^{i_1}_{\gamma_1}} \cdots
          q \indices{^{i_k}_{\gamma_k}}
        \right),
    \end{align*}
    except for the component where $|\alpha | = 0$ in which case we observe $(\varphi \cdot q)^{ai}_\emptyset = \varphi(q^a_\emptyset)$.
    ...
    and easily apply the same process to $Q_N^{(k)}$.  In this case we observe the action to be
      ...
      \left(
        ...
        \substack{
          ...
          q\indices{^{ai}_{[i_1,\dots,i_k]} } 
          \partial_{\gamma_1} \psi^{b_1}(z^a) \cdots
          \partial_{\gamma_k} \psi^{b_k}(z^a)
        \right).
    \end{align*}
  \end{proof}
  ...
  \begin{align*}
	...
	\sum_{a=1}^N \left( p_a \otimes \delta_{q^{a}_\emptyset} +
	 \sum_{|\alpha| < k } \sum_{j = 1}^{|\alpha|}
		\sum_{
			\substack{
				i_1,\dots,i_j \in \{1,\dots,n\} \\
				[\gamma_1,\dots,\gamma_j] \in \Pi(\alpha,j)
				}
			}
			p \indices{_{ai}^{\alpha}} \partial_{i_1 \cdots i_j}u^a ( q^{i}_\emptyset)  q\indices{^{i_1}_{\gamma_1}} \cdots q\indices{^{i_j}_{\gamma_j}}
			\right)
  \end{align*}
  ...
  &\quad \sum_{ | \alpha | \leq k } \sum_{j=1}^{|\alpha|}
		\sum_{
			\substack{
				i_1,\dots,i_j \in \{1,\dots,n\} \\
				[\gamma_1,\dots,\gamma_j] \in \Pi(\alpha,j)
			...
			p\indices{_{ai}^\alpha} q\indices{^{ai}_{[i_1 \cdots i_j]}}   \sum_{m=1}^j  \partial_{\gamma_m} w^{i_m}(z_i) \left( \prod_{\ell \neq m }  \delta \indices{_{\gamma_\ell}^{i_\ell}} \right)
  \end{align*}
  ...
  and equals $0$ otherwise. \dm{I dont' understand this. How can a multi-index by equal to an integer?}
...
\begin{equation}\label{eq:H-two-0jetlets}
  H = \frac{1}{2} \sum_{a,b=1}^2 p_{a\,i} K^{ij}(q_a - q_b) p_{b\,j}
\end{equation}
