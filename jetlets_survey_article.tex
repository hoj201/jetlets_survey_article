...
In the case of Euler's fluid equations, the above conservation law
implies Kelvin's circulation theorem, by the following heuristic
argument~\cite[Ch~1, Thm~5.5]{ArnoldKhesin1998}:
Consider a given closed curve $\gamma_0\colon S^1 \to \R^n$ and a family
of vector fields $w_\varepsilon$ limiting to
\begin{equation*}
  w(x) = \begin{cases}
    \gamma'(s) & \text{if $x = \gamma_0(s)$,}\\
    0          & \text{else.}
  \end{cases}
\end{equation*}
...
\begin{align*}
  ...
  &= \int_{\gamma_0(S^1)} \langle \dot{\varphi}(x) , \textrm{D}\varphi(x)\cdot w(x) \rangle \,\textrm{d} x \\
...
therefore~\eqref{eq:abstract-circulation} is also conserved in the
regularized models, although this does not lead to Kelvin's
circulation law~\eqref{eq:Kelvin-circulation} anymore because of the
