...
%\subsection{Infinite dimensional subtleties}
%\label{eq:Infinite dimension}
%Before getting to the body of the paper, we should mention issues which are not addressed in this article.
%Namely, existence and uniqueness of solutions to infinite dimensional Hamiltonian systems.
%The configuration manifold for fluids and fluid models is (a subgroup) of the volume-preserving diffeomorphisms, $\SDiff(M)$.
%Any characterization of a cotangent bundle, ``$T^{*}\SDiff(M)$'', would be as an infinite-dimensional symplectic manifold with a weak symplectic form \cite{MandS}.
%As a result, the weak form of Hamilton's equations will not (generically) define a vector field because the symplectic form induces an injection rather than a bijection between
%tangent and cotangent fibers.  In order to find a vector field, the differential of the Hamiltonian must be in the range of this injection.
%Furthermore, even if the differential of the Hamiltonian is in the right space, it is not a priori clear that the resulting vector field is smooth enough for solutions to exist.
%Such a requirement places further constraints on the space of admissible Hamiltonians.
%These are non-trivial issue which must be addressed separately.
%Throughout this article, it will be assumed that any Hamiltonians in question yield equations of motion which admit unique solutions locally in time.
...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and the corresponding quotient map is given by $\Jet_z^k$.  
 ...
 consider the element $i(q,p) \in T^*\SDiff(\R^n)$ given by $\langle i(q,p) , v_\varphi \rangle = \langle p , v_\varphi( \varphi^{-1} \cdot q) \rangle$
 \cite[(2.2.4)]{HRS}.
