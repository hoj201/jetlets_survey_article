  ...
  \begin{prop} \label{prop:1-solutions}
	...
	\begin{align*}
		H^{(1)}(q,p) &= \frac{1}{2} p\indices{_a_i} K^{ij}(q_a - q_b) p\indices{_b_j}\\
			...
			&\quad - p\indices{_a_i^n} q\indices{_a^l_n} (\partial_l \partial_k K^{ij})(q_a - q_b) q\indices{_b^k_m} p\indices{_b_j^m}.
  \end{align*}
...
\hoj{These experiments actually don't make sense for the conserved momentum $J_R$ because this momenta is not exchanged.}
\hoj{Perhaps we can just repeat the cascade of the previous article at order $0$ and $1$.  Additionally, we can refine the cascade argument.}
