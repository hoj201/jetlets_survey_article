...
\end{displaymath}
Here, $J_{\rm spatial}$ and $J_{\rm conv}$ are defined in the natural manner through their actions on representatives of the equivalence classes. For the left side of the diagram, we let $(p,q) \in T^*Q^{(k)}_N$ and $\varphi$ such that $\Jet_z^k(\varphi) = q$. Then we obtain from \eqref{i_explicit} that for any $w \in \mathfrak{X}_{\rm div}(\R^n)$
\begin{align}
