...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  \end{prop}
  \begin{proof}
  	By \cite[Corollary 2.6]{GayBalmazVizman2012} we need only show that $J_L$ and $J_R$ are equivariant, and that $J_L$ is invariant under the right action of $\iso(z)$.
	...
	So we need only illustrate that $J_L$ is (right) $\iso(z)$ invariant.
	...
	In particular, one can verify that $\rho^*( \psi , (q,p) )$ is a co-vector over the point $\tilde{q} = \rho( q , \psi^{-1})$.
	...
	\begin{align*}
			...
			&\stackrel{\text{by \eqref{eq:tangent action}}}{=}  \langle  \rho^*( \psi ,  (q,p) )  ,  \rho( \Jet^{(k)}(u \circ \varphi) , \psi^{-1} ) \rangle \\
			&\stackrel{\text{by \eqref{eq:dual action}}}{=}  \langle (q,p) , \Jet^{(k)}( u \circ \varphi) \rangle \\
