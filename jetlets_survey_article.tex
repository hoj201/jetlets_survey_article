...
\emph{vortex methods} \cite{Chorin1973}, do not exhibit structures which express the nested structure of the diffeomorphism group.
 ...
 However, the specifices of the regularized fluid model were not determined,
 and the analysis of \cite{JacobsRatiuDesbrun2013} was thus purely
 theoretical.
...
\pageref{thm:dual_pairs} in the Appendix), which we repeate
...
Before applying this fairly complex theorem to fluids,
we will consider a more familar example.
...
\begin{proof}
  ...
  \hoj{Given any representation, $\rho$, of Lie-algebra on a vector-space, $V$, the action on $V^*$
	 is \emph{defined} by the equation $\langle m , \rho(\xi) \cdot v \rangle + \langle \rho(\xi) \cdot m , v \rangle = 0$
	 for all $v \in V$ and $m \in V^*$.  This is the special case where we consider the adjoint action of a Lie algebra on itself.}
  However $df_v = v$ and 
...
more explicitly in terms of the Dirac-delta distribution as
\begin{align*}
...
\label{sec:first_order}
  ...
  \end{align*}
  If we let $q_i$ denote the $i$th particle in $q^{(0)}$, and let $q\indices{^{ib}_c}$ denote the $(b,c)$ entry of the $i$th matrix in $q^{(1)}$
  then the resulting momentum map is
  \begin{align*}
    J_L^{(1)}\big( (q^{(0)}, q^{(1)} ),(p^{(0)}, p^{(1)}) \big)
    = p_i \otimes \delta_{q_i} - p\indices{_{ia}^c} q\indices{^{ib}_c} dx^b \otimes \partial_b \delta_{q_i}
  \end{align*}
  ...
  \begin{align*}
    J_R^{(1)}( q,p) = - p\indices{_{ia}^c} q\indices{^{ia}_b}  dx^b \otimes \partial_c \delta_{q^i}
  \end{align*}
  \begin{prop}
  ...
  \begin{align*}
    ...
    \Bigg( \sum_{\substack{
          (\beta_1,\dots,\beta_n) \in \{1,\dots,d\}^n \\
          [\gamma_1,\dots,\gamma_n] = \alpha
          }}
        \\
     &\quad \left[
...
\hoj{These experiments actually don't make sense for the conserved momentum $J_R$ because this momenta is not exchanged among the particles in any way.}
