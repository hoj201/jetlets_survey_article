...
In this section we revisit the $1^{\rm st}$-order particle-like solutions of \cite{CotterHolmJacobsMeier2014} and discuss its dual pair, before extending the treatment to the higher levels of the hierarchy in the subsequent section. 
  ...
  \begin{align*}
    Q^{(1)}_N = \left\{  ( q^1 , \dots, q^N ) \mid
      \begin{array}{c}
        q^i = (q^{(0),i},q^{(1),i}) \in \R^n \times \L(n) \\
        q^{(0),i} \neq q^{(0),j} \text{ when } i \neq j
      \end{array} \right\}.
  \end{align*}
...
Our model's are obtained by using the deformed Hamiltonian
...
we see that our particle models contain an model of Kelvin's circulation theorem within them.
...
\begin{equation}\label{eq:exp1-initial}
  ...
  \end{alignedat}
\end{equation}
aimed at each other with offset parameter $d$. Furthermore, we use
...
spinning state and `fly off' in opposite directions. This confirms the
