...
\dm{Looking at this diagram it occurs to me that the derivation of the momentum map $J_R$ could have perhaps been derived more easily. (Although I haven't tried to do it explicitly). Namely, it should be easy to compute the element $p_\varphi \in T^*{\rm SDiff}(\mathbb{R}^n)$ for a jetlet solution since (assuming we know the $J_L$) we know reduced version $m = \lambda(p_\varphi)= TR_{\varphi}^* p_\varphi = J_L( (q, p, Q, P,...))$. Then one only has to apply $\rho$ to it, which should be considerably easier than calculating $J_R$ directly...?? Any opinions/comments?}
