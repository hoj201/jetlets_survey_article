...
\end{abstract}
\dm{Do we need affiliations? Mine is: Department of Mathematics, Brunel University London, Uxbridge UB8 3PH, UK} 
\dm{I'm not too bothered by it, but it might be nicer if we removed '$n \geq 2$  dimensions' from the title.}

\section{Introduction}
Arnold's geometric insight in \cite{Arnold1966}  has forever changed the way mathematicians look at ideal fluid dynamics.  According to \cite{Arnold1966}, ideal incompressible fluid motion on a volume manifold $M$ is equivalent to geodesic motion on the Lie group of volume preserving diffeomorphisms ${\rm SDiff}(M)$ (i.e., the smooth invertible volume preserving maps of $M$ into itself, with smooth inverses).
...
Thus the jets of diffeomorphisms possess a natural sense of the ``self-similarity'' which is present in the diffeomorphism group itself.
 Models such as this are crucial to our understanding of fluids,
  ...
  \dm{The preceding sentence doesn't fit properly. Remove it?}
...
\begin{enumerate}
  ...
  \item We compute a nested sequence of conserved quantities at 
    ...
    of circulation.
  \item We numerically compute some particle-like solutions at the zeroth and first levels in the hierarchy.  We will observe a form of cascade in which interactions of solutions at the $k$-th level tend asymptotically in time toward solutions at the $(k+1)^{\rm th}$ level.
\end{enumerate}
...
\dm{Let's leave that for now in the interest of time.}
...
In Proposition~\ref{HL_action_prop} we will generalize this construction to define a right action for the higher levels in the hierarchy of particle-like solutions.
  ...
  \begin{figure}
  	\centering
        ...
        \caption{(\emph{top}) Quiver plots of 2D vector fields induced by $1^{\rm st}$ order jetlets. 
        (\emph{bottom})   Stream-line plots of 3D vector fields induced by $1^{\rm st}$ order jetlets.}
        \label{fig:zoo}
  \end{figure}

  \subsection{Higher-order particles}
  \label{sec:higher_order}
  In this section we will introduce a hierarchy of particle solutions whose $k$-th level includes the solutions at the $(k-1)^{\rm th}$ level.
  ...
  Finally, for $k \geq 2$ the space of $k$-th order jets is a sub-manifold
  \begin{align*}
...
\dm{Jaap, here it might be useful to mention what $p_\varphi$ and $p_r$ are in this initial state.}
...
experiment persists. \dm{'cannot be studied' is perhaps a bit strong? How about 'are challenging to study analytically'?} We can find parameter values of $d$ close to the
...
preserved \dm{I'm a bit confused by this: $J_L$ is supposed to follow reduced dynamics, no?} with absolute errors less than $4 \cdot 10^{-4}$ over a time
...
\section{Conclusion}
  ...
  group, we promoted \dm{promoted sounds a bit business like. Maybe 'suggested'?} the use of jetlets as a finite-dimensional model
...
\dm{suggest to replace the preceding sentence by:}

\dm{The developments discussed in the present paper give rise to a number of promising directions for future research. These include:}
\begin{enumerate}
	\item An investigation of the relationship between the presentation here \dm{replace 'the presentation here' by 'jetlets' ?} and the theory of point vortices and the vortex blob method.
