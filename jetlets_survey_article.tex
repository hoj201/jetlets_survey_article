...
Lastly, as $\sigma \to 0$, $h_{p,\sigma} \to h_{\rm Euler}$ and one
...
Once one has found a solution $(q(t), p(t))$ to Hamilton's equations on $T^*\mathbb{R}^n$ and thus also a solution to Hamilton's equations on $\mathfrak{X}_{\rm div}(\mathbb{R}^n)$, one can proceed to integrate the corresponding time-dependent vector field to obtain the fluid motion $\varphi_t$ in $\rm{SDiff}(\mathbb{R}^n)$. It is natural (but not mandatory) to choose the initial map $\varphi_0$ to be the identity or at least to be a diffeomorphism that satisfies $\varphi_0(q_a(0))= q_a(0)$, all $a$. 
 ...
 \begin{alignat*}{2}
  ...
  In order to make contact with the later parts of the paper, it is useful to remark that $J_R^{(0)}$ can formally be understood as the cotangent lift momentum map associated with a certain (trivial) group action. To that end, we fix a designated point $z = (z_1, \ldots, z_N) \in Q_N^{(0)}$ and take an arbitrary element $ q= (q_1, \ldots, q_N) \in Q_N^{(0)}$ to represent the set of all $\varphi \in {\rm SDiff}(\mathbb{R}^n)$ that satisfy $\varphi \cdot q  = q$. That is,  $\varphi(z_a) = q_a$, all $a$.  This means in particular that the specification of $(q_1, \ldots, q_N)$ fixes the $0^{\rm th}$ order Taylor expansion (at the locations $z_a$) of the corresponding set of diffeomorphisms. With this in mind, let us define the isotropy group
   \begin{align}
{\rm iso}(z)  = \{ \psi \in {\rm SDiff}(\mathbb{R}^n) | \psi(z_a) = z_a, \mbox{ all } a\} \label{isotropy_group}
\end{align} 
and a (trivial) right action on $Q_N^{(0)}$ where $\psi \in {\rm iso}(z)$ maps an element $(\varphi(z_1), \ldots, \varphi(z_N))$ to $( \varphi \circ \psi (z_1), \ldots, \varphi \circ \psi(z_N))$. Similar constructions will be crucial in later sections, when constructing the right leg of a dual pair for the higher levels in the hierarchy of particle-like solutions. 
...
In this section we revisit the $1^{\rm st}$-order particle-like solutions of \cite{CotterHolmJacobsMeier2014} and discuss its dual pair, before extending the treatment to the higher levels of the hierarchy in the subsequent section. 
  ...
  \begin{align*}
    ...
    = p\indices{_a_i} dx^i \otimes \delta_{q_a}
     -p\indices{_a_i^k} q\indices{_a^j_k} dx^i \otimes \partial_j \delta_{q_a}
  \end{align*}
...
%  \je{Shouldn't $\delta_{q_a}$ be $\delta_{z_a}$ here? Or change the formula above? -- DM: I agree. I have changed it and removed the box.}
  ...
  \begin{proof}
    ...
    Moreover, the action of $\SDiff(\R^n)$ is clearly transitive on the level sets of $J_R^{(1)}$, since it is transitive overall.
  ...
  \end{align*}
  \je{This was an index mess. I think these are ok now, but I don't
    trust the signs and coefficients: I did a quick recalculation and
    think there should be a factor $+\frac{1}{2}$ in front of the last
    term, and the middle term should cancel out because of
    anti-symmetry of $\partial_k K^{ij}(q_a - q_b)$ under $a
    \leftrightarrow b$. Both the current term and my suggested change
    are different again from the code, where the energy seems fairly
    well preserved\ldots}
    \dm{If I compare it with our previous jetlet article I'm inclined to say that there should be a factor of 2 in front of the middle factor, but that the formula is correct otherwise...?}
    \hoj{I think I agree with David up to a factor of 2 of the entire expression.  I changed it to match the derivation in the notes (see the notes folder in this repo)}
  ...
  \begin{figure}
  	\centering
        ...
        \caption{A zoo of $1^{\rm st}$-order jetlets}
        \label{fig:zoo}
  \end{figure}
  ...
  We will let $q\indices{_a_\emptyset}$ denote the position of the $a^{\rm th}$ particle.
  \hoj{I've made a change in notation.  We are letting $q^a_\emptyset$ denote
  the position of the $a^{\rm th}$ particle.  The old notation was something like $q^{(0),a}$.
  This new notation is less clunky and more consistent with our multi-index convention
  in that the zero-jets have the empty set as a lower index.}
  \hoj{A note fore David:
  In terms of the paper with Tudor, we are using the fact
  that $Q_N^{(k)} = \SDiff(\R^n) / \iso^{(k)}(z)$
  Where $\iso^{(k)}(z)$ is the subgroup which matches the identity 
  diffeomorphism to $k$th order.
  We found that $Q_N^{(k)}$
  is the space of Taylor series of elements of $\SDiff(M)$
  evaluated at the points $z_1,\dots,z_N$.  We can think of $z$'s as points in the reference configuration.  If you deform the reference configuration
  the Taylor series transforms accordingly.}

\je{I've changed the index conventions in the text below and I've
  tried to check that I did it right, but I'm not 100\% sure.}
...
where we wrote $\Pi(\alpha, k)$ for the set of $k^{\rm th}$-order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{app:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
     ...
     \sum_{k=1}^{|\alpha|}
      \left(
        ...
        \substack{
          ...
          \partial_{j_1 \cdots j_k} \varphi^i(q_\emptyset)
          q^{j_1}_{\gamma_1} \cdots
          q^{j_k}_{\gamma_k}
        \right),
    \end{align*}
    except for the component where $|\alpha | = 0$ in which case we observe $(\varphi \cdot q)^i_\emptyset = \varphi^i(q_\emptyset)$.
    ...
    and easily apply the same process to $Q_N^{(k)}$.  In this case we observe the action to be
    \begin{align*}
      (q \cdot \psi)\indices{_a^i_\alpha} = \sum_{k=1}^{|\alpha|}
      \left(
        \sum_{
        \substack{
          j_1,\dots,j_k \in \{1,\dots,n\} \\
          ...
          q\indices{_a^i_{[j_1,\dots,j_k]}}
          \partial_{\gamma_1} \psi^{j_1}(z_a) \cdots
          \partial_{\gamma_k} \psi^{j_k}(z_a)
        \right).
    \end{align*}
  \end{proof}
  ...
  \begin{equation*}
	...
	\sum_{a=1}^N \left( p_a \otimes \delta_{q\indices{_a_\emptyset}} +
	 \sum_{|\alpha| < k } \sum_{l = 1}^{|\alpha|}
		\sum_{
			\substack{
				j_1,\dots,j_l \in \{1,\dots,n\} \\
				[\gamma_1,\dots,\gamma_l] \in \Pi(\alpha,l)
				}
			}
			p\indices{_a_m^\alpha} \partial_{j_1 \cdots j_l}u^m (q\indices{_a_\emptyset})
      q\indices{_a^{j_1}_{\gamma_1}} \cdots
      q\indices{_a^{j_l}_{\gamma_l}}
			\right)
  \end{equation*}
  ...
  \begin{equation*}
    ...
    \sum_{ | \alpha | \leq k } \sum_{l=1}^{|\alpha|}
		\sum_{
			\substack{
				j_1,\dots,j_l \in \{1,\dots,n\} \\
				[\gamma_1,\dots,\gamma_l] \in \Pi(\alpha,l)
			...
			p\indices{_a_i^\alpha} q\indices{_a^i_{[j_1 \cdots j_l]}}
      \sum_{m=1}^l  \partial_{\gamma_m} w^{j_m}(z_a) \left( \prod_{n \neq m}  \delta_{\gamma_n}^{[j_n]} \right)
  \end{equation*}
  \je{I think the sum over $m$ should be replaced by a (tensor)
    product, also in the formula two below.}
    \dm{I can't really comment. I'm still struggling to read this formula. In my own derivation I think I was able to get rid of some of the interior sums (this step probably corresponds to contracting out the Kronecker deltas), so it looks a bit different. I don't really understand what you mean anyway though...?}
    \hoj{I also don't understand this comment.  $J_R$ and $J_L$ are currently written as tensor products below.  Perhaps the change was already made and these todo boxes can be deleted.}
...
%  \dm{I dont' understand this. How can a multi-index by equal to an integer?}
%  \je{I hope you agree that saying that we generalize the Kronecker symbol to multi-indices is brief and clear.}
  We can write these momentum maps explicitly, using the Dirac delta distribution, as
  \begin{equation*}
    J_L^{(k)}(q,p) =
    p\indices{_a_i^\alpha} dx^i \otimes
    \sum_{|\alpha| \leq k } \sum_{k=1}^{|\alpha|} (-1)^{k}
    \left(
      \sum_{\substack{
          j_1,\dots,j_k \in \{1,\dots,n\} \\
          [\gamma_1,\dots,\gamma_k] \in \Pi( \alpha,k)
      ...
      q\indices{_a^{j_1}_{\gamma_1}} \cdots q\indices{_a^{j_k}_{\gamma_k}}
      \partial_{j_1 \cdots j_k} \delta(\cdot - q^{(0)}_a) \right)
  \end{equation*}
  and
  \begin{equation*}
    J_R^{(k)}(q,p) =
    \sum_{ | \alpha | \leq k } \sum_{l=1}^{|\alpha|}
		\sum_{
			...
			p\indices{_a_i^\alpha} q\indices{_a^i_{[j_1 \cdots j_l]}}
      \sum_{m} (-1)^{|\gamma_m|}\left( \prod_{n \neq m}  \delta_{\gamma_n}^{[b_n]} \right) \otimes \partial_{\gamma_m}\delta(\cdot - z_a)
  \end{equation*}
...
Let $x(s,t) = \varphi_t(x_0(s))$ for some loop $x_0(s)$.
Kelvin's circulation theorem states that the circulation
\begin{align*}
  \Gamma(t) = \oint u( x(s,t) )  \cdot \partial_s x(s,t) ds
\end{align*}
is constant in time.
...
is merely an instance of Noether's theorem.
...
and this Hamiltonian is $\SDiff(\R^n)$ invariant.
In this instance we have a dual pair $\lambda, \rho : T^*\SDiff(\R^n) \to \mathfrak{X}_{\rm div}(\R^n)$ induced by
the left and right action of $\SDiff(\R^n)$ on itself.
The dynamics on $T^*\SDiff(\R^n)$ exhibits the constant of motion
\begin{align}
  \langle \rho(\varphi,p_\varphi) , w \rangle = \langle p_\varphi , T\varphi \cdot w \rangle \label{eq:circulation}
\end{align}
for all $w \in \mathfrak{X}_{\rm div}(\R^n)$.
A ``regularized'' version of this conservation law holds in our regularized models.
Our models are obtained by using the deformed Hamiltonian
$h_\epsilon$ in place of $h_0$.
These regularized Hamiltonians are also $\SDiff(\R^n)$ invariant, and
therefore \eqref{eq:circulation} is also conserved in the regularized models.

Moreover, these conserved quantities are inherited by the finite dimensional jet-particle solutions.
To see this, it is useful to define the $k$-th order isotropy group,
$\iso^{(k)}(z) = \{ \psi \in \SDiff(\R^n) \mid J^{(k)}_z( \psi) = id_z \}$.
Then we see that $Q^{(k)}_1 = \SDiff(\R^n) / \iso^{(k)}(z)$.
The space $T^*Q^{(k)}$ then naturally embeds into $T^*\SDiff(\R^n) / \iso^{(k)}(z)$.
The commutativity of the following diagram uniquely defines the jet-particle solutions.
\begin{equation}
 \begin{tikzcd}
   \mathfrak{X}_{\rm div}(\R^n)^* & \ar[l,"\lambda"'] T^*\SDiff(\R^n) \ar[r,"\rho"] \ar[d,"\pi"] & \mathfrak{iso}(z)^* \\
   & T^*\SDiff(\R^n) / \iso^{(k)}(z) & \\
   & T^*Q^{(k)} \uar{i} \ar[uur,"J_R"',bend right = 30] \ar[uul,"J_L",bend left = 30] &
 \end{tikzcd}
\end{equation}
Again, the right leg of both dual pairs yields the conserved quantities.
From this commutative diagram, it is immediately clear that the conservation law exhibited in
out particle models is a shadow of the conservation of \eqref{eq:circulation}.
The conservation of circulation of the full regularized model is given by $\rho$,
while that of the jet-particle solutions is given by $J_R$.
In other words, conservation of $J_R$ in the jet-particle solutions is a shadow
of the conservation of $\rho$, which is equivalent to conservation of \eqref{eq:circulation}.
As \eqref{eq:circulation} contains Kelvin's circulation theorem,
we see that our particle models contain a model of Kelvin's circulation theorem within them.
This is proven rigorously in \cite[Theorem 5.5]{JacobsRatiuDesbrun2013}.
\dm{
Henry, I'm not sure I fully understand Theorem 5.5 in that paper, but I'm not convinced the commutativity is shown there...? If I'm reading right, there is only one momentum map mentioned there, which is called $J_\omega$ and I guess is equal to our $J_R$. But the momentum map $\rho$ does not seem to be mentioned there. Still, I agree the diagram does commute...}
\hoj{By having written \cite{JacobsRatiuDesbrun2013} I have brainwashed myself into thinking the relationship with Kelvin's circulation theorem is clearer than it is to people less entrenched.  The relationship seemed obvious to me, but  I think you guys are better judges of what is "obvious".  Anyways, you are correct that $J_\omega$ in Theorem 5.5 \cite{JacobsRatiuDesbrun2013} is nearly identical to our $J_R$.  There is a slight difference.  $J_\omega$ maps to the (finite dimensional) dual Lie algebra of the Lie group $\iso(z) / \iso^{(k)}(z)$. The relationship with Kelvin there is "obvious" because any $\omega$ in Theorem 5.5 is of the form $\omega = Jet^{(k)}( \eta)$ for some $\eta \in \iso(z)$.  The proof is now given by checking the commutativity directly, and noting that the action of $\omega$ on $Q^{(kj)}$ is given by $\omega \cdot q \equiv \eta \cdot q$.  It is probably hard to read any individual part of \cite{JacobsRatiuDesbrun2013} in isolation, and I apologize for that.  Perhaps it would be best if we provide a independent proof of the commutativity and merely say ``c.f. Theorem 5.5 \cite{JacobsRatiuDesbrun2013}".  If this is what you are suggesting, then I agree, and you should go for it David.}
 As a side remark we mention that the commutativity of the above diagram implies 
\begin{align}
	\left<J_R ((q, p)), w\right> = \left<J_L((q, p)), T \varphi (w \circ \varphi^{-1})\right>, \label{eq:alternative}
\end{align}
for all $w \in \mathfrak{X}_{\rm div}(\mathbb{R}^n)$, where $\varphi$ is any diffeomorphism that is mapped to $q$ by the quotient map $ \SDiff(\R^n) \to Q^{(k)}_1$. Recall that in the previous section we computed both $J_L$ and $J_R$ using the coordinate expressions, presented in the proof of proposition \eqref{HL_action_prop},  for the respective actions on $Q^{(k)}_1$. Evidently, equation \eqref{eq:alternative} provides an alternative route to derive one momentum map once one has found the other.
\hoj{The current form of the paper is fairly unified, so I would hesitate to discard any existing proofs.
However, I also suspect you are correct, for the same reasons.  If it turns out to be easy, what do you think about adding the proof
as an alternative derivation?  We can justify having two proofs because this latter proof also tells us how things relate to Kelvin's circulation theorem,
and would smooth the transition into this section (i.e. section \ref{sec:Kelvin}) }
...
the zeroth order particles case, this was already studied by Michor
and Mumford~\cite{MumfordMichor2013}, and they found that two
...
\begin{equation}\label{eq:EoM-lin-reduced}
  \begin{aligned}
    \dot{\tilde{q}}^i &= \frac{1}{2}\big( K^{ij}(0) - K^{ij}(2\tilde{q}) \big) \bar{p}_j, \\
...
quantities are indeed conserved and that two $0$-jetlet particles
...
quantities present in the system. Table~\ref{tab:conservation-errors}
shows the absolute errors in various conserved quantities for the
various simulations described above over a time of $60$ seconds. The
value of the energy in all cases is between $1$ and $2$. The linear
and angular momenta together form the value of $J_L^{(1)}$. The error
in $J_R^{(1)}$ is shown indivually for both jetlet particles, since
this momentum is not exchanged betwee particles (in contrast with
$J_L^{(1)}$); note that a zero here indicates that no difference was
found up to floating point precision.

\begin{table}[tb]
\centering
\begin{tabular}{|l|rrrrr|}
  \hline
  experiment $\backslash$
             conserved quant.       & energy  & lin.~mom. & ang.~mom. & $J_R^{(1)}$ part.~1
                                                                                & $J_R^{(1)}$ part.~2 \\
  \hline
  \hline
  0-jetlets merging                 & 9.9E-05 & 6.6E-10   & 5.9E-05   & 3.5E-05 & 1.4E-04 \\
  \hline
  0-jetlets scattering              & 3.6E-05 & 1.8E-14   & 3.0E-05   & 0       & 0 \\
  \hline
  1-jetlets merging                 & 1.7E-04 & 5.5E-11   & 4.1E-07   & 3.0E-04 & 3.6E-04 \\
  \hline
  1-jetlets scattering              & 1.8E-05 & 7.1E-16   & 1.7E-07   & 3.5E-07 & 3.5E-06 \\
  \hline
  \parbox{4cm}{0-jetlets merging,\\
  third 0-jetlet present}           & 2.1E-04 & 3.8E-12   & 6.1E-05   & 0       & 0 \\
  \hline
  \parbox{4cm}{0-jetlets scattering,\\
  third 0-jetlet present}           & 3.1E-06 & 8.6E-14   & 1.0E-05   & 0       & 0 \\
  \hline
  \parbox{4cm}{0-jetlets merging,\\
  third 1-jetlet present}           & 2.3E-04 & 1.0E-12   & 5.2E-05   & 0       & 0 \\
  \hline
  \parbox{4cm}{0-jetlets scattering,\\
  third 1-jetlet present}           & 1.3E-05 & 4.6E-14   & 2.1E-05   & 0       & 0 \\
  \hline
  `stretching' 1-jetlets merging    & 1.4E-04 & 3.4E-13   & 1.9E-07   & 2.9E-04 & 0 \\
  \hline
  `stretching' 1-jetlets scattering & 1.5E-04 & 4.6E-15   & 2.0E-07   & 3.5E-05 & 0 \\
  \hline
\end{tabular}
\caption{Absolute errors in the conserved quantities for a number of numerical simulations.}
\label{tab:conservation-errors}
\end{table}

\hoj{Some comments on the table:  I'm not sure we can use this table by itself. 
Conservation of $J_R, E,$ etc was verified through formal proof, 
so this table, by itself seems to be a test
of the numerical accuracy of scipy's ode integration method.  We will need to include some plots of momenta exchanges.
One thing we could do is plot $\mu$ vs $J_R$, since those are the same units.}
...
\dm{At some point we'll need to make references uniform. (Currently journal names are sometimes written out, sometimes abbreviated; same for the first names of authors.}
