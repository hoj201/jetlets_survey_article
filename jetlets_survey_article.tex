...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and the corresponding quotient map is given by $\Jet_z^k$.  
 ...
 for any $(p,q) \in T^*Q^{(k)}_N$ and any $v_\varphi \in T\SDiff(\mathbb{R}^n)$ {\color{blue} with $\Jet_z^k(\varphi) = q$ (c.~f. \cite{HRS}, equation (2.2.4)) } we can
 ...
 \dm{Added a condition above in blue that seems necessary -- please check whether you agree and remove color}
