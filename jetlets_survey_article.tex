...
where we wrote $\Pi(\alpha, k)$ for the set of $k^{\rm th}$-order partitions of a multi-index $\alpha$. We refer to \ref{app:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
    ...
    \begin{align*}
    (\varphi \cdot q)\indices{^{ia}_\alpha} = 
     \sum_{k=1}^{|\alpha|}
      \left(
        \sum_{
        \substack{
          ...
          \partial_{b_1 \cdots b_k} \varphi^a(q \indices{^{i}_\emptyset} ) 
          q \indices{^{b_1}_{\gamma_1}} \cdots
          q \indices{^{b_k}_{\gamma_k}}
        \right),
    \end{align*}
    except for the component where $|\alpha | = 0$ in which case we observe $(\varphi \cdot q) \indices{^{ia}_\emptyset} = \varphi(q_\emptyset)$.
    ...
    and easily apply the same process to $Q_N^{(k)}$.  In this case we observe the action to be
      \begin{align*}
      (q \cdot \psi)\indices{^{ia}_\alpha} = \sum_{k=1}^{|\alpha|}
      \left(
        \sum_{
        \substack{
          b_1,\dots,b_k \in \{1,\dots,d\} \\
          ...
          q\indices{^{ia}_{[b_1,\dots,b_k]} } 
          \partial_{\gamma_1} \psi^{b_1}(z^i) \cdots
          \partial_{\gamma_k} \psi^{b_k}(z^i)
        \right).
    \end{align*}
  \end{proof}
  ...
  \begin{align*}
	...
	\sum_{i=1}^N p_i \otimes \delta_{q^{(0)i}} +
	 \sum_{|\alpha| < k } \sum_{j = 1}^{|\alpha|}
		\sum_{
			...
			p \indices{_{ia}^{\alpha}} \partial_{b_1 \cdots b_j}u^a ( q^{i}_\emptyset)  q\indices{^{b_1}_{\gamma_1}} \cdots q\indices{^{b_j}_{\gamma_j}}
  \end{align*}
