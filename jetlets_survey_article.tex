...
that $\psi$ get projected out by (the tangent map of) $\pi = \Jet_z^k$.
...
then the submanifold $i\bigl(T^*Q^{(k)}_N\bigr) = (J_{\rm conv}^{(k)})^{-1}(0) / \iso^{(k)}(z)$.  For more details see Appendix \ref{app:diagramatic}.
