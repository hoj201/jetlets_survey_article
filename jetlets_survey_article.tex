...
In Proposition~\ref{HL_action_prop} we will generalize this construction to define a right action for the higher levels in the hierarchy of particle-like solutions.
  ...
  \todo[inline]{I think this is false, and I agree with the reviewer.  We need to consider a weaker case, perhaps $\kernel(J_{1}) \subset \kernel(J_{2})^{\omega}$ or something}
  ...
  $q^{(1)}_a = D\psi|_{z_a}$, or, $q = \Jet^1_z(\psi)$).
  Secondly, we can observe that the right action of $\iso(z)$ commutes with the
  left action of $\SDiff(\R^{n})$ on $Q^{(1)}$.
  Therefore, Lemma A.9 \todo{hyperlink this} would allow us to complete the proof
  if only we could show that $G_{1}$ is infinitesimally transitive with respect to $J_{2}$.
  In other words, we must verify that for any $(q,p) \in T^{*}Q$ and
  any $w \in T_{(q,p)} T^* Q$ such that $TJ_{R}^{(1)} \cdot w = 0$
  there exists a $u \in \mathfrak{X}_{\rm div}(\R^{n})$
  such that $u \cdot (q,p) = w$.
  To check this, note that
  \begin{align*}
  	J_{R}^{(1)}(q,p) = - p\indices{_{am}^{j}} q \indices{_{a}^{m}_{k}} dz^{k} \otimes  \partial_{j} \delta_{z_{a}}
  \end{align*}
    By representing this as tuple of matrices (where $jk$th compenent of the $a$th matrix is the coefficient of $dz^{k} \otimes \partial_{j} \delta_{z_{a}}$) then the above formula may be simply written as
    \begin{align*}
    	J_{R}^{(1)}(q,p) = -[p^{(1)}]^{T} [q^{(1)}]
    \end{align*}
  If we let $w = (q,p,\delta q , \delta p) \in TT^{*}Q$ then
  \begin{align*}
  	0=TJ_{R}^{(1)} \cdot w = - [ \delta p^{(1)} ]^{T} [q^{(1)}] - [p^{(1)} ]^{T} [\delta q^{(1)} ]
  \end{align*}
  if and only if
  \begin{align}
  	[\delta p^{(1)} ]^{T} = - [p^{(1)}]^{T} [\delta q^{(1)}] [q^{(1)}]^{-1}. \label{eq:transitivity constraint}
  \end{align}
  
  Thus we must prove that there exists a vector-field $u$ such that $u \cdot (q,p)$ satisfies \eqref{eq:transitivity constraint}.
  We can observe that in matrix notation, the cotangent lifted action is given by
  \begin{align*}
  	u \cdot (q,p) = ( q^{(0)} , q^{(1)} , p^{(0)} , p^{(1)} , -u(q^{(0)}) , -Du(q^{(0)}) \cdot q^{(1)} , Du(q^{(0)})^{T} p^{(0)} , Du(q^{(0)})^{T} p^{(1)})
  \end{align*}
  We see that $u(q^{0})$ and $Du(q^{(0)})$ may be chosen as arbitrary elements of $\R^{n}$ and $\SL(n)$ respectively.
  Thus
  
  Secondly, the cotangent-lifted action preserves level sets of $J_R^{(k)}$.
...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and the corresponding quotient map is given by $\Jet_z^k$.  
 The space $T^*Q^{(k)}_N$ naturally embeds into $T^*\SDiff(\R^n) / \iso^{(k)}(z)$, where the quotient is by the cotangent-lifted action, in the sense that
 for any $(q,p) \in T^*Q^{(k)}_N$ and any $v_\varphi \in T_\varphi \SDiff(\mathbb{R}^n)$ with $\Jet_z^k(\varphi) = q$ we can
 consider the element $i(q,p) \in T^*\SDiff(\R^n) / \iso^{(k)}(z)$ given by $\langle i(q,p) , v_\varphi \rangle = \langle p , v_\varphi( \varphi^{-1} \cdot q) \rangle$
 \cite[(2.2.4)]{HRS}.
...
For the left side of the diagram, note that the statement $\pi(p_\varphi) = i(p_q)$ for some $p_\varphi \in T^* \SDiff(\mathbb{R}^n)$ and $p_q \in T^*Q_N^{(k)}$ is equivalent to the statement that $\langle p_\varphi , w \circ \varphi \rangle = \langle p_q , \Jet^{(k)}_z ( w \circ \varphi) \rangle$.
...
\todo[inline]{
 I think we only need to consider the case $\kernel(J_{1}) \subset \kernel(J_{2})$.
In fact, our ``dual pairs'' are not dual pairs, although our conservation theorem apparently does not require this.}
  ...
  \begin{proof}
    ...
    \begin{align*}
      ...
      &= -\lie_{\rho_2(\xi_2) }[ \langle \theta,\rho_1(\xi_1) \rangle] = - \langle \lie_{\rho_2(\xi_2)}[\theta] , \rho_1(\xi_1) \rangle - \langle \theta , [\rho_2(\xi_2), \rho_1(\xi_1) ] \rangle.
    \end{align*}
...
%    symplectically orthogonal.
    ...
    But since $G_1$ acts infinitesimally transitively on the level sets of $J_2$, we see
...
\begin{lem}\label{lem:transitive-action}
  ...
  \item for any $\mu$ in the range of $J_2$, the level set $J_2^{-1}(\mu)$ is a section of $T^* Q$.
  \end{enumerate}
...
\begin{proof}
  ...
  $g \in G_1$ such that $q' = g^{-1} \cdot q$. Since $\rho_1$ preserves
