...
\title[Dual pairs and gauged-particle methods for fluid models]%
{Dual pairs and gauged-particle methods for ideal incompressible fluid models in $n \geq 2$ dimensions}
...
\hoj{During the group meeting I said something like
  ...
  in the context of fluids, where the configuration manifold is Diff(M).}
...
Each method in the hierarchy consists of particles
...
\begin{enumerate}
  ...
  \item We will compute a nested sequence of conserved quantities at 
    ...
    of circulation.
  \item We will numerically compute some particle-like solutions at the zeroth and first levels in the hierarchy.  We will observe a form of cascade in which solutions at the $k^{\rm th}$ level asymptotically approach
...
We shall denote the space of vector fields on a manifold $M$ by
$\mathcal{X}(M)$.


contraction, Lie derivative, group actions, infinitesimal generators,
...
asymptotically $p_r = \frac{\xi}{r} + \mathcal{O}(1)$. Further, we
have dynamics
\begin{equation}
...
\intertext{and inserting our reconstructed solution leads to}
  &= -\Big[R_\varphi \cdot \begin{pmatrix} \xi \\ p_\varphi \end{pmatrix}\Big]_i
...
\end{equation*}
or in short, $\dot{\mu} = [\mu,\xi]$ with $\xi = \textrm{D}u(0)^T$. To
...
\begin{equation*}
  \tilde{\mu}(t)
  = -\Big[R_\varphi \cdot \begin{pmatrix} \xi \\ p_\varphi \end{pmatrix}\Big] \cdot
     \Big[R_\varphi \cdot \begin{pmatrix} 1   \\ 0         \end{pmatrix}\Big]^T
  = -\Ad_{R_\varphi} \begin{pmatrix} \xi & 0 \\ p_\varphi & 0 \end{pmatrix}
\end{equation*}
...
Thus, in the topology induced by $\mathfrak{X}_{\rm div}(\R^n)^*$, we
can view the boundary of $T^*Q_2^{(0)}$ as (part of) $T^*Q_1^{(1)}$,
...
on $\mathfrak{X}_{\rm div}(\R^n)^*$ is continuous. We have not
investigated this.
