...
where we wrote $\Pi(\alpha, k)$ for the set of $k^{\rm th}$-order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{app:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  \begin{equation*}
	...
	\sum_{a=1}^N \left( p_{am} u^m(q^{(0)a})  +
...
%  \je{I hope you agree that saying that we generalize the Kronecker symbol to multi-indices is brief and clear.}
  ...
  \begin{align*}
    ...
    \sum_{a=1}^{N}
    \sum_{m=1}^{n} \Bigg(
	p_{am} dx^m \otimes \delta( \cdot - q^{(0)a} )
	\\
    &\quad+
    \sum_{
    	\substack{
      ...
      q\indices{_a^{j_1}_{\gamma_1}} \cdots q\indices{_a^{j_k}_{\gamma_\ell}}
      \partial_{j_1 \cdots j_\ell} \delta(\cdot - q^{(0)}_a)
      \Bigg)
  \end{align*}
  and
  \begin{equation*}
  ...
  \end{equation*}
  \hoj{I actually found a lot of errors in these momentum maps.  I'm more confident in the first two expression (i.e. the $<J(q,p),u>$ terms)
  	so if you want to check my work, it would be best to check that the second expressions (i.e. the one-form densities) are consistent with the $<J , u>$ expressions.
	and to check the $<J,u>$ via independent derivation.  \emph{However, I'd caution against editing the $<J(q,p) , u>$ terms to make them consistent with the one-form density versions of $J_L$ and $J_R$}.}
