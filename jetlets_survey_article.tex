...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  \todo{DM: I replaced notation $T\varphi \circ w$ by $T\varphi.w$ to make the meaning clearer. Ok?} 
 \begin{align}
  \langle J_R^{(k)}(q,p) , w \rangle = \langle (q,p) , \Jet_z^{(k)}( T\varphi.w) \rangle, \label{Moma_right}
   \end{align} 
  for any $\varphi \in \SDiff(\R^n)$ whose $k$-jet is given by $q$ for any $(q,p) \in T^*Q^{(k)}_N$. Here, we wrote $T\varphi.w$ for the function obtained by applying the differential of $\varphi$ to $w$.   
...
\end{prop}
  ...
  This is a relaxation of the more prevalently invoked notion of a dual pair \cite{MarsdenWeinstein1983,Weinstein1983}.
  Let $(S,\omega)$ be a symplectic manifold.
