...
The unreduced Hamiltonian is $H(R,P) = h(J_R(R,P))$.
  ...
  \begin{prop}[see \S 2.5 \cite{HolmBook2}] \label{prop:Nambu}
    ...
    through the isomorphism of Lie algebras given by the hat map
    \begin{align*}
      \hat{ \bf x } =
      \begin{bmatrix}
        0 & -z & y \\
...
\begin{align*}
 \left(1 - \frac{1}{k \sigma^2} \Delta \right) \cdot \int_{\R^n} K_{k,\sigma}^{ij}(x - y) m_j(y) dy = \delta^{ij} m_j(x).
\end{align*}
\je{The Laplacian should be raised to the power $k$, right?}
...
momentum maps $J_L,J_R : T^{\ast}\SDiff(\R^n) \to \mathfrak{X}_{\rm div}(\R^n)^*$. In particular these maps form the dual pair
\begin{align*}
...
\end{align*}
\je{I moved $\varphi$ acting on $q$ to the other side to make the
  definition more explicit. Henry please revert or remove this comment
  if OK.}
...
That is to say, any covector field, $m \in \mathfrak{X}^*_{\rm
  div}(\R^n)$, it can be paired with a vector field $v \in
\mathfrak{X}_{\rm div}(\R^n)$ to produce the real-valued function
...
\begin{figure}
	...
	\caption{A $0$-jetlet with momentum $m(x) = (1,0) \otimes \delta_{\bf 0}(x)$}
...
\begin{proof}
  ...
  \je{Modulo analytic details I suppose?}
\end{proof}
