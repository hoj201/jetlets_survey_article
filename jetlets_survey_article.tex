...
and this Hamiltonian is $\SDiff(\R^d)$ invariant.
In this instance we have a dual pair $\lambda, \rho : T^*\SDiff(\R^d) \to \mathfrak{X}_{\rm div}(\R^d)$ induces by
the left and right action of $\SDiff(\R^d)$ on itself.
The dynamics on $T^*\SDiff(\R^d)$ exhibit the constant of motion
\begin{align}
  \langle \rho(\varphi,p_\varphi) , w \rangle = \langle p_\varphi , T\varphi \cdot w \rangle \label{eq:circulation}
\end{align}
for all $w \in \mathfrak{X}_{\rm div}(\R^d)$.
A ``regularized'' version of this conservation law holds in our regularized model.
Our models are obtained by using the deformed Hamiltonian
$h_\epsilon$ in place of $h_0$.
These regularized Hamiltonians are also $\SDiff(\R^d)$ invariant, and
...
$\iso^{(k)}(z) = \{ \psi \in \SDiff(\R^d) \mid J^{(k)}_z( \psi) = id_z \}$.
Then we see that $Q^{(k)} := \SDiff(\R^d) / \iso^{(k)}(z)$.
The space $T^*Q^{(k)}$ then naturally embeds into $(T^*\SDiff(\R^d)) / (\iso^{(k)}(z))$.
The commutativity of the following diagram uniquely defines the jet-particle solutions.
\begin{equation}
 \begin{tikzcd}
   \mathfrak{X}_{\rm div}(\R^d)^* & \ar[l,"\lambda"] T^*\SDiff(\R^d) \ar[r,"\rho"] \ar[d,"\pi"] & \mathfrak{iso}(z)^* \\
   & (T^*\SDiff(\R^d)) / \iso^{(k)}(z) & \\
