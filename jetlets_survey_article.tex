...
yields the notorious ``cascade to smaller scales'', providing a raison d'etre
...
Therefore, it would be useful to have a toy model of self-similarity and cascade,
both from the standpoint of understanding the structures involved,
and from the standpoint of creating more accurate CFD code.

One cause of the self-similarity of  fluids 
is the self-similarity of the configuration space
itself, the diffeomorphism group $\Diff(M)$.
In a nutshell, a fluid contains ``little'' fluids.
However, Lagrangian methods in CFD, such as \emph{smooth particle hydrodynamics} and
\emph{vortex methods}, do not exhibit structures to express any notion of self-similarity.
 One means of doing this was presented in \cite{JacobsRatiuDesbrun2013}
 where an infinite-dimensional reduction by symmetry was executed,
 to produce a finite dimensional system from an infinite dimensional
 regularized fluid model.
 The finite dimensional system was an ODE for particles which carry models of the diffeomorphism group, known as jet groups.
 Thus obtaining a Lagrangian analog of ``whirls within whirls''.
 However, the specifics of the regularized fluid model were not determined,
 and the analysis of \cite{JacobsRatiuDesbrun2013} was thus purely theoretical.
 
 Later, a regularized model of ideal fluids was presented by \cite{MumfordMichor2013}.  This new model was subject to the analysis presented in 
 \cite{JacobsRatiuDesbrun2013}, and gave a specific and easily
 implementable manifestation of the particle methods described.
 We subsequently simulated solutions and observed cascade phenomena as an emergent behavior.
 Each of these particles exists in a hierarchy, and the cascade phenomena observed consisted
 of particle at the $k$th level in the hierarchy colliding and merging into a particle in the $k+1$th level.
 As a result we obtained the first orderly characterization of the
 cascade to fine scales in a Lagrangian model
 \cite{CotterHolmJacobsMeier2014}.
  Such a ``toy'' model is crucial to the understanding of fluids,
  ...
  and cascade (at arbitrary resolutions!),
  one is forced to stare at self-similarity in the face directly.
  Our research presents the first such model to fill this gap.
...
\end{align}
Solutions to \eqref{eq:MMDiff} exhibit existence and uniqueness for all time \cite[Thm 2]{MumfordMichor2013}.
...
as a model for ideal fluids. From now on we shall often suppress the
...
\begin{align*}
  Q_N^{(0)} = \left\{ (q_1,\dots,q_N) \in \R^n \times \dots\times \R^n
                 \mid q_a \neq q_b \text{ when } a \neq b \right\}.
\end{align*}
We will index the particles with roman indices $a,b,c,\dots$ from the
start of the alphabet, and Cartesian coordinates in $\R^n$ with roman
indices $i,j,k,\dots$ Thus each $q \in Q_N^{(0)}$ can be decomposed
into $N$ particles as $(q_1,\dots,q_N)$, and the $i^{\rm th}$
coordinate of the $a^{\rm th}$ particle is denoted by $q_a^i$.
...
\end{align*}
where the sum over $a = 1,\dots,N$ is implied under
...
\begin{align*}
	dh( p_a \otimes \delta_{q_a} )  (x) = K_{k,\sigma}^{ij}(x - q_a) p_{a\,j} \pder{}{x^i}
\end{align*}
...
\je{Isn't this $k = \infty$? And same in reference to Figure~\ref{fig:zoo}?}
...
\end{prop}
\begin{proof}
	Note that $H = h_{k,\sigma} \circ J_L^{(0)}$ and apply theorem \ref{thm:commuting_actions}.
\end{proof}
...
\label{sec:first_order}
  ...
  \begin{align*}
    ...
    = p_a^{(0)} \otimes \delta_{q_a^{(0)}} + \dots
  \end{align*}
  ...
  \end{prop}
  \begin{proof}
    The actions of $\SL(n)$\todo{JE: shouldn't this be $\iso(z)$?} and $\SDiff(\R^n)$ on $Q_N^{(1)}$ clearly commute
  ...
  \begin{prop} \label{prop:1-solutions}
  	If $K = K_{k,\sigma}$ is $C^{\frac{n}{2} + 2}$ then $H = h_{k,\sigma} \circ J_L$
	is $C^1$ and given by the expression
	\begin{multline*}
		H^{(1)}(q,p) = \frac{1}{2} p_{a\,i} K^{ij}(q_a - q_b) p_{b\,j}
			+ p_{a\,i}^l\, q^k_{a\,l} (\partial_k K^{ij})(q_a - q_b) p_{b\,j} \\
			- p_{a\,i}^n\, q^l_{a\,n} (\partial_l \partial_k K^{ij})(q_a - q_b) q_{b\,m}^k\, p_{b\,j}^m.
  \end{multline*}
  ...
  multi-indices on $\R^n$ (see Appendix \ref{app:multi} for details).  A typical coordinate on $Q^{(k)}_N$ will therefore look
  ...
  of a diffeomorphism at some point $z_a$, i.e.\ $\partial_\beta \varphi^i(z_a)$.
  This is the primary motivation for this notational convention.
  We will let $q^{(k)\,i}_a$ denote the collection of $q^i_{a\,\beta}$
  where $| \beta | = k$.
  ...
  \begin{proof}
    ...
    \begin{align*}
      \partial_\alpha( \varphi_1 \circ \varphi_2) = \sum_{n=1}^{|\alpha|}
      \left(
        \sum_{
        \substack{
          ...
          \partial_\beta \varphi_1(\varphi_2(x)) 
          \partial_{\gamma_1} \varphi_2^{\beta_1} \cdots
          \partial_{\gamma_n} \varphi_2^{\beta_n}
        \right).
    \end{align*}
    One can read directly from the expression that
    a $k^{\rm th}$ order derivative only depends on $k^{\rm th}$ and lower order
  ...
  \begin{align*}
    J_L^{(k)}(q,p) &= p \indices{_{ia}^{\alpha}} {\bf e}^a \otimes
    \sum_{n=1}^{|\alpha|} (-1)^{n}
    \left(
      \sum_{\substack{
          \beta = [\beta_1,\dots,\beta_n] \in \mathbb{I}^n \\
      ...
      q\indices{^{i\beta_1}_{\gamma_1}} \cdots q\indices{^{i\beta_n}_{\gamma_n}}
      \partial_{\beta} \delta(\cdot - q^{(0)i}) \right)\\
    J_R^{(k)}(q,p) &= p\indices{_{ia}^\alpha} \sum_{n=1}^{|\alpha|}
    \left( \sum_{\substack{
          \beta = [\beta_1,\dots,\beta_n] \in \mathbb{I}^n \\
          [\gamma_1,\dots,\gamma_n] = \alpha
          }}
        \left[
          \sum_{k=1}^{n} (-1)^{|\gamma_k|} q \indices{^{ia}_\beta}
          \delta_{\gamma_1}^{\beta_1} \cdots \delta_{\gamma_{k-1}}^{\beta_{k-1}}
          \delta_{\gamma_{k+1}}^{\beta_{k+1}} \cdots \delta_{\gamma_n}^{\beta_n}
          {\bf e}^{\beta_k} \otimes \partial_{\gamma_k} \delta( \cdot - q^{(0)i})  \right]
     \right)
  \end{align*}
