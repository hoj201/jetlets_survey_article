...
a vector field.  If $h = h_{k,\sigma}$, then we calculate that $dh$
...
\dm{If we change the above and just introduce the momentum map directly, we should probably remove the following sentence.}
  ...
  \begin{align*}
    H(q,p) =\frac{1}{2} K_{k,\sigma}^{ij}(q_a - q_b)\, p_{a\,i}\, p_{b\,j}.
  \end{align*}
...
\end{prop}
\begin{proof}
	Note that $H = h_{k,\sigma} \circ J_L^{(0)}$ and apply theorems \ref{thm:dual_pairs} and  \ref{thm:commuting_actions}.
\end{proof}
...
\dm{We need to clarify the differentiability in the Proposition just below. Henry, do you have a reference where you picked this up when you wrote it? Or can you briefly explain your initial thoughts on this? -- Jaap, can you have a look at this as well, please?}
...
\dm{See earlier remarks about regularity in the Prop. below.}
  \begin{prop}
  	If $K_{k,\sigma}$ is $C^{\frac{n}{2} + 2k}$ then $H^{(k)} = h_{k,\sigma} \circ J_L^{(k)}$
	is $C^1$.
