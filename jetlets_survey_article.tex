...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  \begin{proof}
	...
	The cotangent lifted action is a left action, $\rho^* : \iso(z) \times T^*Q^{(k)} \to T^*Q^{(k)}$, defined implicitly by the constraint
	\begin{align}
		\langle \rho^*( \psi , (q,p) ) , \rho( (q,v) , \psi^{-1} ) \rangle = \langle (q,p) , (q,v) \rangle \label{eq:dual action}
	\end{align}
	for all $(q,p) \in T^*Q^{(k)}$ and $(q,v) \in TQ^{(k)}$.
	In particular, one can verify that $\rho^*( \psi , (q,p) )$ is a co-vector over the point $\tilde{q} = \rho( q , \psi^{-1}) \in T^*Q^{(k)}$.
	...
	\end{align*}
	By \eqref{eq:dual action} and \eqref{eq:tangent action} we see
	\begin{align*}
		\langle J_L( \rho^*( \psi , (q,p) ) ) , u \rangle &=  \langle  \rho^*( \psi ,  (q,p) )  ,  \rho( \Jet^{(k)}(u \circ \varphi) , \psi^{-1} ) \rangle \\
