...
We provide a detailed appendix. In Appendix A, a brief discussion of symplectic and Poisson manifolds is followed by the definition of weak dual pairs. Then we prove Theorem~\ref{thm:dual_pairs}, which is the core result used in the paper. Appendix \ref{app:diagramatic} presents an overview over the spaces used in the paper and relates them to some general results of reduction theory, see in particular Figures \ref{fig_diagram_1} and \ref{fig_diagram_2}. 
...
\end{equation*}
Note that $\Pi^{-1}(q) =g  G_{q_0}$ \todo[inline]{Henry, I believe it should be in this order here}
\hoj{I see.  So $\phi$ is a left action.  One question though, $\psi$ a right action right?}
\todo[inline]{Yes and no. $\phi$ is a left action. $\psi$ does not HAVE to be a right action. The point is that it has to commute with $\phi$. Of course, in our context, we do indeed work with a right action, but I don't think this is a prerequisite.}
for any $g \in G$ such that $\Pi(g) = q$, and hence we can identify $Q$  with $G/G_{q_0}$.\hoj{Here your quotienting with respect to a right action, as in our paper.  However, you've only specified a left action.} \todo[inline]{It is true that the quotient here is on the right. But you don't need to define anything for that, because for the quotient you only need group multiplication from the right. I don't think we specifically need to introduce right group multiplication. That should be understood in the concept of a group.} Consider a further Lie group, $K$, which also acts on $Q$ with group action $\psi: Q \times K \to Q$, and assume that $\psi$ and $\phi$ commute. This situation arises naturally when $K$ is a subgroup of $G$ and, in turn,  $G_{q_0}$ is a normal subgroup of $K$. In that case, one can \todo[inline]{I have reverted this. This really is a definition of what $\psi$ is}
\hoj{That derails my understanding.  If $\phi$ is the left action of $\SDiff(\R^n)$  on $Q$ and $\psi$ is the right action of $\iso(z)$ on $Q$, then the ``definition'' translates to saying $q \cdot k = gk \cdot q$ where $g \in \SDiff(\R^n)$ is such that $g \cdot z = q$ for any $q \in Q$ and $k \in \iso(z)$.  This is certainly false.} \todo[inline]{Your interpretation of this is correct, except that it should say $q \cdot k = gk \cdot z$. Isn't that consistent with your thinking?} determined define 
\begin{equation}
  \psi_k(q) := \Pi( g k) = \phi_{gk}(q_0). \label{right_ac_general}
\end{equation}
 Note that $\phi$ and $\psi$ commute,
\begin{equation*}
