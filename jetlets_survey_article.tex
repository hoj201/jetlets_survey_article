...
and cascade to obtain a better understanding the structures involved.
In particular we would like to emphasize the word \emph{model}.
It is unlikely that we can improve our understanding of self-similarity with more
accurate computational fluid dynamics (CFD) code, because many years have been spent
improving code in the direction of higher accuracy, and experimentalist provide the ultimate 
say on ``truth''.
Instead, we will propose a low dimensional toy model of self-similarity and cascade phenomena.
It is possible this model serves as an accurate numerical scheme for ideal fluids as certain parameters 
approach zero and infinity, but no attempt will be made to demonstrate this.
...
here for convenience.
  ...
  \begin{quote}
    ...
    Let $h \in C^2(P_1)$.
    If $(q,p)(t) \in S$ be a solution to Hamilton's equations
...
\end{enumerate}
By applying Theorem \ref{thm:dual_pairs} we obtained particle like solutions to the equations of motion
of a regularized model of ideal fluids, as well as conserved quantities.
Due to analytical concerns which will become clear later
we will instead consider a hamiltonian $h_\epsilon$
with a regularization parameter $\epsilon > 0$ taken from \cite{MumfordMichor2013}.
...
\hoj{
May we consider removing the stuff on the 
rigid body?  It's nice for us to read,
but I don't know if the most people will like it
or care.}
...
dual vector space to the incompressible vector field, $\mathfrak{X}_{\rm div}(\mathbb{R}^n)^*$, 
...
\end{prop}
\begin{proof}
	Note that $H = h_{k,\sigma} \circ J_L^{(0)}$ and apply theorems \ref{thm:commuting_actions} and \ref{thm:dual_pairs}.
\end{proof}
...
\label{sec:first_order}
  ...
  \begin{proof}
    ...
    Moreover, the action of $\SDiff(\R^n)$ is transitive on $\iso(z)$ orbits
    by inspection.
