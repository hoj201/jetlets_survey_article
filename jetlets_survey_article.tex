...
%  \je{Shouldn't $\delta_{q_a}$ be $\delta_{z_a}$ here? Or change the formula above? -- DM: I agree. I have changed it and removed the box.}
  ...
  For the statement of the next proposition, recall that the definition  of ${\rm iso}(z)$ was given earlier in \eqref{isotropy_group}. Note also that we write $\Jet^k_x: \SDiff(\R^n) \to Q_1^{(k)}$ for
    ...
    \dm{I have moved the definition of $\Jet$ here in front of the proposition statement (otherwise it's buried in the proof). I have also changed it slightly and added the generalization to multiple locations. }
	...
	\sum_{a=1}^N \left( p_{am} u^m(q^{(0)a})  +
	 \sum_{|\alpha| \leq k } \sum_{\ell = 1}^{|\alpha|}
		\sum_{
			...
			p\indices{_a_m^\alpha} \partial_{j_1 \cdots j_\ell}u^m (q\indices{^{(0)}_a})
