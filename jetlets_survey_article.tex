...
Next, we discuss a \todo{weak?} dual pair for this system.
The group $\SDiff(\R^n)$ acts on itself from the left and from the right.
These actions can be lifted to $T^* \SDiff(\R^n)$, and yield
momentum maps $J_L,J_R : T^*\SDiff(\R^n) \to \mathfrak{X}_{\rm div}(\R^n)^*$. In particular these maps form the weak dual pair
\begin{align*}
  ...
  \mathfrak{X}_{\rm div}(\R^n)^*.
\end{align*}
For a mathematically rigorous treatment of this weak dual pair we refer to \cite{GayBalmazVizman2012}.

By Theorem~\ref{thm:dual_pairs}, we can use this weak dual pair to derive dynamical properties of Hamiltonian equations defined on $\mathfrak{X}_{\rm div}(\R^n)^*$.
...
Unfortunately, this weak dual pair does not help us in solving \eqref{eq:MMDiff}
...
This will yield the particle-like solutions described in \cite{MumfordMichor2013}. In the later parts of the paper we will generalize the treatment to obtain a weak dual pair for each level in the hierarchy of particle-like solutions.
...
where we wrote $\Pi(\alpha, k)$ for the set of $k$-th order partitions of a multi-index $\alpha$. We refer to Appendix~\ref{sec:multi} for the details of our index conventions and to \cite{ConstantineSavits1996,Jacobs2014b} for a
  ...
  Equivalently, we may define $J_L^{(k)}$
...
and this Hamiltonian is $\SDiff(\R^n)$ invariant. Moreover, we have a weak dual pair
...
By applying Theorem \ref{thm:dual_pairs} with $J_1 = J_\text{conv}$ and $J_2 = J_\text{spatial}$ we know that Hamiltonian dynamics on $T^*\SDiff(\R^n)$ 
with respect to a Hamiltonian of the form $h \circ J_1$ will
exhibit the constant of motion
...
Then we see that $Q^{(k)}_N = \SDiff(\R^n) / \iso^{(k)}(z)$, and the corresponding quotient map is given by $\Jet_z^k$.  
 Let us also introduce the right action $R: \SDiff(\R^n) \times \SDiff(\R^n) \to \SDiff(\R^n)$ given by right composition of functions and write $R_\varphi = R(\cdot , \varphi)$, so that $R_\varphi \psi = \psi \circ \varphi$. We also introduce the cotangent lifted action $TR^*$ by means of the defining relation
 \begin{align}
 	\langle TR^*_{\psi^{-1}}p_\varphi, v_\varphi \circ \psi \rangle = \langle p_\varphi, v_\varphi\rangle
 \end{align}
 for arbitrary $p_\varphi \in T^*\SDiff(\R^n)$ and $v_\varphi \in T\SDiff(\R^n)$.

 The space $T^*Q^{(k)}_N$ naturally embeds into $T^*\SDiff(\R^n) / \iso^{(k)}(z)$, where the quotient is by the cotangent lifted action. More precisely, for any $(p,q) \in T^*Q^{(k)}_N$ we can construct the corresponding element $i(p, q) \in T^*\SDiff(\R^n) / \iso^{(k)}(z)$ in the following way: Take any $\varphi$ such that $\Jet_z^k(\varphi) = q$ and find $\iota(p, q) \in T^*\SDiff(\R^n)$ that satisfies, for all $v_\varphi \in T_\varphi \SDiff(\mathbb{R}^n)$, 
 \begin{align}
 \langle \iota(q,p) , v_\varphi \rangle = \langle p , v_\varphi( \varphi^{-1} \cdot q) \rangle, 
 \end{align}
 cf. \cite[(2.2.4)]{HRS}. Then set $i(p, q) = [\iota(p, q)]$. 
 
 To see that $i(p, q)$ is well defined, note that if $\psi = \varphi \circ \rho$ for $\rho \in \iso^{(k)}(z)$, then for any $v_\psi \in T_\psi \SDiff(\R^n)$
 \begin{align} \langle TR^*_{\rho^{-1}} \iota(p, q), v_{\psi}\rangle = \langle \iota(p, q), v_{\psi} \circ \rho^{-1}\rangle 
 = \langle p, v_{\psi} (\rho^{-1} \circ (\psi \circ \rho^{-1})^{-1}  \cdot q)\rangle =  \langle p, v_{\psi} ( \psi^{-1} \cdot q) \rangle. \notag
 \end{align}
 Since $[TR^*_{\rho^{-1}} \iota(p, q)] = [\iota(p, q)]$, we conclude that $i(q, p)$ is well defined.
 ...
 \begin{align}
 	\langle TR^*_{\varphi^{-1}} J_L^{(k)}(q, p), v_{\varphi}\rangle = \langle  J_L^{(k)}(q, p), v_\varphi \circ \varphi^{-1}\rangle = \langle p, v_\varphi (\varphi^{-1} \cdot q)\rangle. \notag
 \end{align}
...
%\begin{displaymath}
%\xymatrix{
%\mathfrak{X}_{\rm div}(\R^n)^* & \ar[l]_{J_{\rm spatial}} T^*\SDiff(\R^n) \ar[d]^{\pi} \ar[r]^{J_{\rm conv}} &  \mathfrak{iso}^{(k)}(z)^* \\
%& T^*\SDiff(\R^n) / \iso^{(k)}(z) & \\
...
\begin{displaymath}
\xymatrix{
\mathfrak{X}_{\rm div}(\R^n)^* &  \ar[l]_-{J_{\rm spatial}} T^*\SDiff(\R^n) / \iso^{(k)}(z)  \ar[r]^-{J_{\rm conv}}  & \mathfrak{iso}^{(k)}(z)^* \\
& T^*Q^{(k)}_N \ar[u]^i \ar[ur]_{J_R^{(k)}} \ar[ul]^{J_L^{(k)}} &
}
\end{displaymath}


Here, \todo{Do we need the superscript $k$ at the top right corner of the diagram?} $J_{\rm spatial}$ and $J_{\rm conv}$ are defined in the natural manner through their actions on representatives of the equivalence classes. For the left side of the diagram, we let $(p,q) \in T^*Q^{(k)}_N$ and $\varphi$ such that $\Jet_z^k(\varphi) = q$. Then we obtain from \eqref{i_explicit} that for any $w \in \mathfrak{X}_{\rm div}(\R^n)$
\begin{align}
	&\langle J_\text{spatial}( i(p, q)), w\rangle = \langle J_\text{spatial}( TR^*_{\varphi^{-1}} J_L^{(k)}(q, p)), w\rangle  \notag \\
...
\begin{align}
	&\langle J_\text{conv}( i(p, q)), w\rangle = \langle J_\text{conv}( TR^*_{\varphi^{-1}} J_L^{(k)}(q, p)), w\rangle \notag \\
	&= \langle  TR^*_{\varphi^{-1}} J_L^{(k)}(q, p), T\varphi. w \rangle = \langle  J_L^{(k)}(q, p)), (T\varphi. w) \circ \varphi^{-1}\rangle \notag \\
	&= \langle (p, q),  \Jet_z^k(T\varphi. w)\rangle = \langle  J^{(k)}_R(p, q), w\rangle, \notag
\end{align}
where we used \eqref{Moma_right} in the final step. In particular, $J_{\rm conv}$ is well defined on $T^*\SDiff(\R^n) / \iso^{(k)}(z)$.

%
%
% note that the statement $\pi(p_\varphi) = i(p_q)$ for some $p_\varphi \in T^* \SDiff(\mathbb{R}^n)$ and $p_q \in T^*Q_N^{(k)}$ is equivalent to the statement that $\langle p_\varphi , w \circ \varphi \rangle = \langle p_q , \Jet^{(k)}_z ( w \circ \varphi) \rangle$.
...
%  \end{equation*}
%  when $\pi(p_\varphi) = i(p_q)$.
%  This verifies the left half of the diagram. The right hand side can be verified in the same manner.
...
% This shows the commutativity of the right-hand part of the  diagram above (compare \eqref{compare_1} and \eqref{compare_2}).
  ...
  Again, the right legs of both \todo{both weak?} weak dual pairs yield the conserved quantities.
...
\end{equation}
Note that $\Pi^{-1}(q) = g G_{q_0}$ if $\Pi(g) = q$, and hence we can identify $Q$  with $G/G_{q_0}$. Consider a further group, $K$, which also acts on $Q$ with group action $\psi: G \times Q \to Q$, and assume that $\psi$ and $\phi$ commute. This situation arises naturally when $K$ is a subgroup of $G$ and, in turn,  $G_{q_0}$ is a normal subgroup of $K$. In that case one can define
\begin{equation}
...
%
\item Let $\mathcal{H}:T^*G \to \mathbb{R}$ be a right-invariant Hamiltonian. This means that $\mathcal{H}$ is invariant with respect to the natural right action $TR^*$ of $G$ on $T^*G$. In particular, the reduced Hamiltonian $h:\mathfrak{g}^* \to \mathbb{R}$ satisfies $\mathcal{H}(\alpha_g) = h \circ TR_g^*(\alpha_g)$ for any $\alpha_g \in T^*G$, and the reduced dynamics in $\mathfrak{g}^*$ are of Lie--Poisson type. The momentum map $J_1$ can be used to induce the so-called \emph{collective Hamiltonian} $H = h\circ J_1$ on $T^*Q$. Note that $T^*Q$ is a symplectic manifold, and that the symplectic (canonical) dynamics with respect to the collective Hamiltonian are mapped by (the Poisson map) $J_1$ to the reduced dynamics on $\mathfrak{g}^*$. Moreover, $J_2$ is conserved under the dynamics on $T^*Q$. The conservation law follows from Noether's theorem because $J_1$, and hence $H$, are left invariant by $T^*\psi$ (see Theorem \ref{see thm:dual_pairs}).  Note that the elements of $T^*Q$ play the role of Clebsch variables in the sense of \cite{MarsdenWeinstein1983}. 
%
