...
\hoj{I see.  So $\phi$ is a left action.  One question though, $\psi$ a right action right?} \todo{Yes and no. $\phi$ is a left action. $\psi$ does not HAVE to be a right action. The point is that it has to commute with $\phi$. Of course, in our context, we do indeed work with a right action, but I don't think this is a prerequisite.}
for any $g \in G$ such that $\Pi(g) = q$, and hence we can identify $Q$  with $G/G_{q_0}$. Consider a further group, $K$, which also acts on $Q$ with group action $\psi: Q \times K \to Q$, and assume that $\psi$ and $\phi$ commute. This situation arises naturally when $K$ is a subgroup of $G$ and, in turn,  $G_{q_0}$ is a normal subgroup of $K$. In that case, one can \todo{I have reverted this. This really is a definition of what $\psi$ is} define 
\begin{equation}
