...
\title[Symplectic aspects]{Symplectic aspects of a 
  computational particle method for ideal fluid flow.}
...
ideal incompressible fluids, which we have dubbed jetlet methods.
...
calculated.
Later, the higher-order jetlet methods were developed
by equipping the particles with internal group variables \cite{CotterHolmJacobsMeier2014}.
...
In this paper we seek to change this.
The main contributions are:
\begin{enumerate}
  \item We determine dual-pairs which induce these jetlet methods.
...
The unreduced Hamiltonian is $H(R,P) = h(J_R(R,P))$.
  ...
  suggests that the equations of motion can be written in terms of $\Pi$ along.
