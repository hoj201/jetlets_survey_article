...
The transformation to Eulerian velocity in Hamilton's principle for ideal incompressible  fluids also projects the cotangent bundle $T^*{\rm SDiff}(M)$ for Lagrangian particle motion to the momentum $m_t\in \mathfrak{X}^*(M)$, dual the Eulerian vector fields $u_t\in \mathfrak{X}(M)$. The corresponding coadjoint action ${\rm Ad}^*_{g_t^{-1}}$ of ${\rm SDiff}(M)$ on  $\mathfrak{X}^*(M)$ then governs the motion, as illustrated in the figure below.  

\centerline{
$\,$\hspace{-15mm}
\begin{picture}(150,100)(-15,0)
\put(20,75){\vector(1,0){65} }
\put(23,55){\footnotesize Equivariant}
\put(12,45){\footnotesize Momentum Map}
\put(-47,72){$T^*{\rm SDiff}(M)$}
\put(90,72){$T^*{\rm SDiff}(M)$}
\put(40,83){$\Phi_{g_t}$}
\put(100,65){\vector(0,-1){37} }
\put(105,45){$m_t$}
\put(20,18){\vector(1,0){65} }
\put(35,27){${\rm Ad}^*_{g_t^{-1}}$}
\put(0,65){\vector(0,-1){37} }
\put(-20,15){$\mathfrak{X}^*(M)$}
\put(90,15){$\mathfrak{X}^*(M)$}
\put(-29,45){$m_0$}
\put(174,73){\color{black}Lagrangian flow}
\put(195,65){\vector(0,-1){35} }
\put(200,55){\footnotesize Reduce by}
\put(200,45){\footnotesize relabelling}
\put(200,35){\footnotesize symmetry}
\put(174,17){Eulerian flow}
\end{picture}}
%\includegraphics[width=0.4\textwidth]{Ad-map.pdf}
%
That is, the volume preserving diffeomorphisms ${\rm SDiff}(M)$ act on their cotangent bundle $T^*{\rm SDiff}(M)$ by the \emph{cotangent lift} $\Phi_{g_t}$ of flows of divergence free vector fields $\mathfrak{X}(M)$. The map $T^*{\rm SDiff}(M)\to\mathfrak{X}^*(M)$ reduces the canonical Hamiltonian motion on the Lagrangian particle phase space $T^*{\rm SDiff}(M)$ to coadjoint motion on the dual space $\mathfrak{X}^*(M)$. This map is known as the \emph{momentum map} for ideal incompressible fluids, and it is \emph{equivariant}. That is, the diagram commutes. The momentum map is also Poisson, and its evolution may be expressed as Lie-Poisson Hamiltonian dynamics \cite{HolmMarsdenRatiu1998}. 

The momentum map $m_t:T^*{\rm SDiff}(M)\to\mathfrak{X}^*(M)$ evolves via the EPSDiff$(M)$ equation:
\begin{equation}
\frac{dm_t}{dt} = -\, {\rm ad}_{u_t}^* m_t 
\quad\hbox{for}\quad 
m_t := \frac{\delta \ell}{\delta u_t}\in\mathfrak{X}^*(M)
\,,\label{EPDiff-eqn}
\end{equation}
which is determined from the time derivative of the coadjoint motion by ${\rm Ad}^*_{g_t^{-1}}$. 
Equation \eqref{EPDiff-eqn} for the evolution of $m_t$ yields Euler's fluid equations when $\ell = \frac12\|u_t\|^2_{L^2}$ (fluid kinetic energy). Thus, as in \cite{Arnold1966}, Euler fluid motion $\simeq$ EPSDiff$(M)_{L^2}$, which is \emph{geodesic}.
...
%\centerline
\hspace{5mm}{
Eulerian velocity, $u_t=\dot{g}_tg_t^{-1}$ \hfill Convective velocity, $v_t=g_t^{-1}\dot{g}_t$
}
\bigskip
...
\fbox{volume preserving (?)} diffeomorphisms, these particle-like solutions corresponding to the jet groups paint an intuitive picture of a large scale diffeomorphism that advects particles,
...
of diffeomorphism \fbox{self-similarity ?}.
...
\section{Conclusion}
\todo[inline]{We need to write a conclusion}
...
\begin{align*}
  ...
  [\mu^{(1)}_b]\indices{_{j}^{k}} \partial_k K^{i j}(q_a - q_b) \\
&+p_{a i} [\mu^{(2)}_b] \indices{_j^{k l}} \partial_{l k} K^{i j}(q_a - q_b)
 -\frac{1}{2} [\mu_a^{(1)}]\indices{_{i}^l}
  [\mu_b^{(1)}]\indices{_j^k} \partial_{l k} K^{i j}(q_a - q_b) \\
&+[\mu_a^{(1)}]\indices{_i^m} [\mu_b^{(2)}]\indices{_j^{k l}} \partial_{m k l} K^{i j}(q_a - q_b) \\
&+\frac{1}{2} [\mu_a^{(2)}]\indices{_i^{m n}} [\mu_b^{(2)}]\indices{_j^{k l} }
  \partial_{k l m n} K^{i j}(q_a - q_b)
\end{align*}
...
\begin{align*}
 ...
 +[\mu^{(2)}_b] \indices{_j^{k l}} \partial_{k l} K^{i j}(q_a - q_b)
\end{align*}
equation~\eqref{eq:dHdq} is given by the sum
\begin{align*}
  \dot{p}_{a i} = T^{00}_{a i} + T^{01}_{a i} + T^{02}_{a i}
                + T^{12}_{a i} + T^{11}_{a i} + T^{22}_{a i}
\end{align*}
...
\begin{align*}
  [\dot{q}^{(1)}]\indices{^i_j} &=
  [\xi^{(1)}]\indices{^i_k} [q^{(1)}]\indices{^k_j} \\
  [\dot{q}^{(2)}]\indices{^i_{j k}} &=
  [\xi^{(2)}]\indices{^i_{l m}} \cdot [q^{(1)}]\indices{^l_j} \cdot [q^{(1)}]\indices{^m_k}
 +[\xi^{(1)}]\indices{^i_l}     \cdot [q^{(2)}]\indices{^l_{j k}}
\end{align*}
